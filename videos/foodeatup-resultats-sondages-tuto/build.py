#!/usr/bin/env python3
# FoodEatUp "Resultats des sondages" tutorial (module Marketing, Fidelite &
# Iris, catalogue #18). Same engine as every FoodEatUp tuto (see
...
# every cut, "Utiliser avec Claude" 3-stage sequence (shared module) since
# mcp__Foodeatup__get_survey_results matches exactly what the rush shows.
# 48kHz stereo AAC, +faststart. No crop needed: rush is already 1920x828.
...
BTN_LIEN      = (1348, 698); SZ_LIEN      = (110, 40)   # "Lien" (sondage express card)
BTN_ENVOYER   = (696, 719);  SZ_ENVOYER   = (378, 60)   # "Envoyer mes reponses" (page publique)
BTN_RESULTATS = (1496, 698); SZ_RESULTATS = (150, 40)   # "Resultats" (sondage express card)

# (name, src_start, src_end, target_out_duration, click_time, button, btn_size, caption)
segs = [
    ("A", 0.00,  2.30,  3.00, None,  None,          None,          "1 - Fidelite et jeux, Sondages"),
    ("B", 2.30,  2.60,  0.90, 2.45,  BTN_LIEN,       SZ_LIEN,       None),
    ("C", 9.50, 11.00,  8.50, None,  None,           None,          "2 - Formulaire public"),
    ("D", 16.50, 16.80, 0.90, 16.65, BTN_ENVOYER,     SZ_ENVOYER,    None),
    ("E", 18.00, 19.30,  3.00, None,  None,           None,          "Reponse envoyee"),
    ("F", 20.60, 21.00,  2.20, None,  None,           None,          "3 - Retour sur FoodEatUp"),
    ("G", 21.00, 21.30,  0.90, 21.15, BTN_RESULTATS,   SZ_RESULTATS,  None),
    ("H", 22.00, 29.00, 14.50, None,  None,           None,          "Resultats du sondage"),
]
INTRO_D, OUTRO_D = 6.80, 6.00
...
# videos/FOODEATUP-TUTORIELS-WORKFLOW.md. mcp__Foodeatup__get_survey_results
# matches exactly what the rush shows (moyennes, NPS, repartitions).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Montre-moi les résultats du sondage [nom du sondage] pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère les résultats de ce sondage pour votre établissement…"
# Stage1+2 combined sized to N6's 4.41s duration (same reused mp3 as
# foodeatup-dlc-tuto, which sizes its own stage1+2 to 2.60+1.80=4.40s for
# the identical line -- matching that here). Stage3 sized to N7 (4.62s).
CLAUDE_STAGE_D = [2.60, 1.80, 5.00]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut: click Lien)
             "slideleft",  # B -> C (cut, new tab, public survey)
             "slideleft",  # C -> D (cut: click Envoyer)
             "slideleft",  # D -> E (cut, confirmation)
             "slideleft",  # E -> F (cut, back to FoodEatUp)
             "fade",       # F -> G (continuous: click Resultats)
             "slideleft",  # G -> H (cut into the results panel)
             "slideleft",  # H -> claude1
             "slideleft",  # claude1 -> claude2
             "slideleft",  # claude2 -> claude3
             "slideleft"]  # claude3 -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # opens Sondages, clicks Lien
    "N2": S["C"] + 0.20,         # public survey form
    "N3": S["G"] + 0.20,         # click Resultats
    "N4": S["H"] + 0.20,         # results panel
    "N5": S["H"] + 0.20,         # benefit, same anchor -- sequential placement pushes it right after N4
