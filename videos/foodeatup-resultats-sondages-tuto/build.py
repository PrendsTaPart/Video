#!/usr/bin/env python3
# FoodEatUp "Resultats des sondages (historique)" tutorial (module Marketing,
...
#
# Rush (30.80s) shows the full loop: Fidelite & jeux > Sondages (carte
# "sondage express") -> clic Lien (copie) -> vue publique du sondage cote
# client (demo, notation, NPS, checkbox, envoi) -> confirmation -> retour
# marchand -> clic Resultats -> page de resultats (moyenne, NPS, detail).
...
ROOT = "/home/user/Video/videos/foodeatup-resultats-sondages-tuto"
...
# Coordinate measured by colour-thresholding/visual inspection of the
# "Resultats" button on the real frame at t=21.7s (work/frames/
# clic_resultats.png), just before the click. Centre (1496, 698), size
# (144, 36). No zoom-punch on the "Lien" button (segment B): smaller,
# secondary action, kept as a plain scaled segment.
BTN_RESULTATS = (1496, 698); SZ_RESULTATS = (144, 36)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",     0.00,  4.00, 6.00, None,  None,          None,          "Sondages : votre sondage actif"),
    ("B",     4.00,  6.00, 4.60, None,  None,          None,          "Copier le lien du sondage"),
    ("C",     8.00, 18.50, 7.50, None,  None,          None,          "Vue client : repondre au sondage"),
    ("D",    19.00, 21.60, 3.90, None,  None,          None,          "Retour au tableau de bord"),
    ("clic", 21.60, 22.00, 0.90, 21.80, BTN_RESULTATS, SZ_RESULTATS,  None),
    ("E",    22.50, 30.80, 8.50, None,  None,          None,          "Resultats du sondage"),
]
# Durations sized against measured VO length (N0..N8: 3.24/4.26/4.26/5.69/
# 3.60/5.02/4.41/4.31/5.02s) -- rule from FOODEATUP-TUTORIELS-WORKFLOW.md:
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__get_survey_results
# (establishment_id, survey_id) is the matching tool -- direct, no-reserve
# correspondence with the "Resultats" screen shown in the rush.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Affiche les résultats de mon sondage fidélité (ID [ID du sondage]) "
                  "pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici les résultats de votre sondage…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (carte -> clic lien)
             "slideleft",  # B -> C (marchand -> vue client)
             "slideleft",  # C -> D (envoi -> merci/retour)
             "fade",       # D -> clic (continuous: click on the visible page)
             "slideleft",  # clic -> E (cut into the results page)
             "slideleft",  # E -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # sondage actif
    "N2": S["B"] + 0.20,         # partager le lien
    "N3": S["C"] + 0.20,         # remplissage cote client
    "N4": S["D"] + 0.20,         # merci + retrouvez la reponse
    "N5": S["E"] + 0.20,         # page resultats
...
FINAL = f"{ROOT}/out/foodeatup-resultats-sondages-tuto-v1.mp4"
