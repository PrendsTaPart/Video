#!/usr/bin/env python3
# FoodEatUp "Lancer une campagne marketing" tutorial (Marketing, Fidélité & Iris module).
# No avatar clip: full ElevenLabs VO throughout (Adam Instructor FR). Speed =
...
#
# Rush: accueil "Campagnes & automatisations" -> clic "Nouvelle campagne" ->
# modale 4 étapes (1. Cible -> 2. Message -> 3. Planification -> 4. Conformité)
# -> sélection segment "Tous les clients" (39) -> nom "Reconquête clients à
# risque" + canal Email + message -15%/RETOUR15 -> "Envoyer maintenant" ->
# vérification conformité (39 -> 38 contactables, 0,08€) -> "Lancer vers 38
# client(s)" -> toast "Campagne lancée..." -> retour liste. Trois points de
# clic zoom-punch : "Nouvelle campagne", la carte segment "Tous les clients",
# "Lancer vers 38 client(s)". Coordonnées mesurées par seuillage couleur (bleu
...
ROOT = "/home/user/Video/videos/foodeatup-campagne-marketing-tuto"
...
# les frames réelles du screen recording, vérifiées par recadrage visuel
# (voir SCRIPT.md). Pas de banner() sur cette vidéo : la modale affiche déjà
# ses propres étapes ("1. Cible / 2. Message / 3. Planification / 4.
# Conformité") en toutes lettres, un bandeau superposé serait redondant.
BTN_NOUVELLE_CAMPAGNE = (1672, 340); SZ_NOUVELLE_CAMPAGNE = (280, 84)   # bouton "+ Nouvelle campagne"
BTN_TOUS_LES_CLIENTS  = (943, 242);  SZ_TOUS_LES_CLIENTS  = (1100, 110) # carte segment "Tous les clients"
BTN_LANCER            = (1333, 653); SZ_LANCER            = (340, 80)  # bouton "Lancer vers 38 client(s)"

# (name, src_start, src_end, target_out_duration, button_or_None, btn_size_or_None)
segs = [
    ("A", 0.30,  3.90,  2.40, None,                  None),
    ("B", 4.00,  4.35,  0.90, BTN_NOUVELLE_CAMPAGNE,  SZ_NOUVELLE_CAMPAGNE),  # clic "Nouvelle campagne"
    ("C", 4.60,  5.80,  3.00, None,                  None),                  # étape 1. Cible -- liste segments
    ("D", 5.80,  6.15,  0.90, BTN_TOUS_LES_CLIENTS,   SZ_TOUS_LES_CLIENTS),   # sélection "Tous les clients"
    ("E", 8.00,  19.30, 6.60, None,                  None),                  # étape 2. Message -- nom + canal + texte
    ("F", 19.30, 31.60, 6.00, None,                  None),                  # variables / offre / code promo / url
    ("G", 32.00, 36.60, 4.50, None,                  None),                  # étape 3. Planification -- envoyer maintenant
    ("H", 38.50, 39.90, 3.40, None,                  None),                  # étape 4. Conformité -- récap 39/38/0,08€
    ("I", 40.20, 40.55, 0.90, BTN_LANCER,             SZ_LANCER),            # clic "Lancer vers 38 client(s)"
    ("J", 45.50, 47.50, 3.60, None,                  None),                  # toast "Campagne lancée..."
    ("K", 48.00, 51.16, 2.20, None,                  None),                  # liste mise à jour
]
...
# tools: mcp__FoodEatUp__create_campaign(establishment_id, name, channel,
# segment, message_template, offer_label, promo_code, link_url) crée le
# brouillon, puis mcp__FoodEatUp__launch_campaign(establishment_id,
# campaign_id, confirm:true) l'envoie réellement -- exactement le flux filmé
# (Continuer... -> "Lancer vers N client(s)").
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une campagne email nommée [nom de la campagne] pour le "
                  "segment [tous/champions/fideles/prometteurs/a_risque/perdus/"
                  "nouveaux], avec le message « [votre message, avec {prenom}] », "
                  "l'offre [-15%] et le code promo [CODE], puis lance-la pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée la campagne et je la lance…"
...
CLAUDE_STAGE_D = [2.80, 2.40, 6.00]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: clic sur le bouton visible)
             "slideleft",  # B -> C (cut: ouverture de la modale)
             "fade",       # C -> D (continuous: sélection du segment)
             "slideleft",  # D -> E (cut: passage à l'étape Message)
             "fade",       # E -> F (continuous: scroll dans la même étape)
             "slideleft",  # F -> G (cut: passage à l'étape Planification)
             "slideleft",  # G -> H (cut: passage à l'étape Conformité)
             "fade",       # H -> I (continuous: clic sur Lancer)
             "fade",       # I -> J (résultat: toast de confirmation)
             "slideleft",  # J -> K (cut: retour à la liste complète)
             "slideleft",  # K -> claude1
...
anchor = {
    "N0":  0.30,                 # intro hook
    "N1":  S["B"] + 0.05,        # clic "Nouvelle campagne"
    "N2":  S["D"] + 0.10,        # sélection du segment "Tous les clients"
    "N3":  S["E"] + 0.15,        # nom + canal
    "N4":  S["F"] + 0.15,        # message + offre + code promo
    "N5":  S["G"] + 0.15,        # planification "Envoyer maintenant"
    "N6":  S["H"] + 0.15,        # conformité (39 -> 38, 0,08€)
    "N7":  S["I"] + 0.05,        # clic "Lancer" -> toast
...
FINAL = f"{ROOT}/out/foodeatup-campagne-marketing-tuto-v1.mp4"
