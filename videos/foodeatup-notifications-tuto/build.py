#!/usr/bin/env python3
# FoodEatUp "Lire ses notifications et taches du jour" tutorial (module Equipe &
# Planning). Same engine as every FoodEatUp tuto (see
...
#
# Rush (18.6s, 1920x1020): "mon espace" (hub employe) -> clic sur la cloche de
# notifications -> panneau "Mes notifications" (5 notifications : conge
# approuve, nouveau shift, tache HACCP a faire, modification planning, rappel
# solde conges) -> filtres Tout / Conges / Planning / Taches, cliques un a un.
#
# Pas de sequence Claude ici : aucun outil MCP FoodEatUp ne couvre la lecture
# des notifications employe (flux self-service cote client), meme raison que
# creer-son-code-pin / se-connecter-cote-employe.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-notifications-tuto"
SRC  = f"{ROOT}/assets/screen.mp4"
W, H, FPS = 1920, 1020, 25
SEG = f"{ROOT}/work/seg"
...
# Coordinates measured on the extracted frames (native 1920x1020), same method
# as the rest of the series. No apostrophe in any caption below.
BTN_BELL     = (1670, 325); SZ_BELL     = (110, 110)  # cloche de notifications
BTN_CONGES   = (1295, 533); SZ_CONGES   = (150, 50)   # filtre Conges
BTN_PLANNING = (1445, 533); SZ_PLANNING = (150, 50)   # filtre Planning
BTN_TACHES   = (1590, 533); SZ_TACHES   = (140, 50)   # filtre Taches

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.00,  3.60, None,  None,        None,        "1 · Mon espace"),
    ("B", 5.40,  5.70,  1.00, 5.55,  BTN_BELL,     SZ_BELL,     None),
    ("C", 5.80,  9.60,  4.80, None,  None,        None,        "2 · Mes notifications"),
    ("D", 10.00, 10.30, 1.00, 10.15, BTN_CONGES,   SZ_CONGES,   None),
    ("E", 10.35, 13.10, 3.20, None,  None,        None,        "Filtre : Congés"),
    ("F", 13.30, 13.60, 1.00, 13.45, BTN_PLANNING, SZ_PLANNING, None),
    ("G", 13.65, 15.30, 2.20, None,  None,        None,        "Filtre : Planning"),
    ("H", 15.40, 15.70, 1.00, 15.55, BTN_TACHES,   SZ_TACHES,   None),
    ("I", 15.75, 18.60, 3.80, None,  None,        None,        "Filtre : Tâches"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: clic cloche)
             "fade",       # B -> C (continuous: panneau ouvert)
             "fade",       # C -> D (continuous: clic filtre Conges)
             "fade",       # D -> E (continuous: vue filtree)
             "fade",       # E -> F (continuous: clic filtre Planning)
             "fade",       # F -> G (continuous: vue filtree)
             "fade",       # G -> H (continuous: clic filtre Taches)
             "fade",       # H -> I (continuous: vue filtree)
             "fade"]       # I -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # cloche / compteur
    "N2": S["C"] + 0.30,         # types de notifications
    "N3": S["D"] + 0.10,         # filtres (Conges/Planning/Taches)
    "N4": S["I"] + 0.30,         # tache HACCP urgente
...
FINAL = f"{ROOT}/out/foodeatup-notifications-tuto-v1.mp4"
