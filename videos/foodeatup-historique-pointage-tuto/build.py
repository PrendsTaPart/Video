#!/usr/bin/env python3
# FoodEatUp "Retrouver les pointages (historique)" tutorial.
#
# Short rush (18.6s): "Pointage" table (employe/date/heures/pauses/temps
# total/statut) filtered by date range, export to PDF. Matches
# list_attendances(establishment_id, date_from, date_to) exactly -- shared
# "use it with Claude" 3-stage sequence included.
#
# No avatar. Same engine as the rest of the series: setpts for speed (never
# zoompan on real footage), fixed crop+scale zoom-punch on the export click,
...
ROOT = "/home/user/Video/videos/foodeatup-historique-pointage-tuto"
...
BTN_EXPORT = (1262, 284); SZ_EXPORT = (240, 46)  # "Exporter PDF"

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  0.00,  8.00, 4.70, None,       None,     "Historique des pointages"),
    ("A2", 8.00, 10.00, 5.10, BTN_EXPORT, SZ_EXPORT, None),
    ("B",  11.00, 18.64, 6.20, None,      None,     "Export PDF"),
]
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared 3-stage chatbot animation. Matches
# list_attendances(establishment_id, date_from, date_to).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Fais-moi un dashboard des pointages de mon équipe du [date "
                  "début] au [date fin] pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère les pointages et je vous prépare ça…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> A2 (continuous: click Exporter PDF)
             "slideleft",  # A2 -> B (PDF opens)
             "slideleft",  # B -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]  + 0.20,  # table + date range
    "N2": S["A2"] + 0.10,  # click Exporter PDF
    "N3": S["B"]  + 0.20,  # benefice (rapport pret)
    "N4": S["claude1"] + 0.20,  # explains the prompt (reveal + copied)
    "N5": S["claude3"] + 0.20,  # paste into Claude -> instant result
...
FINAL = f"{ROOT}/out/foodeatup-historique-pointage-tuto-v1.mp4"
