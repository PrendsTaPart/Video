#!/usr/bin/env python3
# FoodEatUp "Brancher son MCP sur Claude" tutorial.
#
# No avatar, and no "use it with Claude" 3-stage sequence at the end --
# unlike every other tutorial, this ENTIRE video already is "how to use it
# with Claude" (copy the MCP URL, add a custom connector in Claude, connect,
# authorize). Adding the usual appendix would be circular.
#
# Rush (49s): FoodEatUp Marketplace de prompts (copy MCP URL) -> Claude
# Parametres > Connecteurs > Ajouter un connecteur personnalise (name + URL)
# -> click Ajouter -> tool list revealed -> OAuth "Autoriser" screen ->
# final connected state with per-tool permissions.
#
# Same engine as the rest of the series: setpts for speed (never zoompan on
# real footage), fixed crop+scale zoom-punch on each click, xfade on every
# cut forced back to yuv420p, 48kHz stereo AAC, +faststart.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-mcp-tuto"
...
def banner(text, seg_dur):
    ...
    # See foodeatup-qrcode-tuto/build.py: drawbox's expression evaluator in
    # this ffmpeg (6.1.1) silently fails to draw when x combines a leading
    # constant with two subtracted min(1,max(0,...)) clamps. Fixed from the
    # start here: single slide-in clamp only, no animated slide-out.
    tin, sl = 0.15, 0.32
...
# Coordinates measured by extracting full-res frames around each click.
BTN_COPY = (1663, 429); SZ_COPY = (213, 52)  # "Copier l'URL" (Marketplace)
BTN_ADD  = (1271, 742); SZ_ADD  = (117, 50)  # "Ajouter" (connecteur personnalise)
BTN_AUTH = (742, 595);  SZ_AUTH = (140, 46)  # "Autoriser" (OAuth)

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A", 0.00,  7.00, 4.90, BTN_COPY, SZ_COPY, "Copier le lien MCP"),
    ("B", 12.00, 26.00, 6.60, None,    None,    "Connecteur personnalise Claude"),
    ("C", 26.00, 27.50, 2.80, BTN_ADD, SZ_ADD,  None),
    ("D", 28.00, 33.00, 3.50, None,    None,    "Outils FoodEatUp disponibles"),
    ("E", 34.00, 37.50, 4.80, BTN_AUTH, SZ_AUTH, None),
    ("F", 42.00, 48.00, 6.20, None,    None,    "Connexion active"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut to Claude Connecteurs)
             "fade",       # B -> C (continuous: click Ajouter)
             "slideleft",  # C -> D (page reload -> tool list)
             "slideleft",  # D -> E (cut to OAuth screen)
             "slideleft",  # E -> F (cut to final connected state)
             "fade"]       # F -> outro
...
GAP = 0.22
anchor = {
    "N1": S["A"] + 0.20,  # copy MCP URL
    "N2": S["B"] + 0.20,  # Claude Connecteurs -> Ajouter un connecteur personnalise
    "N3": S["C"] + 0.10,  # click Ajouter
    "N4": S["D"] + 0.20,  # tool list revealed
    "N5": S["E"] + 0.10,  # Autoriser
    "N6": S["F"] + 0.20,  # benefice
...
FINAL = f"{ROOT}/out/foodeatup-mcp-tuto-v1.mp4"
