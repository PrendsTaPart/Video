#!/usr/bin/env python3
# FoodEatUp "Voir son planning cote employe" tutorial (module Equipe & Planning).
...
#
# Rush (26.2s, 1920x1020): page "Mon planning" (employe) -> stats (heures, shifts,
# taches) -> scroll semaine (repos / taches par jour) -> coche "Prendre les
# commandes" comme faite (toast "Tache faite") -> compteur de taches passe a 1/3 ->
# clic "Ajouter a mon agenda" (export ICS, telechargement navigateur).
#
# Pas de sequence Claude classique ici, mais un seul claudePrompt cote Lovable
# base sur mcp__FoodEatUp__list_plannings (shifts de la semaine) -- outil en
# lecture seule, pas d'action de creation/mise a jour a demontrer dans le rush.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-planning-employe-tuto"
...
BTN_CHECKBOX = (150, 500);   SZ_CHECKBOX = (60, 60)    # case "Prendre les commandes"
BTN_AGENDA   = (1640, 527);  SZ_AGENDA   = (360, 85)   # "Ajouter a mon agenda"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.00,  4.50, None,  None,        None,        "1 · Mon planning"),
    ("B", 3.00,  6.50,  5.00, None,  None,        None,        "2 · Vos horaires et tâches"),
    ("C", 13.00, 13.50, 1.00, 13.25, BTN_CHECKBOX, SZ_CHECKBOX, None),
    ("D", 13.60, 16.50, 3.60, None,  None,        None,        "Tâche faite"),
    ("E", 18.00, 19.60, 2.60, None,  None,        None,        "3 · Tâches : 1/3"),
    ("F", 20.70, 21.15, 1.00, 20.90, BTN_AGENDA,  SZ_AGENDA,   None),
]
INTRO_D, OUTRO_D = 3.90, 6.20
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous scroll)
             "slideleft",  # B -> C (cut: retour au haut et clic checkbox)
             "fade",       # C -> D (continuous: tache cochee)
             "slideleft",  # D -> E (cut: retour en haut, stats)
             "fade",       # E -> F (continuous: clic Ajouter a mon agenda)
             "fade"]       # F -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # stats de la semaine
    "N2": S["B"] + 0.30,         # scroll jour par jour
    "N3": S["C"] + 0.10,         # clic case + tache faite
    "N4": S["E"] + 0.20,         # compteur mis a jour
    "N5": S["F"] + 0.10,         # clic Ajouter a mon agenda
...
FINAL = f"{ROOT}/out/foodeatup-planning-employe-tuto-v1.mp4"
