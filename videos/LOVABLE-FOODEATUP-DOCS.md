...
| 4 | Service Multi-Canal | 04 · Retrouver ses commandes | `retrouver-ses-commandes-multi-canal` | **oui, 3 prompts** — tous basés sur `mcp__FoodEatUp__list_orders` (filtres période/canal/statut combinés, commandes en attente du jour, suivi d'un canal sur 7 jours). Intrants fournis par Michael (carte intro `COMMANDES_MULTICANAUX.jpg`, carte outro CTA, rush `Retrouver_vos_commandes_multiCanal.mp4`, 30,88 s). Rush : widget "Commandes" du tableau de bord (recherche, filtres période, pastilles statut, pastille canal "Web · 3") → clic sur une commande → fiche détail (canal/statut/paiement/mode/table/téléphone/articles/totaux/client/notes) → fermeture → "Ouvrir la gestion complète" → page "Mes commandes" (stats + recherche + filtres Statut/Canal/Date). Complémentaire de `mes-commandes-tous-canaux` (déjà publié, axé création/modification/suppression) : angle recherche/filtrage/consultation, gardé en entrée séparée. `videos/foodeatup-commandes-multicanal-tuto/` (61,80 s, peak -7,2 dBFS, voix Adam FR, dérive nulle sur tous les ancrages sauf N1/N8 <0,7s). Publiée le 2026-08-06 : RapidoCMS (`foodeatup-commandes-multicanal-tuto-v1` / `-thumbnail`) + Lovable (module `service-commande`, `expectedCount` 3→4). LinkedIn non demandé pour cette vidéo. |
| 2 | Service Multi-Canal | 02 · Canaux de commande | `commander-sur-site-qr-ou-vocal` | **oui** — `create_order` (channel `vitrine`, mode `sur_place`, table renseignée) : correspond exactement à la commande passée par le client via le QR. Intrants fournis par Michael (carte intro `COMMANDER_PAR_QR_CODE.jpg`, carte outro CTA réutilisée, rush `scanner_le_qrcode_de_la_table_pour_realiser_une_commande.mp4`, 75,96 s). Rush en deux temps : côté staff (Plan de salle → sélection table QA3 → bouton "QR code de la table" → modale avec lien) puis côté client (page "Gosushi démo" par catégories → ajout au panier → "Commander" → toast "Commande transmise en cuisine !" → écran de suivi `cmd-2026-00109` → "Payer l'addition" → partage Tout/÷2/÷3/÷4 → paiement Stripe Link/Visa). Les 13 premières secondes (édition d'une réservation, hors sujet) et ~8 s de bascule d'onglet navigateur (bruit d'enregistrement) coupées au montage. **Remplit un slot Lovable préexistant** (placeholder "en cours de tournage" du module `service-commande`, section "Canaux de commande", pas de nouvelle entrée créée). `videos/foodeatup-qrcode-commande-tuto/` (70,12 s, peak -7,15 dBFS, voix Adam FR, dérive mineure <2 s sur N1/N10). Publiée le 2026-08-06 : RapidoCMS (`foodeatup-qrcode-commande-tuto-v1` / `-thumbnail`) + Lovable (mise à jour de l'entrée existante, pas de changement d'`expectedCount`). Pas de prompt pour l'agent vocal (absent du rush) ni pour le paiement Stripe (aucun outil MCP côté client). LinkedIn non demandé pour cette vidéo. |
