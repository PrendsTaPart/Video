...
| 10 | HACCP | Retrouver mes plats sondés (historique) | `retrouver-historique-plats-sondes` | **oui** — `list_haccp_temperatures(establishment_id, start_date?, end_date?)`, sans `equipment_id`, sur une période (même outil que la vidéo sœur `relever-une-temperature-equipement`, côté Plats cette fois). Rush fourni par Michael (Production > Températures > onglet Plats : ajustement + Enregistrer + confirmation, puis Historique > Températures > onglet Plats : 6 KPIs, recherche/filtre/export CSV, liste des plats sondés avec statut conforme/non conforme). 56,48 s, voix Adam FR ElevenLabs (N6/N8 réutilisées telles quelles depuis `foodeatup-tva-tuto/vo/`). Fiche Lovable préexistante en placeholder ("en cours de tournage", créée par une session concurrente) mise à jour avec vidéo/vignette/étapes/astuce du chef/prompt Claude plutôt que dupliquée (commit `77f46de`). Publiée sur instruction explicite de Michael ("continue le montage vidéo et ajoute à lovable") : RapidoCMS (`foodeatup-plats-sondes-historique-tuto-v1` / `-thumbnail`) + Lovable (site redéployé, https://foodeatup-guide-star.lovable.app). **LinkedIn non demandé** — pas de brouillon programmé. Voir `videos/foodeatup-plats-sondes-historique-tuto/SCRIPT.md`. |
| 8 | Marketing, Fidélité & Iris | Gérer les récompenses fidélité | `gerer-les-recompenses-fidelite` | **oui** — `upsert_loyalty_reward(establishment_id, kind, label, points_cost, plat_id?, amount?, stock?, active?, reward_id?)` : couvre les 3 types de récompenses vus dans le rush (`product`/`amount`/`perk`) et à la fois la création et la modification — la meilleure correspondance MCP trouvée sur la série jusqu'ici, aucune réserve. Rush fourni par Michael (Fidélité & jeux > Récompenses : modification d'une récompense Bon €→Produit, ajout d'une récompense Bon €, ajout d'une récompense Avantage, 2 clics "Enregistrer le catalogue"). 70,64 s, voix Adam FR ElevenLabs (N7/N9 réutilisées telles quelles depuis `foodeatup-tva-tuto/vo/`). Script présenté et validé avant génération VO/montage (STOP pipeline respecté), vidéo livrée et validée avant publication (2e STOP respecté). Fiche Lovable préexistante en placeholder mise à jour plutôt que dupliquée (commit `9085bae`). Publiée : RapidoCMS (`foodeatup-gerer-recompenses-tuto-v1` / `-thumbnail`) + Lovable (site redéployé). **LinkedIn non demandé** — pas de brouillon programmé. Voir `videos/foodeatup-gerer-recompenses-tuto/SCRIPT.md`. |
| 8 | Marketing, Fidélité & Iris | Résultats des sondages — historique | `resultats-des-sondages-historique` | **oui, sans réserve** — `get_survey_results(establishment_id, survey_id)`, correspondance directe avec l'écran "Résultats" (moyennes, NPS, répartitions). Rush fourni par Michael (Fidélité & jeux > Sondages : partage du lien du sondage actif, remplissage démo côté client — étoiles, score NPS, checkbox —, confirmation, retour marchand, consultation des résultats). 53,00 s, voix Adam FR ElevenLabs (N6/N8 réutilisées telles quelles depuis `foodeatup-tva-tuto/vo/`). Script présenté et validé avant génération VO/montage (STOP pipeline respecté), vidéo livrée et validée avant publication (2e STOP respecté). Fiche Lovable préexistante en placeholder mise à jour plutôt que dupliquée (commit `69d1d3b`) — sondage voisin `creer-un-sondage-fidelite` (#17) laissé intact, pas les mêmes intrants. Publiée : RapidoCMS (`foodeatup-resultats-sondages-tuto-v1` / `-thumbnail`) + Lovable (site redéployé). **LinkedIn non demandé** — pas de brouillon programmé. Voir `videos/foodeatup-resultats-sondages-tuto/SCRIPT.md`. |
