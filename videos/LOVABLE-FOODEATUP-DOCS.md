...
| — | **Agent IA Caroline** *(nouveau module `caroline-ia`, première vidéo)* | 04 - Gérer ses Tables (ajout & blocage) | `gerer-ses-tables-ajout-blocage` | **oui, 2 prompts** — `create_table` (prompt vidéo, séquence Claude) + `update_table_status` (`claudePrompts[1]`, fiche Lovable uniquement — les deux actions sont montrées à l'écran mais la règle du pipeline limite la vidéo à un seul prompt affiché). Rush + cartes intro/outro fournis par Michael. Montage `videos/foodeatup-gerer-tables-tuto/` (63,40 s, peak -7,21 dBFS, dérive nulle après correctif du zoom-punch final — voir `SCRIPT.md`). Validée par Michael le 2026-08-07 (« tu peux publier »). Publiée : RapidoCMS (`foodeatup-gerer-tables-tuto-v1` / `foodeatup-gerer-tables-tuto-thumbnail`). **LinkedIn : brouillon bloqué par le classificateur de permissions du mode auto** (action de publication sur réseau social public) — créneau prévu 2026-08-26 07h (prochain libre de la rotation FoodEatUp, pleine jusqu'au 2026-08-25), à créer manuellement ou avec une autorisation explicite. **Lovable : message envoyé mais file d'attente du projet en pause côté éditeur** (`status: queued`, position 1) — partira automatiquement à la reprise de la file ; entrée pas encore confirmée présente dans `tutorials.ts`, à vérifier avant de considérer cette ligne comme définitive. |
