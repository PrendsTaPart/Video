...
| 12 | StockVisionAI | Gérer son stock par WhatsApp avec l'agent IA (PredBot) | `gerer-son-stock-par-whatsapp` | **oui** — `create_supplier_order`. Rush fourni par Michael dans le chat (pas de dossier Drive) : conversation WhatsApp avec l'agent StockVisionAI ("PredBot") — liste stocks (ruptures CRITIQUE), liste recettes, vérification fournisseur, création de commande fournisseur (confirmée + preuve dans l'app réelle "Gestion des Livraisons"), génération du dashboard stock (39 stocks critiques, 193 835,36 €, 26 fournisseurs actifs). Voix Adam FR, 9 lignes VO (2 réutilisées telles quelles depuis `foodeatup-predibot-suggestions-tuto`), séquence Claude 3 temps. **⚠️ Collision de slug évitée** : le slug/titre catalogue "Parler à PrediBot avec nos prompts" (module `predibot`, ordre 03) était déjà pris sur `tutorials.ts` par une autre session, sur un rush RH différent (congés/pointages) — celle-ci placée à la place dans `stockvision-ai`, section "Statistiques & agent IA", order 21 (le module affichait déjà 20/20 rempli en direct sur le site, très en avance sur le suivi local `PROGRESSION-157-TUTORIELS.md` qui indiquait encore 7/20). Publiée le 2026-08-04 sur validation explicite de Michael ("publi sur lovable et Rapidocms") : Lovable mis à jour (commit `f82f83ab`) — **RapidoCMS non disponible dans cette session** (aucun connecteur `mcp__RapidoCMS__*` chargé, même situation que sur plusieurs vidéos précédentes) : `videoUrl`/`thumbnailUrl` pointent temporairement sur le raw GitHub de `claude/foodeatup-video-tutorial-3uthjr` (`videos/foodeatup-predibot-prompts-tuto/out/`), à remplacer par les URLs S3 RapidoCMS dès que le connecteur est disponible. LinkedIn non demandé pour cette vidéo. |
