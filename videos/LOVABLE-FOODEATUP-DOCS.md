...
| 5 | Réservations & Plan de salle | 05 · Placement en salle | `scanner-le-qr-code-de-la-table` | **oui** — `create_order` (établissement, table, items, couverts — génère facture + devis). Intrants fournis par Michael (carte intro « COMMANDER PAR QR CODE », carte outro CTA réutilisée, rush "scanner le qrcode de la table pour réaliser une commande" 76s). Rush recadré sur le fil du titre : Plan de salle → génération/copie du QR de la table QA3 → vue client (page Commander) → ajout au panier → validation → confirmation cuisine ; le tunnel "nouvel onglet Google" (artefact d'enregistrement) et le bloc paiement fractionné en fin de rush laissés hors montage (matière pour un futur tutoriel "Payer l'addition"). A remplacé la fiche stub préexistante sur Lovable (`videoUrl`/`thumbnailUrl`/`durationSeconds` vides, `order: 5`) — champs conservés, diff vérifié (`get_diff`) : seule cette fiche touchée. Script validé par Michael avant génération VO (règle STOP), montage `videos/foodeatup-qrcode-table-tuto/` (45,4s, peak -7,28 dBFS, QA visuelle par frame à l'offset de chacune des 8 lignes). Validée et publiée le 2026-08-07 (RapidoCMS `foodeatup-qrcode-table-tuto`/`-thumbnail` + LinkedIn FoodEatUp 2026-09-16 07h — rotation pleine jusque-là + Lovable, module `reservation-salle`) |
