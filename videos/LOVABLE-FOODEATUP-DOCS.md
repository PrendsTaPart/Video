...
| 31 | Hygiène & HACCP | Agent HACCP : relevez vos températures depuis WhatsApp | `agent-haccp-whatsapp` | **oui** — `add_temperature` (même prompt que `relever-une-temperature-equipement`, même outil/action, juste un autre canal de saisie). Nouvelle sous-catégorie ajoutée en fin de module `haccp` (`31 · Agent HACCP (WhatsApp)`, au-delà du catalogue 30/30 existant — sujet hors catalogue initial). Intrants fournis par Michael (carte intro « AGENT HACCP », outro CTA générique, rush WhatsApp 15,28 s montrant l'agent conversationnel Predibot/StockVisionAI). Montage `videos/foodeatup-agent-haccp-tuto/` (48,48 s, H.264/AAC, faststart, peak -7,09 dBFS), voix Adam FR (ElevenLabs, N8 réutilisé). Vidéo hébergée en raw GitHub (branche `claude/foodeatup-video-tutorial-i2rrhv`, comme `ouvrir-sa-vitrine-en-ligne`) plutôt que RapidoCMS — non demandé sur ce tutoriel. Publiée sur Lovable le 2026-08-04 (`src/data/tutorials.ts`, déployé en production sur `foodeatup-guide-star.lovable.app`). LinkedIn non demandé. |
