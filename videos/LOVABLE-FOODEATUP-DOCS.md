...
| — | Configuration *(hors 157, vidéo agent)* | 0 - L'agent Configurateur | `installer-son-restaurant-avec-le-configurateur` | **oui, 2 prompts** — bulk (`create_category`/`create_dish`/`create_zone`/`create_table`/`create_employee`/`create_employee_contract`/`create_tva`/`create_supplier`, correspond à la commande "installe mon restaurant" du storyboard) + `create_supplier`/`create_ingredient` (2e prompt, validé par un rush réel fourni par Michael montrant ces deux actions enchaînées via un bot conversationnel WhatsApp — `videos/foodeatup-configurateur/reference/cas-usage-predibot-whatsapp.mp4`, non intégré au montage, gardé comme référence). Vidéo déjà montée (HyperFrames, 5 frames, voix ElevenLabs Adam FR `TGAegA0zNRi8I6nUdq3i`, 45,4s) — pas un des 157 tutoriels granulaires, c'est la vidéo de présentation de l'agent, placée en tête (`order: 0`) du module Configuration Boutique. RapidoCMS non demandé (connecteur indisponible dans cette session) : `videoUrl`/`thumbnailUrl` sur raw GitHub (`claude/foodeatup-video-tutorial-zqtpn1`), vignette = `assets/thumbnail/agent-configurateur.jpg` fournie par Michael. Publiée sur Lovable le 2026-08-04 (commit `d112f134`), instruction explicite de Michael de publier directement. LinkedIn non demandé. |
