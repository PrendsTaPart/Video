...
| 33 | Agent IA Caroline | 05 · Salle & créneaux — Ouvrir ses créneaux de réservation | `ouvrir-ses-creneaux-de-reservation` | **oui** — `reservation_availability`. Voir `videos/foodeatup-creneaux-reservation-tuto/SCRIPT.md` (52,08 s, peak -7,2 dBFS, 5 zoom-punchs vérifiés frame par frame, dérive nulle). Remplace la fiche stub préexistante sur Lovable (module `caroline-ia`, `subcategory: "05 · Salle & créneaux"`, `order: 5`) — `slug`/`title`/`moduleSlug`/`subcategory`/`section`/`order` conservés. RapidoCMS : vidéo + vignette uploadées (`foodeatup-creneaux-reservation-tuto-v1.mp4` / `foodeatup-creneaux-reservation-thumbnail.jpg`). **Lovable : mise à jour bloquée le 2026-08-06 — `mcp__Lovable__send_message` a échoué avec "workspace is out of credits" (`https://lovable.dev/settings/billing`).** Le prompt complet à renvoyer dès que les crédits sont disponibles est conservé dans l'historique de session (branche `claude/foodeatup-video-tutorials-5bfibs`) ; renvoyer le message `send_message` avec le bloc `Tutorial` ci-dessous une fois débloqué, puis cocher cette ligne comme publiée. LinkedIn non demandé pour cette vidéo. |
