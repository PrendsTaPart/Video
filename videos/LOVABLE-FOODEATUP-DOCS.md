...
| 19 | Service Multi-Canal | 03 - Envoyer en Cuisine en direct | `envoyer-en-cuisine-en-direct` | **oui** — `update_order_status` (statut `confirmee`, correspond exactement au clic "Confirmer → envoyée en cuisine" montré dans le rush). 1er tutoriel publié du module `service-commande` (0/3 → 1/3). Rush fourni par Michael (`Envoyer_la_commande_en_cuisine.mp4`, 28,48 s) : liste Commandes → clic Confirmer (zoom-punch) → toast "envoyée en cuisine" → écran KDS (tickets, allergènes, minuteur) → retour Commandes → statut Prête. L'entrée `envoyer-en-cuisine-en-direct` existait déjà sur Lovable comme placeholder ("vidéo en cours de tournage", `videoUrl: ""`) — remplie in place (même slug/title/subcategory/order), pas de doublon créé. Script VO validé par Michael le 2026-08-06 avant génération audio (règle STOP). Publiée : RapidoCMS (vidéo + vignette), LinkedIn programmé 2026-08-26 07h (rotation FoodEatUp pleine jusqu'au 25/08), Lovable (commit `fdf0fe0`) |
