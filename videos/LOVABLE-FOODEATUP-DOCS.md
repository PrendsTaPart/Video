...
| 33 | HACCP | 20 - Archiver ses documents de nettoyage | `archiver-ses-documents-de-nettoyage` | non — aucun outil `mcp__Foodeatup__*` ne couvre l'ajout, l'aperçu ou le renommage d'un document dans un dossier HACCP (`list_employee_documents` est en lecture seule, côté RH, sans rapport). Rush fourni par l'utilisateur (58,36s, HACCP > Documents > Nettoyage et désinfection : Nouveau fichier → choisir + nommer → Enregistrer → aperçu via l'icône œil → renommer via le crayon). A remplacé une fiche stub préexistante sur Lovable (`section: "Documents & historiques"`, `order: 20`) — champs conservés. Voir `videos/foodeatup-documents-nettoyage-tuto/SCRIPT.md` (51,68 s, peak -7,3 dBFS, dérive nulle après extension auto de l'outro). Publiée le 2026-08-05 : RapidoCMS (`foodeatup-documents-nettoyage-tuto-v1` / `archiver-ses-documents-de-nettoyage-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `c8524b8`). Production et publication demandées en un seul message par l'utilisateur — STOP de validation levé comme pour `ouvrir-son-classeur-haccp`. LinkedIn non demandé. |
| 34 | Marketing, Fidélité & Iris | 18 - Résultats des sondages (historique) | `resultats-des-sondages-historique` | **oui** — `get_survey_results` (`establishment_id`, `survey_id` → moyennes, NPS, répartitions). Correspond exactement au rush fourni par l'utilisateur (30,8s : Fidélité & jeux > Sondages > clic Lien → formulaire public rempli côté client (étoiles, échelle NPS, case service) → Envoyer → retour FoodEatUp → clic Résultats → panneau Moyenne 4/5, Score NPS -100, Service — 1). Séquence Claude 3 temps ajoutée (module partagé), N6/N8 réutilisés tels quels (textes génériques identiques, `foodeatup-categories-tuto`/`foodeatup-dlc-tuto`). Dérive VO recalibrée avant livraison (segments C et H élargis, stages Claude redimensionnés sur les durées réelles de N6/N7). A remplacé une fiche stub préexistante sur Lovable (`section: "Jeux & sondages"`, `order: 18`) — champs conservés. Voir `videos/foodeatup-resultats-sondages-tuto/SCRIPT.md` (54,0 s, peak -7,2 dBFS). Publiée le 2026-08-05 : RapidoCMS (`foodeatup-resultats-sondages-tuto-v1` / `resultats-des-sondages-historique-thumbnail-v1`) + Lovable (`src/data/tutorials.ts`). Production et publication demandées en un seul message par l'utilisateur (même autorisation que #33). LinkedIn non demandé. |
| 35 | Marketing, Fidélité & Iris | 11 - Recharger ses Crédits (pack Com') | `recharger-ses-credits-pack-com` | non — aucun outil `mcp__Foodeatup__*` ne couvre l'achat ou le suivi de crédits SMS/Email/WhatsApp (facturation Stripe côté produit, pas un objet MCP). Rush court fourni par l'utilisateur (13,52s, sans clic filmé) : carte "Marketing & Commercial" (99€/mois, déjà Activé) — "1 500 crédits et 30 min audio/mois inclus" — puis scroll vers "Options & modules". A remplacé une fiche stub préexistante sur Lovable (`section: "WhatsApp, SMS & crédits"`, `order: 11`) — champs conservés. Voir `videos/foodeatup-recharger-credits-tuto/SCRIPT.md` (28,9 s, peak -7,3 dBFS, dérive quasi nulle dès le premier montage malgré l'absence de zoom-punch — pas de clic dans le rush). Publiée le 2026-08-05 : RapidoCMS (`foodeatup-recharger-credits-tuto-v1` / `recharger-ses-credits-pack-com-thumbnail-v1`) + Lovable (`src/data/tutorials.ts`, commit `4b7a1477`). Production et publication demandées en un seul message par l'utilisateur (même autorisation que #33/#34). **Correctif au passage** : #33 et #34 avaient une vignette uploadée sur RapidoCMS mais pas le MP4 lui-même (`videoUrl` pointait vers une clé S3 jamais créée) — corrigé en uploadant les deux vidéos manquantes sous les mêmes noms de fichier déjà référencés sur Lovable, aucune modification de fiche nécessaire. LinkedIn non demandé. |
