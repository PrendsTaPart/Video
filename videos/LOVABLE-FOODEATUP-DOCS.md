...
| 14 | Marketing, Fidélité & Iris | 14 - Lancer un jeu concours fidélité | `lancer-un-jeu-concours` | non — le MCP FoodEatUp n'expose que `list_wheel_games`/`get_wheel_stats` (lecture seule), aucun outil de création de roue cadeaux, pas de prompt inventé. **Premier tutoriel publié sur le module `marketing-fidelite`** (0/24 avant celui-ci) : a rempli en place le stub "en cours de tournage" déjà présent sur Lovable (même `slug`/`order: 14`/`subcategory`), pas de nouvelle entrée créée. Validée par Michael le 2026-08-05 (`videos/foodeatup-jeu-concours-tuto/`, 47,8 s, voix Adam FR ; zoom-punch mesurés par colour-thresholding, voir `SCRIPT.md`). Publiée : RapidoCMS (`foodeatup-jeu-concours-tuto-v1` / `foodeatup-jeu-concours-tuto-thumbnail`) + LinkedIn programmé 2026-09-11 16h (rotation FoodEatUp pleine jusque-là, une autre session publiait en parallèle sur ce même module pendant cette session — `retrouver-son-agenda-marketing` programmé 2026-09-11 07h) + Lovable (`src/data/tutorials.ts`, message `umsg_01kz8zp2cafkvr917n22r5cj99`). |
