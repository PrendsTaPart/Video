...
| 3 | Site Web & Vitrine | 03 - Personnaliser son site (éditeur web) | `personnaliser-son-site` | **oui** — `update_section` (fusion partielle des props d'une section : badge + titre modifiés, sous-titre/bouton inchangés dans le rush). Intrants fournis par Michael (carte intro `PERSONNALISER TON SITE`, carte outro CTA réutilisée octet-pour-octet, screen recording `assets/screen.mp4`). Remplace la fiche placeholder préexistante sur Lovable (même slug/order/subcategory, `chefTipAvatar` = photo chef Michael comme le reste de la sous-catégorie « Template & design »). Hébergée en raw GitHub (`videos/foodeatup-personnaliser-site-tuto/out/`, branche `claude/foodeatup-video-tutorial-4qnhnz`). Validée par l'utilisateur le 2026-08-04, publiée sur Lovable le 2026-08-04 (`src/data/tutorials.ts` mis à jour via `send_message`, tsgo OK — RapidoCMS/LinkedIn non demandés). |
