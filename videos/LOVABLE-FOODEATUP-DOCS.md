...
| 11b | PrediBot | Tour du tableau de bord PrediBot (stock, commandes, production) | `lire-ses-previsions-predibot` | non — `list_low_stocks`/`list_production_alerts`/`list_top_productions`/`get_daily_brief` passés en revue, aucun ne reproduit une prédiction IA en lecture seule (voir `videos/foodeatup-predibot-previsions-tuto/SCRIPT.md`). Complète le module `predibot` (3/3). Rush + cartes intro/outro fournis par Michael ; carte outro réutilisée à l'identique (md5 confirmé). Validée par Michael le 2026-08-06 (« c'est validé »), montée (42,48 s, zoom-punch sur les 3 cartes du dashboard, QA peak -7,18 dBFS/faststart/0 erreur decode) puis publiée directement sur la fiche placeholder déjà présente dans `tutorials.ts` (pas de doublon) — Lovable uniquement, hébergée en raw GitHub (`videos/foodeatup-predibot-previsions-tuto/out/`, branche `claude/foodeatup-video-tutorials-bivyyp`) ; RapidoCMS/LinkedIn non demandés pour cette vidéo. Site redéployé (`deploy_project`, commit `55ca217`). |
