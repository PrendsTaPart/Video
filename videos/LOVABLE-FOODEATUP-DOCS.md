...
| 05 | Site Web & Vitrine (1er tuto du module) | Créer un site par IA FoodEatUp | `creer-un-site-par-ia` | **oui** — prompt combiné `apply_site_template` + `publish_site` (aucun outil MCP ne couvre littéralement l'interview conversationnelle générative ; c'est l'équivalent exposé le plus proche, même bénéfice pour le restaurateur). Rush = début de l'interview IA (12 questions, 3 visibles à l'écran) fourni par Michael avec sa propre carte intro/outro ; pas d'avatar, pas de clic (chat UI) donc pas de zoom-punch. Script validé par Michael avant génération voix/montage. **Bug de mapping VO/étages Claude repéré et corrigé avant livraison** (voir `videos/foodeatup-site-ia-tuto/SCRIPT.md`) : la ligne bénéfice et les deux lignes Claude étaient mal alignées sur les 3 étages de la séquence, et une ligne (N6) manquait carrément du mix — recalé sur les 8 fichiers réels, drift vérifié à <10ms au raccord claude2→claude3. Fiche Lovable déjà présente en placeholder (`slug` réservé à l'avance, position 05 du module) : remplacée en place le 2026-08-04, `tsgo --noEmit` OK. RapidoCMS non demandé pour cette vidéo (Lovable + dépôt GitHub uniquement, cf. instruction reçue). Vidéo/vignette hébergées en raw GitHub (`claude/foodeatup-video-tutorial-mx3ez2`). |
