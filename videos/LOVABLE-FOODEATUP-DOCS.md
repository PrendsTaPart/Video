...
| 21 | Site Web & Vitrine (1er tuto du module) | 08 - Connecter son Domaine (mon site) | `connecter-son-domaine` | **oui** — `get_domain_status` (aucun outil MCP ne *connecte* un domaine ; celui-ci correspond à l'action "Vérifier maintenant" montrée en fin de rush : statut DNS/SSL). Rush + cartes intro/outro fournis par Michael (image d'intro initialement mal nommée, corrigée après clarification). Montage recalibré (N3/N5 sont des lignes VO longues pour un rush court de 31,6 s ; segments A/C/G redimensionnés sur la durée VO réelle — voir `videos/foodeatup-domaine-tuto/SCRIPT.md`). Validée par Michael et publiée le 2026-08-05 : RapidoCMS (`foodeatup-domaine-tuto-v1.mp4` / `-thumbnail.jpg`) + Lovable (a remplacé une fiche placeholder déjà existante dans `tutorials.ts`, `section: "Horaires, réservations & domaine"` conservée). Pas de créneau LinkedIn demandé pour cette vidéo |
| 22 | Marketing, Fidélité & Iris (1er tuto du module) | 01 - Débloquer les Avis clients | `debloquer-les-avis-clients` | non — achat/activation de l'option "Avis & réputation Google" (29€/mois) via paiement Stripe, aucun outil `mcp__Foodeatup__*` ne couvre cette action (même cas que `choisir-son-abonnement`) ; les outils `list_reviews`/`moderate_review`/`reply_review` gèrent les avis une fois l'option active, pas son achat. Rush + cartes intro/outro fournis par Michael, rush court (19,72 s). Montage sans séquence Claude (gabarit `foodeatup-abonnement-tuto`) ; N3/N5 recalibrés sur la durée VO réelle — voir `videos/foodeatup-avis-abonnement-tuto/SCRIPT.md`. Validée par Michael et publiée le 2026-08-05 : RapidoCMS (`foodeatup-avis-abonnement-tuto-v1.mp4` / `-thumbnail.jpg`) + Lovable (a remplacé une fiche placeholder déjà existante dans `tutorials.ts`). Premier tutoriel du module `marketing-fidelite` (24 vidéos attendues, le plus gros du catalogue). Pas de créneau LinkedIn demandé |
