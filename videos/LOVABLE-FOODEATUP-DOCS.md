...
| 30 | Équipe & Planning | 18 - suivre ses performances côté employé | `suivre-ses-performances-cote-employe` | non — aucun outil MCP ne calcule/n'expose ce score de performance employé (métrique interne, pas un endpoint API). Voir `videos/foodeatup-performances-tuto/SCRIPT.md` (36,84 s, dérive nulle). Publiée sur Lovable le 2026-08-04 (module `equipe-planning`). **Ré-uploadée sur RapidoCMS S3 le 2026-08-05** (`foodeatup-performances-tuto-v1`/`foodeatup-performances-tuto-thumbnail`) après le retour du service (panne 502 du 2026-08-04) — `videoUrl`/`thumbnailUrl` mis à jour sur Lovable (commit `f5d5b4b`), l'hébergement GitHub raw temporaire n'est plus utilisé |
| 31 | HACCP | Contrôle à réception : produits, DLC, température, conformité | `controler-reception-livraisons` | **oui** — `create_haccp_reception`. Voir `videos/foodeatup-reception-tuto/SCRIPT.md` (50,72 s, peak -7,16 dBFS, dérive nulle). Publiée sur Lovable le 2026-08-04 (module `haccp`). **Ré-uploadée sur RapidoCMS S3 le 2026-08-05** (`foodeatup-reception-tuto-v1`/`foodeatup-reception-tuto-thumbnail`) après le retour du service — `videoUrl`/`thumbnailUrl` mis à jour sur Lovable (commit `f5d5b4b`), l'hébergement GitHub raw temporaire n'est plus utilisé |
| 32 | HACCP | Poser une DLC sur ses productions | `poser-une-dlc-sur-ses-productions` | **oui** — `create_haccp_label`. Voir `videos/foodeatup-dlc-tuto/SCRIPT.md`. A remplacé une fiche stub préexistante sur Lovable (`section: "Production & plats"`, `order: 15`) — champs conservés. Publiée sur Lovable le 2026-08-04 (module `haccp`, commit Lovable `608cf98`). **Ré-uploadée sur RapidoCMS S3 le 2026-08-05** (`foodeatup-dlc-tuto-v1`/`foodeatup-dlc-tuto-thumbnail`) après le retour du service — `videoUrl`/`thumbnailUrl` mis à jour sur Lovable (commit `f5d5b4b`), l'hébergement GitHub raw temporaire n'est plus utilisé |
