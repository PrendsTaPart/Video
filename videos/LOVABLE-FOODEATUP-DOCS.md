...
| 14 | Site Web & Vitrine | 07 · Ajouter du contenu sur son site (Recrutement, Producteurs, Boissons, Privatisations, Agenda) | `ajouter-du-contenu-sur-son-site` | **oui, 2 prompts** — `create_job_offer` (animé dans la vidéo, séquence Claude 3 temps) + `upsert_beverage_item` (`claudePrompts[]`, second exemple texte seul). Intrants fournis par Michael : carte intro "AJOUTER DU CONTENU PRO", carte outro CTA générique, screen recording 132,84 s (`videos/foodeatup-contenu-site-tuto/`, remplit le stub "en cours de tournage" déjà présent côté Lovable, module `site-web-vitrine` 0/8→1+/8, une autre session publiait en parallèle `gerer-les-pages-de-son-site` sur le même module). 50,00 s, voix Adam FR (N6/N8 réutilisés depuis `foodeatup-vitrine-tuto`). Publiée le 2026-08-04 : GitHub (commit `8fb6667`, RapidoCMS indisponible cette session → URLs raw GitHub temporaires, à remplacer par S3 dès que le connecteur revient) + Lovable (`tsgo --noEmit` OK). **LinkedIn pas programmé** — pas demandé pour cette vidéo. |
