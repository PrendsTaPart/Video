...
| 2 | Site Web & Vitrine | Choisir son template (mon site) | `choisir-son-template` | **oui** — `apply_site_template(establishment_id, slug, confirm)` : le paramètre `confirm` reproduit très précisément la modale "Le site actuel sera remplacé (une version est sauvegardée)" montrée dans le rush. **1ère vidéo publiée du module `site-web-vitrine`** — remplit la fiche placeholder déjà en place (`order: 2`, `subcategory: "02 · Template & design"`), pas une nouvelle entrée créée. Rush fourni par Michael (bibliothèque de templates > filtre catégorie > Aperçu grandeur nature dans un nouvel onglet > Utiliser > confirmation). Publiée sur Lovable le 2026-08-04 sur instruction explicite de publier la vidéo une fois le montage terminé (`commit_sha` `26ebc159`). **Pas de RapidoCMS/LinkedIn** — connecteur RapidoCMS non disponible dans cette session, `videoUrl`/`thumbnailUrl` pointent en attendant vers le raw GitHub de la branche `claude/foodeatup-video-tutorial-8ddbi5`, même convention que `creer-produit-pour-etiquetage`. Dossier repo `videos/foodeatup-choisir-template-tuto/` — bug `banner()` (plaque `drawbox` invisible, piège documenté plus haut) rencontré et corrigé pendant ce montage, voir son `SCRIPT.md` |
