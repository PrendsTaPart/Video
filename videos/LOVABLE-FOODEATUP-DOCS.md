...
| 11 | Configuration | 11 - dessiner son plan de salle | `dessiner-son-plan-de-salle` | **oui** — `create_zone` + `create_table` (prompt combiné, dans l'ordre de construction du plan). Rush montre un plan déjà dessiné (8 tables, 2 zones) : sélection d'une table, génération de son QR code, copie du lien, puis la page client (« Gosushi démo ») — commander, demander l'addition, payer, seul ou à plusieurs. Pas d'UI de création de zone/table à l'écran (noté dans le `SCRIPT.md`, comme fournisseurs/produits). Validée par Michael et publiée le 2026-08-07 (RapidoCMS + LinkedIn programmé 2026-08-21 07h — calendrier plein jusqu'au 2026-08-20 — + Lovable) |
