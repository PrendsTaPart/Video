...
| 6a | Agent IA Caroline | Réécouter le résumé et la transcription complète d'un appel, filtrer par statut | `reecouter-ses-appels-et-reservations` | non — aucun outil `mcp__FoodEatUp__*` ne couvre "consulter l'historique des appels vocaux" (seuls des outils de réservation existent : `list_reservations`, `create_reservation`... action différente de celle filmée). Intro/outro/rush fournis directement par Michael dans la conversation (pas de Drive). Deuxième tutoriel du module `caroline-ia` (« 02 Réécouter ses Appels et réservations » du catalogue 157) : une fiche placeholder existait déjà côté Lovable (autre branche), remplacée par le contenu réel. Module à 2/6 après publication. Validée par Michael puis publiée le 2026-08-06 : RapidoCMS (`foodeatup-appels-reservations-tuto-v1` / `foodeatup-appels-reservations-tuto-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `3cc5d912`) + LinkedIn FoodEatUp programmé le 2026-09-21 07:00 (post `575`, prochain créneau libre, calendrier complet jusqu'au 2026-09-20). Facebook FoodEatUp : brouillon créé (`685`) mais programmation refusée, compte expiré — à reprogrammer une fois le compte renouvelé par Michael. |
