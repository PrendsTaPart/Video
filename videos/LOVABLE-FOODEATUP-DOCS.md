...
| 12 | HACCP | Contrôle à réception : produits, DLC, température, conformité | `controler-reception-livraisons` | **oui** — `create_haccp_reception` (les 4 actions rapides par produit — Photo DLC, DLC manuelle, Température, Scanner produit — n'ont pas d'outil MCP dédié). Publiée sur Lovable le 2026-08-04 à la demande explicite de Michael (MP4 + vignette uploadés sur RapidoCMS). **LinkedIn non demandé pour ce tuto** — pas de brouillon programmé. |
