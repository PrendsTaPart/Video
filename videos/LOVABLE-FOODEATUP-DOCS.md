...
| 33 | Agent IA Caroline | 05 · Salle & créneaux — Ouvrir ses créneaux de réservation | `ouvrir-ses-creneaux-de-reservation` | **oui** — `reservation_availability`. Voir `videos/foodeatup-creneaux-reservation-tuto/SCRIPT.md` (52,08 s, peak -7,2 dBFS, 5 zoom-punchs vérifiés frame par frame, dérive nulle). Remplace la fiche stub préexistante sur Lovable (module `caroline-ia`, `subcategory: "05 · Salle & créneaux"`, `order: 5`) — `slug`/`title`/`moduleSlug`/`subcategory`/`section`/`order` conservés. RapidoCMS : vidéo + vignette uploadées (`foodeatup-creneaux-reservation-tuto-v1.mp4` / `foodeatup-creneaux-reservation-thumbnail.jpg`). **Lovable : publiée le 2026-08-06** — premier essai refusé ("workspace is out of credits" sur le workspace `Contact.prendstapart`), Michael a rattaché le projet `foodeatup-guide-star` au workspace `Braindcode` (business, crédits dispo) ; renvoyé avec succès (`send_message`, commit Lovable `899c30d9`, typecheck OK, compteur du site passé de 139/152 à 140/152). LinkedIn non demandé pour cette vidéo. |
