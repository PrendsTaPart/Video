...
| — | Marketing, Fidélité & Iris *(nouveau module, première vidéo — 1/24)* | 05 · Pack marketing & campagnes | `lancer-une-campagne-marketing` | **oui** — `create_campaign` (brouillon email/sms/whatsapp/vocal, segment RFM, offre + code promo) puis `launch_campaign` (`confirm:true`), exactement le flux filmé (Continuer... → "Lancer vers N client(s)"). Storyboard reconstruit frame par frame depuis le screen recording fourni par Michael (`assets/screen.mp4`, 51,16 s) : sélection segment "Tous les clients" (39) → nom + canal Email + message → offre -15%/code RETOUR15 → "Envoyer maintenant" → conformité (39→38 contactables, 0,08€, 1 exclu STOP) → "Lancer vers 38 client(s)" → toast de confirmation. VO ElevenLabs (Adam FR, 11 lignes) validée par Michael avant génération. Montage `videos/foodeatup-campagne-marketing-tuto/build.py` : 3 zoom-punch (coordonnées mesurées par seuillage couleur PIL/numpy sur les frames réelles, pas à l'oeil), pas de `banner()` (la modale affiche déjà ses 4 étapes). 50,32 s, peak -7,3 dBFS, dérive résiduelle 2,64 s sur une seule ligne (N3, acceptée — la narration rattrape le canal Email déjà sélectionné à l'écran plutôt que de le devancer). Validée par Michael le 2026-08-05. Publiée : RapidoCMS (`foodeatup-campagne-marketing-tuto-v1` / `foodeatup-campagne-marketing-tuto-thumbnail`) + Lovable (a rempli la fiche stub préexistante du même slug, section "Pack marketing & campagnes", order 5 — pas de doublon créé). **LinkedIn non demandé pour l'instant.** |
| — | Marketing, Fidélité & Iris (2/24) | 08 · Pack marketing & campagnes | `creer-ses-templates-whatsapp` | **oui** — `create_whatsapp_template` (brouillon : nom Meta, catégorie, langue, corps avec variables `{{1}}..{{n}}`, libellés) puis `submit_whatsapp_template` (`confirm:true`, approbation Meta 24-48h). Storyboard reconstruit frame par frame depuis `assets/screen.mp4` (37,96 s) : onglet Templates WhatsApp (page vide, bandeau "Twilio non configuré : soumissions simulées") → "Nouveau template" → nom Meta "reconquete_clients" + catégorie Marketing + langue fr → corps "Bonjour {{1}}, on vous attend chez {{2}} ! Profitez de {{3}} avec le code {{4}}." (4 variables détectées) → libellés "prenom, restaurant, offre, code" → "Enregistrer" → toast "Template enregistré" → carte Brouillon (Modifier/Soumettre à Meta/Supprimer). VO ElevenLabs (Adam FR, 11 lignes). Montage `videos/foodeatup-templates-whatsapp-tuto/build.py` : 3 zoom-punch (onglet, Nouveau template, Enregistrer — mêmes coordonnées mesurées par seuillage couleur), pas de `banner()`. 56,48 s, peak -7,3 dBFS, dérive résiduelle acceptée sur 2 lignes (N3 +2,41s, N7 +2,85s — même nature que sur le tuto campagne, inhérente aux fenêtres fixes des zoom-punch face à des lignes VO plus longues, ne se réduit pas en agrandissant les segments en amont puisque ancrage et fin de ligne précédente dépendent des mêmes segments). Validée par Michael le 2026-08-05. Publiée : RapidoCMS (`foodeatup-templates-whatsapp-tuto-v1` / `foodeatup-templates-whatsapp-tuto-thumbnail`) + Lovable (a rempli la fiche stub préexistante du même slug — qui portait par erreur `section: "WhatsApp, SMS & crédits"`, corrigée en "Pack marketing & campagnes" à cette occasion). **LinkedIn non demandé pour l'instant.** |
