...
| — | Marketing, Fidélité & Iris | 24 - Calendrier de com' (agent Iris) | `calendrier-de-com-agent-iris` | **oui** — `propose_campaigns` (agent IA marketing : 2-4 propositions de campagnes chiffrées depuis RFM/jours creux/marges/marronniers, correspond exactement à l'agent Iris montré dans le rush). **Premier tutoriel publié du module `marketing-fidelite`** (0/24 avant cette vidéo). Rush : onglet Opportunités (Iris) → clic onglet Calendrier → clic « Générer les propositions de la semaine » (~22 s de « Fabrication en cours... » compressés au montage, aucune animation dans le rush pendant cette attente) → nouvelle semaine de propositions avec visuels générés automatiquement → clic menu « ⋯ » → « Valider & planifier » → toast « Proposition validée. ». Voir `videos/foodeatup-calendrier-iris-tuto/SCRIPT.md` (55,7 s, peak -7,22 dBFS, dérive nulle après correction — voir section « Bug de dérive » du SCRIPT.md pour le premier passage raté à +11,9 s). A remplacé une fiche stub préexistante sur Lovable (même slug, `order: 24`, `section: "Agent Iris"` — champs conservés). Demande explicite de produire et publier cette vidéo en une fois (mêmes conditions que `foodeatup-jarvis-tuto`). Publiée : RapidoCMS (`foodeatup-calendrier-iris-tuto-v1` / `foodeatup-calendrier-iris-tuto-thumbnail`) + Lovable (`src/data/tutorials.ts`, module `marketing-fidelite`). Poussée sur GitHub, branche `claude/foodeatup-video-tutorial-hc21s2`, commit `ac346d9`. LinkedIn non demandé pour cette vidéo. |
