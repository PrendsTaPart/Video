...
| 20 | Marketing, Fidélité & Iris | Vue publique fidélité — côté client | `vue-publique-fidelite-cote-client` | **oui** — `get_loyalty_account` (solde + prochaine récompense) + `list_loyalty_rewards` (catalogue de récompenses), combinés en un seul prompt (vue client en lecture seule, aucune action de création dans ce rush). Rush GoSushi Démo (19,28 s) : onglet Fidélité (solde, progression, récompenses, historique), onglet Mes commandes (état vide), onglet Mes infos → Mon compte (connexion passwordless). **Fiche Lovable préexistante** (placeholder du module déjà scaffoldé aux 24 emplacements du catalogue) : remplie avec vidéo/vignette RapidoCMS, 4 étapes, astuce du chef et `claudePrompt`, sans toucher aux autres entrées (`src/data/tutorials.ts`, commit `5679dd3a`). Validée et publiée le 2026-08-05 : RapidoCMS + Lovable. **LinkedIn non demandé pour cette vidéo.** Voir `videos/foodeatup-fidelite-client-tuto/SCRIPT.md` (bug de bascule d'onglet mal repéré + bug bandeaux `drawbox`/`t` corrigés au montage). |
