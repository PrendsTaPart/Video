...
- **Workspace Lovable** : `Braindcode` (id `7gbZqk6lsnITUluFMgSc`) — corrigé le
  2026-08-06 sur indication directe de Michael (« le workplace a changer c'est BraindCode
  sur lovable »), confirmé via `get_project`/`list_workspaces`. Le project_id et l'URL du
  projet ne changent pas — seul le workspace propriétaire diffère de ce qui était noté
  précédemment ici (`Contact.prendstapart`, id `NetRd8k1jtiPYJO1Jlaz`).
...
| 22 | Marketing, Fidélité & Iris | 22 · Agent Iris — Synchroniser la Charte graphique Iris | `synchroniser-la-charte-graphique-iris` | non — aucun outil `mcp__Foodeatup__*` n'expose le pairage marque/Iris (uniquement des outils métier : plats, employés, HACCP, réservations...), donc pas de prompt fabriqué. Fiche placeholder préexistante (« en cours de tournage ») remplacée par la version finale (même slug/order 22, pas de doublon) — commit `6ec4b214`, typecheck OK. Storyboard/script analysés depuis les 3 intrants fournis par Michael (carte intro « Synchro Design & Charte », carte outro CTA générique, rush RapidoCMS 49 s montrant le clic sur « Appairer la marque » → résultat couleurs/police/logo + comptes sociaux + 728 assets). Voix Adam FR ElevenLabs, 37 s. Validée par Michael le 2026-08-05 puis publiée le jour même : RapidoCMS (`foodeatup-charte-graphique-tuto-v1` / `-thumbnail`), **Facebook FoodEatUp programmé 2026-08-06 16h** (LinkedIn FoodEatUp `68807312` refusé par l'API — `account_id invalide` malgré la file d'attente existante qui le référence encore, à faire renouveler par Michael), Lovable. |
| 03 | Agent IA Caroline & Salle | 03 · Créer & modifier — Gérer ses no-shows et modifications | `gerer-ses-no-shows` | **oui** — `no_show_reservation(establishment_id, reservation_id)`. Ni la modification de détails de réservation ni la suppression définitive n'ont d'équivalent MCP (`cancel_reservation` change juste le statut vers « Annulée », différent de « Supprimer » qui efface la ligne) — un seul `claudePrompt`, centré sur le no-show. Fiche placeholder préexistante (« en cours de tournage », module `reservation-salle`, order 3) remplacée par la version finale — commit `6fcf62bc`, typecheck OK. Storyboard analysé depuis les 3 intrants fournis par Michael (carte intro « GÉRER & NO-SHOWS », carte outro CTA réutilisée telle quelle, rush 36,32 s montrant Réservations > menu à trois points > Modifier / No-show / Supprimer, avec bonus non anticipé : boîte de dialogue de confirmation de suppression). Voix Adam FR ElevenLabs, 44,12 s, peak -7,14 dBFS, dérive quasi nulle (`N2` à 0,34 s, négligeable). Validée par Michael le 2026-08-06 puis publiée le jour même : RapidoCMS (`foodeatup-noshow-tuto-v1` / `-thumbnail`), **Facebook FoodEatUp programmé 2026-08-07 07h**, Lovable (workspace `Braindcode`). |
