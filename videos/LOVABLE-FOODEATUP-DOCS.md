...
| — | PrediBot *(hors 157, doublon volontaire)* | 04 - Configuration rapide | `configurer-sa-boutique-avec-predibot` | même vidéo/claudePrompts que `installer-son-restaurant-avec-le-configurateur` ci-dessous, publiée en plus dans le module `predibot` (section "Dialoguer avec PrediBot", order 4 — slot bonus au-delà de l'`expectedCount: 3`, les 2 fiches "en cours de tournage" du module non touchées) sur demande explicite de Michael, vu que c'est littéralement le bot PrediBot qui est montré à l'écran. Commit Lovable `edt-8b36e194`. |
| — | Configuration *(hors 157, vidéo agent)* | 0 - L'agent Configurateur | `installer-son-restaurant-avec-le-configurateur` | **oui, 3 prompts** — bulk (correspond à la commande "installe mon restaurant" du storyboard HyperFrames) + `create_supplier`/`create_ingredient` + `create_product`/`create_recipe` (ces deux derniers correspondent chacun à une action réellement filmée dans le montage final). Vidéo remplacée le 2026-08-04 : d'abord la version HyperFrames (5 frames, 45,4s), puis un second montage `videos/foodeatup-configurateur-cas-usage-tuto/` (pipeline screen-recording standard, 39,75s) construit sur le rush WhatsApp/Predibot fourni par Michael — 5 actions enchaînées (ajouter un employé, un fournisseur, un ingrédient, un produit, une recette), chacune vérifiée en direct dans l'app, sous-titrées, voix ElevenLabs Adam FR `TGAegA0zNRi8I6nUdq3i` (9 lignes régénérées pour ce montage). Toujours hors des 157 tutoriels granulaires, en tête (`order: 0`) du module Configuration Boutique. RapidoCMS non demandé (connecteur indisponible dans cette session) : `videoUrl`/`thumbnailUrl` sur raw GitHub (`claude/foodeatup-video-tutorial-zqtpn1`, dossier `foodeatup-configurateur-cas-usage-tuto/out/`). Publiée sur Lovable (commit `d112f134` puis mise à jour `6c03f635`), instruction explicite de Michael de publier directement (pas de STOP de validation). LinkedIn non demandé. |
