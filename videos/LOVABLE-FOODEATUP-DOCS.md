...
| 11 | PrediBot | 3 - Parler à PrediBot avec nos prompts | `parler-a-predibot-avec-nos-prompts` | **oui** — `approve_leave`. Rush WhatsApp fourni par Michael (53,76 s, conversation avec l'agent Predibot) : liste des employés, liste des congés en attente, approuve un congé, rejette un congé (avec motif), pointages sur une période, classement des employés par performance — pas de clic à zoomer (ce n'est pas l'app FoodEatUp mais une capture WhatsApp), segments en lecture avec bandeau d'étape. Séquence "cas d'usage Claude" animée en fin de vidéo (module partagé). Montage + publication (Lovable + dépôt GitHub) faits en une seule passe sur instruction explicite de Michael, sans le STOP intermédiaire habituel. **Remplit le 3e et dernier slot du module `predibot` (désormais complet, 3/3)**. Pas de RapidoCMS/LinkedIn demandé pour cette vidéo. `videoUrl`/`thumbnailUrl` pointent vers le raw GitHub de la branche `claude/foodeatup-video-tutorial-o6l2ea` (dossier repo `videos/foodeatup-parler-predibot-tuto/`) |
