...
| 1 | Flux de Service & KDS | 01 · Postes de travail — Créer un poste de travail (module KDS) | `creer-un-poste-de-travail` | non — aucun outil MCP FoodEatUp ne crée de poste KDS (seuls `get_station_load` en lecture et `update_kds_item_status` existent pour ce module) : pas de `claudePrompt`, section "Utiliser avec Claude" masquée sur la fiche. **Première vidéo du module `kds-cuisine`** (0/3 avant celle-ci, catalogue 157 — voir `videos/CATALOGUE-157-TUTORIELS.md`). Rush unique (26,88 s) fourni par Michael : page Cuisine (kds) avec un poste existant → + Nouveau poste → nom "Froid" → bascule du type Préparation/Pass (envois) → couleur → Créer le poste → toast + liste à 3 postes. Voix ElevenLabs Adam Instructor (N0-N6 frais, N7 CTA réutilisé octet-identique). A remplacé une fiche stub préexistante sur Lovable (`subcategory: "01 · Postes de travail"`, `order: 1`, "en cours de tournage") — champs conservés. Publiée sur instruction explicite de publier une fois le montage terminé : RapidoCMS (`foodeatup-kds-poste-tuto-v1` / `foodeatup-kds-poste-tuto-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `3dbfb9f`) — LinkedIn non demandé pour cette vidéo. Voir `videos/foodeatup-kds-poste-tuto/SCRIPT.md` pour le détail complet (découpage, coordonnées zoom-punch, cas d'usage/astuce du chef). **Mise à jour 2026-08-06** (sur demande explicite) : `howItWorks`/`whatItsFor`/`chefTip` enrichis avec une vraie définition du KDS (Kitchen Display System), l'aspect temps réel (affichage instantané dès confirmation de commande) et des exemples concrets de postes pour le dispatch des bons de commande (Entrée, Chaud, Grillade, Bar) — commit Lovable `e37791e`, déployé en production et vérifié en direct sur `tutoriel.foodeatup.com/tutoriel/creer-un-poste-de-travail`. |
| 2 | Agent IA Caroline & Salle | 02 · Créer & modifier — Ajouter une réservation (module Table) | `ajouter-une-reservation` | **oui** — `create_reservation` (establishment_id, customer_name, party_size, date, time ; table_id/zone optionnels comme dans le produit). **Première vidéo du module `reservation-salle`** (0/5 avant celle-ci). Rush unique (65,86 s) fourni par Michael : page Réservations (plan de salle + file d'attente → liste) → + Nouvelle réservation → nom/téléphone/email → date/heure/couverts → sélection de table (T4, filtre Salle principale) → Créer la réservation → statut "En attente" → menu d'actions (Confirmer/Check-in/Modifier/No-show/Annuler/Supprimer). Voix ElevenLabs Adam Instructor (N0-N8 frais, N9 CTA réutilisé octet-identique). Séquence Claude incluse (3 étages). A remplacé une fiche stub préexistante sur Lovable (`subcategory: "02 · Créer & modifier"`, `order: 2`, "en cours de tournage") — champs conservés. Publiée sur instruction explicite de publier une fois le montage terminé : RapidoCMS (`foodeatup-reservation-tuto-v1` / `foodeatup-reservation-tuto-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `dcff3d5`) — LinkedIn non demandé pour cette vidéo. Déployé en production et vérifié en direct sur `tutoriel.foodeatup.com/tutoriel/ajouter-une-reservation`. Voir `videos/foodeatup-reservation-tuto/SCRIPT.md` pour le détail complet. |
