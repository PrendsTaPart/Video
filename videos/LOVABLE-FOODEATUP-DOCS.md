...
| 9 | Site Web & Vitrine | 09 · Commander sur son site, par vocal ou QR code | `commander-via-site-vocal-qrcode` | **oui, 3 prompts** — `list_orders` filtré par `channel` (`vitrine`/`agent_vocal`/`sur_place`), un exact match avec les 3 canaux du tutoriel. Rush fourni par Michael filme uniquement le parcours "site" (menu → panier → coordonnées → paiement Stripe) ; l'agent vocal et le QR code à table ne sont pas des écrans capturables, introduits en VO + une carte de transition qui réutilise `assets/intro.jpg` (elle montre déjà les 3 canaux ensemble). Validée par Michael (demande explicite de réaliser puis publier la vidéo complète dans la foulée — STOP de livraison levé pour cette vidéo, cf. `SCRIPT.md` du dossier). Publiée le 2026-08-06 : RapidoCMS (vidéo + vignette) + Lovable (`order: 9`, `expectedCount` du module passé de 8 à 9). 9ème tutoriel du module `site-web-vitrine` — les 8 précédents couvrent la configuration du site, celui-ci couvre l'expérience de commande du client final, sujet distinct. LinkedIn non demandé pour cette vidéo. |
| 4 | Réservations & Plan de salle | 04 · Placement en salle | `placer-un-client-a-table` | **oui** — `checkin_reservation` (correspondance exacte : "installe le client (table occupée) et crée la commande sur place", exactement l'action montrée à l'écran, `table_id` optionnel si non assignée). Rush fourni par Michael (22,96 s) : réservation "En attente" → menu d'actions → Check-in → toast "Client installé, commande CMD-... créée" → statut Installée → Modifier pour réassigner la zone/table. **Complète une fiche placeholder déjà présente sur le site** ("en cours de tournage", `order: 4`) plutôt que de créer une nouvelle entrée — slug/moduleSlug/subcategory/order inchangés. Script validé par Michael le 2026-08-06. Publiée le 2026-08-06 : RapidoCMS (vidéo + vignette) + Lovable. LinkedIn non demandé pour cette vidéo. |
