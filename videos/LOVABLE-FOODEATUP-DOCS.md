...
| 21 | Équipe & Planning | 15 - Pointer son service (pauses & photo), côté employé | `pointer-son-service-cote-employe` | **oui** — `list_attendances` (résumé hebdo heures/pauses de l'employé). Comblait le dernier placeholder du module (20/20). **v1 (carte-based) puis v2 (screen recording réel) le même jour** : le premier rush envoyé était le fichier Drive mal étiqueté déjà documenté sur `creer-ses-roles-et-permissions` (Accueil/QR/Rôles, pas le pointage) — v1 montée en carte-based en attendant. Michael a ensuite renvoyé le bon fichier (23,88 s) : montre le vrai flux Entrée → Pause → Fin de pause → Sortie, chaque étape confirmée par une photo. v2 remonté en zoom-punch classique (coordonnées de clic mesurées frame par frame) sur ce rush, remplace v1. Voir `videos/foodeatup-pointer-service-tuto/SCRIPT.md` pour le détail. Publié le 2026-08-07 : RapidoCMS (vidéo v2 + vignette) + Lovable + GitHub (`claude/foodeatup-video-tutorials-x5a46z`). `whatItsFor` enrichi sur demande explicite avec l'apport IA/planning (masse salariale AnalytiX BI, écarts planning/pointage, ajustement des prochains plannings). LinkedIn programmé le 2026-08-21 07:00 (post id 579, job_id 678 — rotation FoodEatUp pleine jusqu'au 2026-08-20 16h), caption format standard (hook + comment ça marche + astuce du chef + prompt Claude). |
