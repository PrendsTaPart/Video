...
| 12 | Service Multi-Canal | Commander sur Site & QR — « Commander par QR Code » | `commander-qr-code` | **oui** — `create_order` (`service_mode="sur_place"`, `table_id`). Rush fourni par Michael (`COMMANDER_PAR_QR_CODE.jpg` + `scanner_le_qrcode__de_la_table_pour_realiser_une_commande.mp4`, 76 s) : plan de salle → révélation QR de la table → scan client → carte/panier → envoi cuisine → paiement carte (Stripe). Sans avatar (voix Adam FR, N6/N8 réutilisées depuis `foodeatup-qrcode-tuto/vo/`). Voir `videos/foodeatup-commander-qrcode-tuto/SCRIPT.md` (49,24 s, peak -7,15 dBFS, dérive max 1,2 s). Premier tutoriel du module `service-commande` (0/3 → 1/3). Publiée : RapidoCMS (`foodeatup-commander-qrcode-tuto-v1` / `foodeatup-commander-qrcode-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `f1a3c242`, fin du module `service-commande`). **LinkedIn/Facebook non demandés pour cette vidéo** — à programmer sur demande. |
