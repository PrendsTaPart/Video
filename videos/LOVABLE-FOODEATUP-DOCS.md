...
| — | PrediBot (Agent IA Directeur) | 02 - Piocher dans la marketplace de prompts | `piocher-dans-la-marketplace-de-prompts` | **oui** — `create_supplier`, prompt identique à celui affiché dans la vidéo (carte marketplace « Crée un nouveau fournisseur »). Pas de séquence Claude synthétique en fin de vidéo : le rush montre déjà le round-trip marketplace → Claude → FoodEatUp en entier (voir `videos/foodeatup-predibot-marketplace-tuto/SCRIPT.md`). Montage 39,68 s, peak -7,15 dBFS, voix Adam FR. Complète le module `predibot` (3/3). Validée par Michael le 2026-08-07. Publiée : RapidoCMS (`foodeatup-predibot-marketplace-tuto-v1` / `-thumbnail`) + Lovable (`src/data/tutorials.ts`, commit `ebc815c`, a remplacé un placeholder préexistant sur le même slug — pas de doublon, types validés par l'agent Lovable) + LinkedIn FoodEatUp (page `68807312`, draft #687, programmé 2026-08-21 07h00 — prochain créneau libre de la rotation, pleine jusqu'au 2026-08-20). Pas de Facebook (non demandé). |
