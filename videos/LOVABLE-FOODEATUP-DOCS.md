...
| 7 | Agent IA Caroline & Salle | Gérer ses Tables (ajout & blocage) | `gerer-ses-tables-ajout-blocage` | **oui, 2 prompts** — `create_table` (ajout, animé dans la vidéo) + `update_table_status` en `blocked` (blocage, `claudePrompts[]` site uniquement). Rush fourni par Michael (screen recording réel Plan de salle : Ajouter une table → forme/couverts/zone → Enregistrer, puis cycle de statuts Libre → Réservée → Nettoyage → Bloquée → Libre), + carte intro/outro fournies. **Fiche Lovable préexistante** (créée par une session concurrente, contenu déjà très proche : mêmes `howItWorks`/`chefTip`/2 `claudePrompts`, mais vidéo pas encore déployée en prod — le module `caroline-ia` affichait encore « Coming soon » au moment de la publication) : le MP4 et la vignette rendus dans cette session ont été **réuploadés sous les mêmes noms de fichiers RapidoCMS** (`foodeatup-gerer-tables-tuto-v1` / `-thumbnail`, mêmes URLs S3 déjà référencées par la fiche — pas de nouvelle entrée créée, pas de doublon de slug), puis `durationSeconds` corrigé à 65 (commit `3ee9b391`). Site redéployé (`deploy_project`) pour pousser la fiche en production. Publiée le 2026-08-07 (RapidoCMS + Lovable + LinkedIn FoodEatUp programmé 10h20 CEST). Voir `videos/foodeatup-tables-tuto/SCRIPT.md`. |
