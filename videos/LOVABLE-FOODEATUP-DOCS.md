...
| 21 | Marketing, Fidélité & Iris | 06 · Pack marketing & campagnes | `creer-une-campagne-par-ia` | **oui, 2 prompts** — `propose_campaigns` (l'action montrée à l'écran : clic "Proposer des campagnes", l'IA lit segments RFM/jours creux/marges et rend 2-4 propositions chiffrées) + `create_campaign` (créer directement une campagne, second exemple `claudePrompts[]`). Première vidéo publiée du module `marketing-fidelite` (23 fiches placeholder restantes) — remplit le placeholder existant `creer-une-campagne-par-ia` (order 6) plutôt que d'ajouter une entrée séparée : un premier envoi Lovable avait créé un doublon (`creer-campagne-agent-ia`) faute d'avoir vérifié le scaffold du module au préalable, corrigé dans la foulée par un second envoi (suppression du doublon + remplissage du placeholder). Montage + VO ElevenLabs (Adam FR) + séquence Claude, publié le 2026-08-05 sur demande explicite de Michael (workflow complet en un message) — RapidoCMS/LinkedIn non demandés, URLs GitHub raw (branche `claude/foodeatup-video-tutorials-ky005u`) |
| 22 | Marketing, Fidélité & Iris | 12 · Fidélité & récompenses | `booster-la-fidelite-programme` | **oui** — `update_loyalty_program` (establishment_id, earn_mode, visit_points, points_validity_months, active) : couvre le mode de gain (par passage, 10 pts/passage montré à l'écran) et la validité des points (12 mois glissants). Les multiplicateurs jours creux et le plafond de points/commande sont des réglages UI sans équivalent MCP (schéma vérifié), non mentionnés dans le prompt. Placeholder existant `booster-la-fidelite-programme` (order 12) rempli directement en un seul envoi cette fois (leçon retenue du doublon précédent : vérifier le scaffold du module avant d'ajouter une entrée). Montage + VO ElevenLabs (Adam FR) + séquence Claude, publié le 2026-08-05 sur demande explicite de Michael (workflow complet en un message) — RapidoCMS/LinkedIn non demandés, URLs GitHub raw (branche `claude/foodeatup-video-tutorials-ky005u`). Dérive de narration corrigée avant livraison (segments redimensionnés sur les offsets réels de la VO, voir `SCRIPT.md` du tuto) |
| 23 | Marketing, Fidélité & Iris | 04 · Pack marketing & campagnes | `activer-le-pack-marketing` | non — achat/activation d'un pack ou module d'abonnement passe par Stripe, aucun outil `mcp__Foodeatup__*` ne le couvre (recherché explicitement, aucun résultat). Placeholder existant (order 4) rempli directement, sans doublon. Rush le plus court de la série (15,88 s) : montage à 2 segments fortement ralentis (contenu source statique), pas de zoom-punch (le clic réel dure moins de 0,6 s dans le rush, non isolable proprement — coupure franche `slideleft` à la place). Montage + VO ElevenLabs (Adam FR), publié le 2026-08-05 sur demande explicite de Michael (workflow complet en un message) — RapidoCMS/LinkedIn non demandés, URLs GitHub raw (branche `claude/foodeatup-video-tutorials-ky005u`) |
