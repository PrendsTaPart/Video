...
| 21 | Équipe & Planning | 15 - Pointer son service (pauses & photo), côté employé | `pointer-son-service-cote-employe` | **oui** — `list_attendances` (résumé hebdo heures/pauses de l'employé). Comblait le dernier placeholder du module (20/20). **Rush inexploitable** : fichier Drive mal étiqueté (même bug déjà documenté sur `creer-ses-roles-et-permissions` — montre Accueil/QR/Rôles, pas le pointage employé). Montage carte-based construit sur la vraie capture produit `pointage.png` (crops entrée/pause/sortie) + un graphique illustratif pour le volet photo anti-fraude (aucun écran fabriqué). Voir `videos/foodeatup-pointer-service-tuto/SCRIPT.md`. Publié le 2026-08-07 : RapidoCMS (vidéo + vignette) + Lovable + GitHub (`claude/foodeatup-video-tutorials-x5a46z`). LinkedIn non demandé. **À refaire en zoom-punch classique si un vrai screen recording du flux employé arrive un jour.** |
