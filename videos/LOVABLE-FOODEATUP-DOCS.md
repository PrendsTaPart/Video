...
| — | **Site Web & Vitrine** *(nouveau module, première vidéo)* | 04 - Gérer les Pages de son site | `gerer-les-pages-de-son-site` | **oui** — `toggle_site_page` (dépublier/republier une page). Rush fourni par Michael (carte intro « Gérer tes pages », carte outro réutilisable, screen recording 1920x828/25fps/17,96s) : Éditeur visuel → onglet Pages (5 pages, toutes publiées) → Dépublier Accueil (statut Brouillon) → Publier (retour Publiée) → Voir (page live, défilement hero/carte/horaires/footer). Script VO à 9 lignes (voix Adam FR), N6/N8 réutilisées telles quelles depuis `foodeatup-tva-tuto/vo/`. Bandeaux d'étape rendus avec `drawtext box=1` (pas `drawbox`, cf. pitfalls `FOODEATUP-TUTORIELS-WORKFLOW.md`). Hébergée en raw GitHub (`videos/foodeatup-pages-tuto/out/`, branche `claude/foodeatup-video-tutorial-ncql3u`). Ajoutée à `tutorials.ts` le 2026-08-04 (module `site-web-vitrine`, 1/8) — **publication RapidoCMS/LinkedIn non demandée pour cette vidéo, en attente si souhaitée.** |
