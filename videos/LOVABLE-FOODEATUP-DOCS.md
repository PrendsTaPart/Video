...
| 23 | Marketing, Fidélité & Iris | Booster les stocks dormants (opportunités IA) | `booster-les-stocks-dormants` | **oui** — `propose_campaigns`. Rush fourni par l'utilisateur (carte intro/outro + screen recording agent Iris, onglet Opportunités : clic « Redétecter maintenant » → surstocks détectés, triés par score urgence × valeur × fraîcheur). `videos/foodeatup-stocks-dormants-tuto/` — 47,92 s, voix Adam FR, montage réussi au premier passage (zéro dérive VO/image). Validée par l'utilisateur le 2026-08-05. Publiée : RapidoCMS (`foodeatup-stocks-dormants-tuto-v1` / `-thumbnail`) + Lovable (`src/data/tutorials.ts`, module `marketing-fidelite`, commit `53a89c8`) + Facebook FoodEatUp (`201499969703551`, programmé 2026-08-06 07h00). **LinkedIn FoodEatUp indisponible** : l'ancien compte `68807312` n'existe plus côté RapidoCMS (`account_id invalide`) et le remplaçant apparent « FoodEatUp TN » (`106239843`) a un token révoqué (`schedule_draft_tool` a échoué avec « The token used in the request has been revoked by the user ») — à signaler pour reconnexion avant la prochaine vidéo, sinon toute la série reste bloquée sur LinkedIn tant que ce n'est pas résolu. |
