...
- **Workspace Lovable** : `Contact.prendstapart` (id `NetRd8k1jtiPYJO1Jlaz`) — c'est le
  ...
  **Correction du 2026-08-06 (Michael)** : le workspace correct est **`Braindcode`**
  (id `7gbZqk6lsnITUluFMgSc`, plan business, rôle admin) — le projet FoodEatUp Academy y
  est bien visible/accessible. Ça ne change rien en pratique : toutes les opérations
  Lovable de ce dépôt adressent le projet par **`project_id`**, pas par workspace, donc
  aucune migration nécessaire. Garder ce `project_id` comme seule clé stable ; ne plus
  citer "Contact.prendstapart" comme le workspace de référence dans une future doc.
...
| 19 | Marketing, Fidélité & Iris | 19 - Fidélité multi-canal (canaux de vente) | `fidelite-multi-canal` | **oui** — `update_loyalty_program`. Rush : configuration du programme (mode de gain, multiplicateur jours creux, règles) puis clic "Voir la page publique" (reveal multicanal) → dashboard fidélité client (GoSushi Démo, connexion par code email). Une fiche placeholder existait déjà sur Lovable (`slug: "fidelite-multi-canal"`, "tutoriel en préparation") — **mise à jour en place**, pas de doublon créé : c'est le premier tutoriel du module `marketing-fidelite` réellement rempli depuis ce dépôt. Script validé par Michael avant génération VO (2 passes de retiming nécessaires : décalage voix/image cumulatif classique de la série, puis bug `drawbox`/`t` sur `banner()` — corrigé en reprenant le pattern à deux `drawtext` de `foodeatup-mouvement-stock-tuto`). Vidéo validée puis publiée le 2026-08-05 (RapidoCMS + Lovable). LinkedIn non demandé pour ce tuto. |
| 02 | Écran Cuisine (KDS) | 02 - Afficher le KDS par poste | `afficher-le-kds-par-poste` | **oui** — `get_station_load` (lecture seule, charge par poste). Rush court (21,3s) : liste des postes (chaud/pass/froid) → clic "Ouvrir l'écran" (poste chaud) → écran KDS Chaud (tickets, Bump) → bascule Pass (tickets transférés, Fire Service) → bascule Froid (file vide). Fiche placeholder déjà présente (`slug: "afficher-le-kds-par-poste"`) — **mise à jour en place**. Même bug de drift voix/image que `fidelite-multi-canal` réapparu au 1er rendu (segments trop courts vs VO mesurées), corrigé par calcul direct des cibles à partir de la chaîne d'offsets séquentielle plutôt qu'en devinant — vérifié image par image, zéro dérive au 2e rendu. **Fiche enrichie sur demande explicite de Michael** : `howItWorks` détaille le mécanisme bump (poste prépa → Pass) / Fire Service (Pass → salle) / synchronisation multi-postes, `chefTip` explique qu'un ticket reste bloqué au Pass tant qu'un poste n'a pas bumpé son plat (anti-commande-incomplète). Publiée le 2026-08-06 (RapidoCMS + Lovable). LinkedIn non demandé. |
| 01 | Réservations & Plan de salle | 01 - Retrouver ses réservations du jour | `retrouver-ses-reservations-du-jour` | **oui** — `list_reservations` (lecture seule, filtre date). Rush (24,7s) : simple scroll continu, sans clic, sur le tableau de bord Réservations (plan de salle temps réel, file d'attente, liste des réservations du jour avec table assignée). Premier tutoriel publié du module `reservation-salle` (fiche placeholder déjà présente, mise à jour en place). Cibles de segment dérivées d'emblée de la chaîne d'offsets séquentielle des VO mesurées (leçon des deux tutos précédents appliquée dès le premier rendu : sync quasi parfaite du premier coup). Un bug d'apostrophe dans deux bandeaux ("File d'attente", "Vue d'ensemble") repéré et corrigé avant livraison. Publiée le 2026-08-06 (RapidoCMS + Lovable). LinkedIn non demandé. |
