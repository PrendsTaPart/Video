#!/usr/bin/env python3
# FoodEatUp "Saisir ses ingrédients" tutorial.
...
#
# Rush shows ONE ingredient ("Huile d'olive") filled through a long, dense
# form (nom/fournisseur/unité/prix -> coefficient/type/particularités/plats
# -> stock/seuil (scrolled back up) -> allergènes -> valeurs nutritionnelles
# -> composition -> Enregistrer). VO line order follows that same visual
# order; audio FILE numbers (N0..N10) were assigned in the order they were
# generated, not necessarily the playback order -- `keys` below lists them
# in actual chronological/narrative order, which is what the sequential
# GAP-based placement algorithm needs to avoid overlap.
...
ROOT = "/home/user/Video/videos/foodeatup-ingredients-tuto"
...
# work/frames/), not eyeballed.
BTN_ADD  = (1621, 344); SZ_ADD  = (334, 57)  # "Ajouter un ingrédient"
BTN_SAVE = (1674, 672); SZ_SAVE = (157, 61)  # "Enregistrer"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.00,  2.90,  None, None,     None,     "1 · Vos ingrédients"),
    ("B", 3.00,  3.25,  0.90,  3.10, BTN_ADD,  SZ_ADD,   None),
    ("C", 4.00,  24.00, 6.60,  None, None,     None,     "2 · Nom, fournisseur, unité, prix"),
    ("D", 24.00, 45.00, 6.60,  None, None,     None,     "3 · Type, particularités, plats associés"),
    ("E", 45.00, 54.00, 5.00,  None, None,     None,     "4 · Stock et seuil minimum"),
    ("F", 54.00, 96.00, 9.50,  None, None,     None,     "5 · Allergènes et valeurs nutritionnelles"),
    ("G", 96.00, 96.35, 0.90,  96.20, BTN_SAVE, SZ_SAVE, None),
    ("H", 96.50, 103.90, 5.50, None, None,     None,     "Ingrédient ajouté"),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__create_ingredient
# (establishment_id, name, unit, price_per_unit, quantity_in_stock, +optional
# calories/proteins/carbs/fats/cooking_coefficient/alert_threshold) is the
# matching tool -- fields checked against the schema before writing the
# prompt below, they line up with what the rush shows on screen.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée l'ingrédient [nom] en [unité], prix [prix]€, "
                  "quantité en stock [quantité], pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cet ingrédient pour votre établissement…"
...
CLAUDE_STAGE_D = [7.00, 3.00, 6.00]  # reveal, copied, chatbot mockup
# Sized generously (vs. shared default [2.20,1.30,2.50]): N7's benefit
# example (pâte à tartiner / Nutella) is long and unrelated to any specific
# click, so its tail is allowed to drift into stage1's reveal (silent while
# the card animates in) rather than forcing a fixed segment to play back at
# below-real-time speed to fully absorb it. Verified by simulation: N8
# lands within stage1->stage2, N9 within stage2->stage3 -- both narrate the
# stage that's actually on screen, which is what matters (see
# FOODEATUP-TUTORIELS-WORKFLOW.md for why zero-drift everywhere isn't
# always achievable/desirable).
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # C -> D (cut, contact fields -> classification)
             "slideleft",  # D -> E (cut, classification -> stock/seuil, scrolled up)
             "slideleft",  # E -> F (cut, stock -> allergènes/nutrition)
             "fade",       # F -> G (continuous: click Enregistrer)
             "fade",       # G -> H (submit -> success/list)
             "slideleft",  # H -> claude1
...
GAP = 0.22
# Anchors mapped by MEANING, not by file-number order (files were generated
# in a different order than they play). `keys` below lists them in the
# actual chronological playback order the GAP algorithm must walk.
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Ajouter un ingrédient"
    "N2": S["C"] + 0.20,         # nom / fournisseur / unité / prix
    "N5": S["D"] + 0.20,         # type / particularités / plats associés
    "N3": S["E"] + 0.20,         # stock / seuil d'alerte
    "N4": S["F"] + 0.20,         # allergènes / valeurs nutritionnelles
    "N6": S["G"] + 0.10,         # "cliquez sur Enregistrer"
    "N7": S["H"] + 0.20,         # exemple pâte à tartiner / Nutella
...
keys = ["N0", "N1", "N2", "N5", "N3", "N4", "N6", "N7", "N8", "N9", "N10"]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-ingredients-tuto-v1.mp4"
