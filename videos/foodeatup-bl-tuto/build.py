#!/usr/bin/env python3
# FoodEatUp "Valider son BL en détail" tutorial.
...
#
# Rush (28.08s) shows: liste "Gestion des livraisons" filtrée Expédiée -> clic
# Filtrer -> sélection du statut Livrée -> liste filtrée avec "Voir le détail"
# -> clic -> détail de la commande (fournisseur/mode livraison/dates/prix) ->
# scroll vers le tableau "Produits livrés" (quantité commandée vs reçue +
# température) et la section Facture. Correspond exactement à l'outil MCP
# create_haccp_reception (contrôle de réception HACCP, arrivée fournisseur).
...
ROOT = "/home/user/Video/videos/foodeatup-bl-tuto"
...
# Coordinates eyeballed on extracted frames (1920x828 native, no scaling) --
# no colour-thresholding script run this time, kept generous (zoom 1.20x)
# to tolerate a bit of imprecision. No apostrophe in any caption (bug hit on
# foodeatup-ingredients-tuto -- see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_FILTER = (1493, 343);  SZ_FILTER = (130, 50)   # "Filtrer" button
BTN_LIVREE = (1580, 371);  SZ_LIVREE = (300, 42)   # "Livrée" option in dropdown
BTN_DETAIL = (372, 570);   SZ_DETAIL = (280, 54)   # "Voir le détail" (1st card, scrolled)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",  0.50,  9.50,  4.20, None, None,       None,       "1 - Filtrez vos livraisons"),
    ("B",  9.50, 11.90,  0.90, 9.60, BTN_FILTER,  SZ_FILTER,  None),
    ("C", 11.90, 13.40,  1.30, 12.00, BTN_LIVREE, SZ_LIVREE,  None),
    ("D", 18.50, 19.90,  4.60, None, None,       None,       "2 - Ouvrez le detail"),
    ("E", 19.90, 20.40,  0.90, 20.00, BTN_DETAIL, SZ_DETAIL,  None),
    ("F", 21.50, 23.30,  5.00, None, None,       None,       "3 - Fournisseur et dates"),
    ("G1",23.30, 24.10,  5.80, None, None,       None,       "4 - Quantite et temperature"),
    ("G2",24.10, 24.80,  4.80, None, None,       None,       "5 - Ajoutez la facture"),
]
...
# tool: create_haccp_reception(establishment_id, date_controle, heure_controle,
# etat_livraison, fournisseur_id?, fournisseur_nom?, reference_bl?,
# temperature_produits_frais?, commentaires?, validate?) -- schema checked.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée un contrôle de réception pour le fournisseur [nom du fournisseur], "
                  "référence BL [référence BL], livraison [conforme/non conforme], "
                  "température produits frais [température]°C, pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! J'enregistre ce contrôle de réception…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on the visible list)
             "fade",       # B -> C (continuous: dropdown stays open, select Livrée)
             "slideleft",  # C -> D (cut: filtered list appears)
             "fade",       # D -> E (continuous: click Voir le détail)
             "slideleft",  # E -> F (cut into detail page)
             "fade",       # F -> G1 (continuous: scroll down on same page)
             "fade",       # G1 -> G2 (continuous: scroll to Facture)
             "slideleft",  # G2 -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "filtrez par statut" -- click Filtrer
    "N2": S["D"] + 0.20,         # "cliquez sur Voir le détail" -- filtered list
    "N3": S["F"] + 0.20,         # fournisseur / mode livraison / dates
    "N4": S["G1"] + 0.20,        # quantité commandée/reçue + température
    "N5": S["G2"] + 0.20,        # ajouter la facture
...
FINAL = f"{ROOT}/out/foodeatup-bl-tuto-v1.mp4"
