#!/usr/bin/env python3
# FoodEatUp "Gerer ses fournisseurs cote achats" tutorial (module Comptabilite &
# Achats). Same engine as every FoodEatUp tuto (see
...
#
# Rush (79.2s): "Liste des fournisseurs" -> "Ajouter un fournisseur" -> saisie
# complete du formulaire (nom, adresse, email, telephone, categorie, livraison,
# fiabilite) -> Enregistrer -> le fournisseur apparait dans la liste -> clic
# "Modifier" -> passage de la fiabilite de 4 a 5 etoiles -> "Mettre a jour" ->
# toast succes -> clic "Supprimer" -> modale de confirmation "Etes-vous sur ?"
# -> clic "Annuler" (la suppression n'est PAS confirmee dans le rush, le
# fournisseur reste dans la liste).
#
# La saisie du formulaire et le scroll d'edition sont tres longs a l'ecran
# (creation ~44s, edition ~15s) : acceleres en montage (facteur setpts eleve)
# plutot que coupes, pour montrer que tous les champs sont bien remplis sans
# etirer le tuto.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-fournisseurs-achats-tuto"
...
BTN_AJOUTER      = (1670, 345); SZ_AJOUTER      = (330, 70)   # "Ajouter un fournisseur"
BTN_ENREGISTRER  = (1055, 745); SZ_ENREGISTRER  = (210, 80)   # "Enregistrer" (modale ajout)
BTN_MODIFIER     = (390, 515);  SZ_MODIFIER     = (175, 65)   # "Modifier" (carte Carrefour)
BTN_METTREAJOUR  = (1050, 745); SZ_METTREAJOUR  = (215, 80)   # "Mettre a jour" (modale edition)
BTN_SUPPRIMER    = (210, 657);  SZ_SUPPRIMER    = (210, 85)   # "Supprimer" (carte Carrefour)
BTN_ANNULER      = (1082, 587); SZ_ANNULER      = (160, 60)   # "Annuler" (modale confirmation)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.00,  3.20, None,  None,           None,           "1 · Liste des fournisseurs"),
    ("B", 3.85,  4.15,  0.80, 4.00,  BTN_AJOUTER,     SZ_AJOUTER,     None),
    ("C", 4.30,  48.00, 5.50, None,  None,           None,           "2 · Renseignez ses informations"),
    ("D", 49.70, 50.10, 0.80, 49.90, BTN_ENREGISTRER, SZ_ENREGISTRER, None),
    ("E", 50.30, 53.00, 2.60, None,  None,           None,           "Fournisseur ajouté"),
    ("F", 53.45, 53.75, 0.80, 53.60, BTN_MODIFIER,    SZ_MODIFIER,    None),
    ("G", 53.80, 68.60, 4.20, None,  None,           None,           "3 · Ajuster sa fiabilité"),
    ("H", 68.65, 68.95, 0.80, 68.80, BTN_METTREAJOUR, SZ_METTREAJOUR, None),
    ("I", 69.00, 72.30, 2.80, None,  None,           None,           "Fournisseur mis à jour"),
    ("J", 73.45, 73.75, 0.80, 73.60, BTN_SUPPRIMER,   SZ_SUPPRIMER,   None),
    ("K", 73.80, 77.20, 2.60, None,  None,           None,           "4 · Confirmation avant suppression"),
    ("L", 76.90, 77.20, 0.80, 77.05, BTN_ANNULER,     SZ_ANNULER,     None),
    ("M", 77.60, 79.20, 2.00, None,  None,           None,           "Annulé, rien de supprimé"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: clic Ajouter un fournisseur)
             "fade",       # B -> C (continuous: modale ouverte)
             "fade",       # C -> D (continuous: clic Enregistrer)
             "fade",       # D -> E (continuous: retour a la liste)
             "fade",       # E -> F (continuous: clic Modifier)
             "fade",       # F -> G (continuous: modale edition)
             "fade",       # G -> H (continuous: clic Mettre a jour)
             "fade",       # H -> I (continuous: toast succes)
             "fade",       # I -> J (continuous: clic Supprimer)
             "fade",       # J -> K (continuous: modale confirmation)
             "fade",       # K -> L (continuous: clic Annuler)
             "fade",       # L -> M (continuous: retour a la liste)
             "fade"]       # M -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # liste des fournisseurs
    "N2": S["C"] + 0.30,         # renseigner ses infos
    "N3": S["E"] + 0.20,         # ajoute a la liste
    "N4": S["G"] + 0.30,         # modifier / fiabilite
    "N5": S["I"] + 0.20,         # mise a jour immediate
    "N6": S["K"] + 0.20,         # confirmation avant suppression
    "N7": S["M"] + 0.20,         # annule, rien de supprime
...
FINAL = f"{ROOT}/out/foodeatup-fournisseurs-achats-tuto-v1.mp4"
