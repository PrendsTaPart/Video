#!/usr/bin/env python3
# FoodEatUp "Créer son code PIN — Accès & Jarvis" tutorial (module Équipe & Planning).
...
#
# Rush (29.4s): QR code permanent (pointage/appairage Jarvis) -> section
# "Accès des employés" -> "Définir un PIN" pour Alice Charbit -> saisie du code ->
# "Enregistrer" -> succès "PIN défini pour alice Charbit".
#
# Pas de séquence Claude ici : aucun outil MCP FoodEatUp ne couvre la définition
# d'un code PIN employé (fonctionnalité de sécurité/accès, pas de donnée métier
# exposée par le serveur MCP) -- même raison que creer-son-compte / monter-sa-
# boutique / regler-ses-unites dans le reste de la série.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-pin-jarvis-tuto"
...
BTN_DEFINE_PIN = (1494, 198); SZ_DEFINE_PIN = (150, 38)  # "Definir un PIN" (ligne Alice)
BTN_SAVE_PIN   = (1021, 521); SZ_SAVE_PIN   = (160, 56)  # "Enregistrer" (modale)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  8.00,  4.00, None,  None,           None,          "1 · QR code Jarvis"),
    ("B", 8.00,  9.30,  1.60, None,  None,           None,          "2 · Accès des employés"),
    ("C", 9.30,  9.60,  0.80, 9.45,  BTN_DEFINE_PIN, SZ_DEFINE_PIN, None),
    ("D", 10.00, 21.30, 4.50, None,  None,           None,          "3 · Code PIN (4 à 6 chiffres)"),
    ("E", 21.30, 21.60, 0.80, 21.45, BTN_SAVE_PIN,   SZ_SAVE_PIN,   None),
    ("F", 24.00, 28.50, 3.00, None,  None,           None,          "PIN défini"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous scroll to Acces des employes)
             "fade",       # B -> C (continuous: click Definir un PIN)
             "slideleft",  # C -> D (cut into the opened modal)
             "fade",       # D -> E (continuous: click Enregistrer)
             "fade",       # E -> F (submit -> success toast)
             "fade"]       # F -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # QR code Jarvis
    "N2": S["B"] + 0.10,         # Acces des employes overview
    "N3": S["C"] + 0.10,         # click Definir un PIN
    "N4": S["D"] + 0.30,         # saisie + Enregistrer
    "N5": S["F"] + 0.10,         # benefice (pointage + acces role/permissions)
...
FINAL = f"{ROOT}/out/foodeatup-pin-jarvis-tuto-v1.mp4"
