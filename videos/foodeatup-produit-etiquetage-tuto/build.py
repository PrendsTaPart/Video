#!/usr/bin/env python3
# FoodEatUp "Créer un produit pour l'étiquetage" tutorial (module HACCP).
...
#
# NO "use it with Claude" sequence on this video: mcp__FoodEatUp__* only
# exposes create_haccp_label (creates the DLC *label* instance downstream --
# dlc/lot_number/temperature/storage_location) and create_product (menu/carte,
# price+TVA). Neither matches this rush's action: creating the reusable
# "Etiqueteuse" product master (brand, barcode, allergens, shelf life, DLC
# alert threshold). Per the standing rule (FOODEATUP-TUTORIELS-WORKFLOW.md),
# skip the sequence rather than invent a prompt -- see SCRIPT.md.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-produit-etiquetage-tuto"
...
BTN_ADD_PRODUCT = (1429, 343); SZ_ADD_PRODUCT = (235, 62)  # "Ajouter un produit"
BTN_SUBMIT      = (1024, 604); SZ_SUBMIT      = (125, 62)  # "Ajouter" (modal submit)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",  0.20,  7.45,  4.50, None, None,            None,           "1 · Ouvrir Etiqueteuse"),
    ("B",  7.45,  7.65,  0.90, 7.45, BTN_ADD_PRODUCT,  SZ_ADD_PRODUCT, None),
    ("C1", 9.00,  24.00, 4.20, None, None,            None,           "2 · Nom, marque, code-barres"),
    ("C2", 24.00, 48.00, 4.70, None, None,            None,           "3 · Catégorie, poids, unité"),
    ("C3", 48.00, 58.30, 3.50, None, None,            None,           "4 · Durée de vie, alerte DLC"),
    ("D",  58.30, 58.80, 0.90, 58.30, BTN_SUBMIT,      SZ_SUBMIT,      None),
    ("E",  58.80, 64.88, 9.80, None, None,            None,           "Produit ajouté"),
]
INTRO_D, OUTRO_D = 3.00, 5.50
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C1 -> C2 (continuous scroll within the form)
             "fade",       # C2 -> C3 (tab switch Détails -> Paramètres)
             "fade",       # C3 -> D (continuous: click Ajouter)
             "fade"]       # D -> E (submit -> list updated)
    trans.append("fade")   # E -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.10,         # "ouvrez l'Etiqueteuse, cliquez sur Ajouter un produit"
    "N2": S["C1"] + 0.20,        # nom / marque / code-barres
    "N3": S["C2"] + 0.20,        # catégorie / poids / unité
    "N4": S["C3"] + 0.20,        # onglet Paramètres : durée de vie / alerte DLC
    "N5": S["D"] + 0.10,         # "cliquez sur Ajouter" -> succès
    "N6": S["E"] + 0.20,         # bénéfice : prêt pour l'impression, DLC auto
...
FINAL = f"{ROOT}/out/foodeatup-produit-etiquetage-tuto-v1.mp4"
