#!/usr/bin/env python3
# FoodEatUp "Retrouver ma traçabilité (historique)" tutorial.
# No avatar clip (module HACCP, cf. FAISABILITE-SERIE-TUTORIELS.md §3.1): full
# ElevenLabs VO throughout. Speed = setpts (never zoompan on real footage).
# xfade on every cut, forced back to yuv420p at the end of the chain. 48kHz
# stereo AAC, +faststart. Pattern copied from foodeatup-tva-tuto/build.py.
...
ROOT = "/home/user/Video/videos/foodeatup-tracabilite-historique-tuto"
...
# Buttons/cards clicked in the rush (pixel coords read frame-by-frame at
# fps=5 extraction, native 1920x828 -- see SCRIPT.md "Déroulé reconstitué").
BTN_HISTORIQUE = (1389, 138); SZ_HISTORIQUE = (140, 55)   # nav pill "Historique"
BTN_TRACA_CARD = (749, 525);  SZ_TRACA_CARD = (370, 260)  # dashboard card "Traçabilité"
BTN_ROW        = (1755, 192); SZ_ROW        = (34, 26)    # chevron, ligne "Abricot"
BTN_MODIFIER   = (1474, 370); SZ_MODIFIER   = (108, 44)   # bouton "Modifier"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
# Rush stops at 16.2s (end of the edit-modal shot) -- the delete/confirm demo
# later in the raw footage (~16.5-20s) isn't narrated by any VO line, so it's
# left out of the cut rather than shown silently.
segs = [
    ("A", 0.20, 4.20, 2.90, None,  None,          None,          "1 · Onglet Historique"),
    ("B", 4.30, 4.70, 0.70, 4.40,  BTN_HISTORIQUE, SZ_HISTORIQUE, None),
    ("C", 4.90, 6.30, 4.50, None,  None,          None,          "2 · Historique HACCP"),
    ("D", 6.30, 6.70, 0.60, 6.40,  BTN_TRACA_CARD, SZ_TRACA_CARD, None),
    ("E", 7.40, 9.00, 3.80, None,  None,          None,          None),
    ("F", 9.00, 9.40, 0.60, 9.10,  BTN_ROW,        SZ_ROW,        "3 · Detail de la fiche"),
    ("G", 9.60, 12.20, 6.70, None, None,          None,          None),
    ("H", 12.20, 12.60, 0.60, 12.30, BTN_MODIFIER, SZ_MODIFIER,  "4 · Corriger une fiche"),
    ("I", 13.00, 16.20, 4.20, None, None,          None,          None),
]
INTRO_D, OUTRO_D = 4.90, 5.80
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared module, see foodeatup-tva-tuto for
# the first use. list_haccp_tracabilite is a read action (not create): the
# prompt asks Claude to show the history rather than create a record.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Montre-moi l'historique de traçabilité HACCP de mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici l'historique de vos traçabilités HACCP…"
...
# N6 explains the prompt over stage1+2 (reused verbatim from foodeatup-tva-tuto,
# same generic line: "vous pouvez aussi le faire depuis Claude..."), N7 (own
# line) presents sending it over stage3. Measured N6=4.41s / N7=4.60s.
CLAUDE_STAGE_D = [3.20, 1.90, 5.20]  # reveal, copied, chatbot mockup
...
FINAL = f"{ROOT}/out/foodeatup-tracabilite-historique-tuto-v1.mp4"
