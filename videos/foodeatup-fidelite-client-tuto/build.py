#!/usr/bin/env python3
# FoodEatUp "Vue publique Fidélité côté client" tutorial (module marketing-fidelite).
# Rush: page fidélité publique GoSushi Démo -- onglet "Fidélité" (solde de points,
# prochaine récompense, catalogue de récompenses, historique), onglet "Mes
# commandes" (état vide), onglet "Mes infos" -> "Mon compte" (connexion
# passwordless par email). Le rush revient brièvement sur l'onglet Fidélité
# entre les clics "Mes commandes" et "Mes infos" (redite, rien de nouveau à
# l'écran) : coupé au montage, comme les temps morts habituels de la série.
# No avatar clip: full ElevenLabs VO (Adam FR). Speed = setpts (never zoompan
...
ROOT = "/home/user/Video/videos/foodeatup-fidelite-client-tuto"
...
BTN_MES_COMMANDES = (801, 338); SZ_MES_COMMANDES = (294, 46)
BTN_MES_INFOS     = (1065, 338); SZ_MES_INFOS     = (209, 46)
...
# tools: mcp__FoodEatUp__get_loyalty_account(establishment_id, email) for the
# points balance + next-reward progress shown on screen, then
# mcp__FoodEatUp__list_loyalty_rewards(establishment_id) for the rewards
# catalog -- exactly the two blocks of information the rush displays (solde
# + progression, puis catalogue de récompenses). Read-only consultation, no
# creation involved (this is the public client-facing view).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Vérifie le solde de points, la prochaine récompense et le "
                  "catalogue de récompenses fidélité de [email du client] pour "
                  "mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère son compte fidélité et les récompenses…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B1 (continuous scroll begins)
             "fade",       # B1 -> B2 (continuous scroll)
             "fade",       # B2 -> C (continuous: click on the visible tab)
             "slideleft",  # C -> D (cut into the loaded Mes commandes tab)
             "slideleft",  # D -> E (cut: rush revisits Fidélité, click Mes infos)
             "slideleft",  # E -> F (cut into Mon compte)
             "slideleft",  # F -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B1"] + 0.15,        # solde + prochaine récompense
    "N2": S["B2"] + 0.15,        # récompenses + historique
    "N3": S["D"] + 0.15,         # onglet Mes commandes
    "N4": S["F"] + 0.15,         # Mon compte (connexion passwordless)
    "N5": S["claude1"] + 0.20,   # explique le prompt (reveal + copié)
    "N6": S["claude3"] + 0.20,   # colle dans Claude -> résultat
...
FINAL = f"{ROOT}/out/foodeatup-fidelite-client-tuto-v1.mp4"
