#!/usr/bin/env python3
# FoodEatUp "Valider une production (quantité, température, note)" tutorial.
#
# No avatar clip in this folder (module HACCP ships none -- see
# FAISABILITE-SERIE-TUTORIELS.md §3.1), so the intro card carries N0 and the
# ElevenLabs voice covers the whole narration.
#
# Shared "use it with Claude" 3-stage sequence: validate_production
# (establishment_id, production_id, produced_quantity, temperature_log, notes)
# matches the wizard exactly -- quantity, HACCP control, note, confirm.
#
...
ROOT = "/home/user/Video/videos/foodeatup-production-tuto"
...
def banner(text, seg_dur):
    # No apostrophes in banner text: drawtext injects it between single quotes
    # in -vf, an apostrophe breaks the whole filter chain (bug hit on the
    # ingredients tuto).
...
# Coordinates measured on the actual frames (source space 1920x828).
BTN_VALIDER   = (1592, 504); SZ_VALIDER   = (352, 44)   # green "Valider la production"
BTN_CONFIRMER = (1216, 646); SZ_CONFIRMER = (304, 64)   # blue "Confirmer et valider"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.00,  5.50,  5.00, None,  None,          None,          "1 · Ouvrez Mes productions"),
    ("B", 5.55,  5.95,  0.90, 5.70,  BTN_VALIDER,   SZ_VALIDER,    None),
    ("C", 5.95, 11.95,  6.60, None,  None,          None,          "2 · Quantité réellement produite"),
    ("D", 12.15, 19.95, 7.60, None,  None,          None,          "3 · Contrôle HACCP et notes"),
    ("E", 20.05, 23.40, 3.60, None,  None,          None,          "4 · Récapitulatif avant validation"),
    ("F", 23.10, 23.50, 0.90, 23.50, BTN_CONFIRMER, SZ_CONFIRMER,  None),
    ("G", 26.40, 32.15, 5.60, None,  None,          None,          "Production validée"),
]
...
# temperature_log, notes) -- one call covers the whole 3-step wizard shown on
# screen, which is exactly the point of the prompt.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Valide la production [nom de la production] pour mon établissement "
                 "FoodEatUp (ID [ID établissement]) : quantité réellement produite "
                 "[nombre] portions, température de contrôle [température] degrés, "
                 "note [votre remarque].")
CLAUDE_RESPONSE = "Bien sûr ! Je valide la production et mets le stock à jour…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # B -> C (button click opens the wizard)
             "slideleft",  # C -> D (step 1 -> step 2)
             "slideleft",  # D -> E (step 2 -> step 3)
             "fade",       # E -> F (continuous: click Confirmer et valider)
             "fade",       # F -> G (submit -> refreshed list)
             "slideleft",  # G -> claude1
...
anchor = {
    "N0": 0.35,                  # intro card
    "N1": S["A"] + 0.25,         # open Mes productions -> click Valider la production
    "N2": S["C"] + 0.20,         # step 1: produced quantity / efficiency
    "N3": S["D"] + 0.20,         # step 2: HACCP control + notes
    "N4": S["E"] + 0.20,         # step 3: recap -> Confirmer et valider
    "N5": S["G"] + 0.25,         # stock updated, HACCP history
...
FINAL = f"{ROOT}/out/foodeatup-production-tuto-v1.mp4"
