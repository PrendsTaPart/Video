#!/usr/bin/env python3
# FoodEatUp "Suivre ses performances (côté employé)" tutorial.
#
# Short rush (18.42s), Drive folder 18 ("Affichages des performances de
# l'employé") -- confirmed matching file size on Google Drive, no mismatch
# this time. Source recording includes the full Chrome browser chrome
# (tabs/address bar), cropped out in assets/screen.mp4 (crop=1920:828:0:191)
# to match the rest of the series' framing.
#
# Employee-side "Performances" page: score circle (Excellent/Moyen/A
# ameliorer), per-day hours cards, team ranking ("Classement de l'equipe"),
# and a weekly performance bar chart. No MCP tool covers the performance
# score itself, but list_attendances(establishment_id, date_from, date_to)
# is a real match for a director-facing "team dashboard" prompt (hours,
# attendance) -- same tool as retrouver-les-pointages-historique /
# creer-ses-roles-et-permissions, reframed here per Michael's explicit
# request for a dashboard + cas d'usage.
#
# No avatar. Same engine as the rest of the series: setpts for speed (never
# zoompan on real footage), fixed crop+scale zoom-punch on the "Performances"
# menu click, xfade on every cut forced back to yuv420p, 48kHz stereo AAC,
# +faststart.
...
ROOT = "/home/user/Video/videos/foodeatup-performances-employe-tuto"
...
# Coordinates measured on the cropped 1920x828 frames.
BTN_PERFORMANCES = (1627, 422); SZ_PERFORMANCES = (300, 40)  # "Performances" menu item

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  0.00, 3.00, 5.00, None,              None,             "Mon planning — vue employé"),
    ("A2", 3.40, 4.30, 4.00, BTN_PERFORMANCES,  SZ_PERFORMANCES,  "Ouvrir Performances"),
    ("B",  7.00, 8.30, 8.50, None,               None,            "Mon score de performance"),
    ("C",  9.00, 13.00, 7.00, None,              None,            "Classement & historique"),
]
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared 3-stage chatbot animation. No MCP
# tool computes the FoodEatUp performance score itself, but a director can
# ask Claude for a real team dashboard from attendance data via
# list_attendances(establishment_id, date_from, date_to) -- same tool as
# retrouver-les-pointages-historique / creer-ses-roles-et-permissions.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Fais-moi un dashboard de performance de mon équipe (heures "
                  "travaillées, ponctualité) pour la semaine du [date début] "
                  "au [date fin], pour mon établissement FoodEatUp (ID [ID "
                  "établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère les pointages et je vous prépare le dashboard…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> A2 (continuous: open the profile menu)
             "slideleft",  # A2 -> B (Performances page loads)
             "fade",       # B -> C (scroll down to ranking + chart)
             "slideleft",  # C -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]  + 0.20,  # mon planning
    "N2": S["A2"] + 0.10,  # ouvre Performances
    "N3": S["B"]  + 0.20,  # score + statut + heures
    "N4": S["C"]  + 0.20,  # classement + historique
    "N5": S["claude1"] + 0.20,  # explique le prompt (reveal + copied)
    "N6": S["claude3"] + 0.20,  # colle dans Claude -> resultat instantane
...
FINAL = f"{ROOT}/out/foodeatup-performances-employe-tuto-v1.mp4"
