#!/usr/bin/env python3
# FoodEatUp "Créer un devis" (Comptabilité) tutorial.
#
# Rich rush (99.3s), Drive folder "3- Créer un devis" -- confirmed matching
# file size on Google Drive. Covers: Devis list -> "Créer un devis" -> pick
# client/délai/mode de règlement -> search & add a product (auto price/TVA)
# -> global remise/TVA/devise -> acompte demandé (deposit %) -> "Enregistrer
# & Envoyer" -> success toast -> back to list with the new quote. The rush
# also shows editing/deleting a quote afterwards, out of scope here (video
# stays focused on creation, per Michael's request).
#
# No avatar. Same engine as the rest of the series: setpts for speed (never
# zoompan on real footage), fixed crop+scale zoom-punch on key buttons/UI,
...
ROOT = "/home/user/Video/videos/foodeatup-creer-devis-tuto"
...
# Coordinates measured on the 1920x828 frames.
BTN_CREER_DEVIS = (1697, 329); SZ_CREER_DEVIS = (185, 50)   # "Créer un devis" (list page)
BTN_SALMON_MAKI = (513, 403);  SZ_SALMON_MAKI = (745, 50)   # autocomplete option "Salmon Maki"
BTN_ACOMPTE     = (513, 530);  SZ_ACOMPTE     = (745, 300)  # "Acompte demandé" panel + résultat
BTN_ENVOYER     = (1622, 535); SZ_ENVOYER     = (270, 55)   # "Enregistrer & Envoyer"
BTN_TOAST       = (1614, 90);  SZ_TOAST       = (530, 150)  # toast de succès

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  0.00,  4.00, 7.00, None,            None,             "Liste de vos devis"),
    ("A2", 3.70,  4.30, 3.00, BTN_CREER_DEVIS, SZ_CREER_DEVIS,    "Créer un devis"),
    ("B",  5.50,  8.00, 5.50, None,            None,             "Client, délai, paiement"),
    ("C",  12.60, 13.40, 6.00, BTN_SALMON_MAKI, SZ_SALMON_MAKI,   "Ajouter un produit"),
    ("D",  24.00, 26.00, 6.00, None,           None,             "Quantité & prix auto"),
    ("E",  44.00, 46.00, 8.50, BTN_ACOMPTE,    SZ_ACOMPTE,        "Remise, TVA & acompte"),
    ("F1", 60.00, 61.20, 4.50, BTN_ENVOYER,    SZ_ENVOYER,        "Enregistrer & Envoyer"),
    ("F2", 63.30, 64.30, 5.50, BTN_TOAST,      SZ_TOAST,          None),
    ("G",  69.00, 71.00, 5.00, None,           None,             "Devis créé"),
]
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared 3-stage chatbot animation. Real
# match: create_quote(establishment_id, items[], client_id, discount_amount,
# tax_rate, advance_percentage, payment_method, quote_date, validity_date) --
# same fields shown on screen (produit/quantité/prix/TVA, remise, acompte).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée un devis pour le client [nom du client], avec "
                  "[quantité] [nom du produit] à [prix]€ HT (TVA [taux]%), "
                  "pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée le devis avec ces lignes tout de suite…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> A2 (click Créer un devis)
             "slideleft",  # A2 -> B (form opens)
             "fade",       # B -> C (continuous: search a product)
             "fade",       # C -> D (product added, qty/prix)
             "fade",       # D -> E (scroll to remise/TVA/acompte)
             "fade",       # E -> F1 (click Enregistrer & Envoyer)
             "slideleft",  # F1 -> F2 (toast + back to list)
             "fade",       # F2 -> G (new devis visible)
             "slideleft",  # G -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]   + 0.20,  # liste des devis
    "N2": S["A2"]  + 0.10,  # clic Créer un devis
    "N3": S["B"]   + 0.20,  # client/délai/paiement
    "N4": S["C"]   + 0.20,  # ajouter un produit
    "N5": S["D"]   + 0.20,  # quantité/prix
    "N6": S["E"]   + 0.20,  # remise/tva/acompte
    "N7": S["F1"]  + 0.10,  # enregistrer & envoyer + toast + retour liste
    "N8": S["claude1"] + 0.20,  # explique le prompt (reveal + copied)
    "N9": S["claude3"] + 0.20,  # colle dans Claude -> resultat instantane
    "N10": OUTRO_START + 0.35,  # CTA
}
keys = ["N1","N2","N3","N4","N5","N6","N7","N8","N9","N10"]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-creer-devis-tuto-v1.mp4"
