#!/usr/bin/env python3
# FoodEatUp "Choisir son abonnement" tutorial (rebuild -- the previous version
# predates this pipeline and shipped with the 96kHz mono / no-faststart bug,
# and no animations).
#
# No avatar clip in this project: full ElevenLabs VO throughout, same as
# foodeatup-inscription-tuto. Speed = setpts (never zoompan on real footage --
# freezes the image). xfade on every cut, forced back to yuv420p at the end
# of the chain (xfade negotiates 4:4:4 with its inputs and libx264 will keep
# an unplayable profile if you let it). 48kHz stereo AAC, +faststart.
...
BTN_CHOISIR_PACK = (1522, 693); SZ_CHOISIR_PACK = (516, 76)
BTN_CONTINUER    = (1037, 707); SZ_CONTINUER    = (326, 50)
BTN_DEMARRER     = (1373, 641); SZ_DEMARRER     = (600, 62)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.30,  2.30,  3.20, None, None,             None,             "1 · Choisissez votre pack"),
    ("B", 2.30,  2.80,  0.90, 2.30, BTN_CHOISIR_PACK,  SZ_CHOISIR_PACK,  None),
    ("C", 3.00,  7.40,  4.30, None, None,             None,             "2 · Récapitulatif de commande"),
    ("D", 7.40,  7.90,  0.90, 7.40, BTN_CONTINUER,     SZ_CONTINUER,     None),
    ("E", 10.20, 14.00, 2.80, None, None,             None,             "3 · Vos coordonnées"),
    ("F", 14.00, 44.00, 6.80, None, None,             None,             "4 · Informations de paiement"),
    ("G", 44.00, 44.50, 0.90, 44.00, BTN_DEMARRER,    SZ_DEMARRER,      None),
    # 44.5 -> 54.0 (processing spinner then a small checkmark) is cut: dead time,
    # nothing new is communicated before the final confirmation screen.
    ("H", 54.00, 56.80, 3.60, None, None,             None,             "Abonnement activé"),
]
# Target durations are close to their matching VO line's measured length (see
# vo/*.mp3) -- the push-chain that keeps VO lines from overlapping is
# monotonic, so once one line runs long every later offset is pushed too and
# the whole deficit lands on the auto-extended outro (hit 22s of silent card
# before this rebalance -- fixed by shortening N1/N2/N6 AND loosening targets).
...
def build_silent(outro_d):
    ...
    # E->F->G are continuous typing into the click (fade); G->H skips the
    # processing/checkmark dead time (slideleft reads as "moving on").
    trans = ["fade","fade","fade","fade","fade","fade","fade","slideleft","fade"]
...
FINAL = f"{ROOT}/out/foodeatup-abonnement-tuto-v1.mp4"
