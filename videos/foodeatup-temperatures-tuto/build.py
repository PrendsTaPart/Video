#!/usr/bin/env python3
# FoodEatUp "Ajouter une température de production (plats)" tutorial.
# No Claude-prompt sequence: add_temperature (MCP) only covers the
# Equipements tab, not Plats shown here; create_recipe/create_dish don't
# have the fields seen on screen (allergenes, duree de vie, piece jointe).
...
# +faststart. Segment targets set close to each VO line's measured
# duration (see vo/*.mp3) before building, not after.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-temperatures-tuto"
...
BTN_ADD_RELEVE  = (1668, 304); SZ_ADD_RELEVE  = (170, 46)   # "+ Ajouter un relevé" (haut droite)
BTN_ADD_RECETTE = (1382, 334); SZ_ADD_RECETTE = (220, 26)   # "+ Ajouter une recette" (lien)
BTN_SAVE_SUB    = (1018, 648); SZ_SAVE_SUB    = (163, 50)   # "Enregistrer" (sous-modale Nouvelle recette)
BTN_SAVE_MAIN   = (1024, 648); SZ_SAVE_MAIN   = (163, 50)   # "Enregistrer" (modale principale, submit final)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.30, 3.60, 4.05, None, None,            None,            "Aucun releve pour le moment"),
    ("B", 4.30, 4.60, 0.90, 4.45, BTN_ADD_RELEVE,   SZ_ADD_RELEVE,   None),
    ("C", 5.00, 10.80, 3.15, None, None,            None,            "Choisir un plat existant"),
    ("D", 24.50, 24.80, 0.90, 24.65, BTN_ADD_RECETTE, SZ_ADD_RECETTE, None),
    ("E", 26.00, 45.00, 6.30, None, None,            None,            "Nouvelle recette a la volee"),
    ("F", 57.90, 58.20, 0.90, 58.05, BTN_SAVE_SUB,   SZ_SAVE_SUB,     None),
    ("G", 58.90, 59.60, 4.60, None, None,            None,            "Seuil recommande : 63C minimum"),
    ("H", 60.00, 60.30, 0.90, 60.15, BTN_SAVE_MAIN,  SZ_SAVE_MAIN,    None),
    ("I", 60.30, 60.60, 2.85, None, None,            None,            None),
    ("J", 61.50, 65.50, 4.60, None, None,            None,            "Releve enregistre"),
]
INTRO_D, OUTRO_D = 4.95, 6.20
...
FINAL = f"{ROOT}/out/foodeatup-temperatures-tuto-v1.mp4"
