#!/usr/bin/env python3
# FoodEatUp "Ajouter ses fournisseurs" tutorial.
...
# avoids: checking only "does the total fit" lets a multi-second voice/
# picture offset build up by the end of the video).
...
BTN_ADD_SUPPLIER = (1621, 346); SZ_ADD_SUPPLIER = (343, 57)  # "Ajouter un fournisseur"
BTN_SUBMIT        = (1034, 735); SZ_SUBMIT       = (161, 63)  # "Enregistrer"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  1.60,  2.20, None, None,             None,              "1 · Ajouter un fournisseur"),
    ("B", 1.60,  1.85,  0.90, 1.70, BTN_ADD_SUPPLIER,  SZ_ADD_SUPPLIER,   None),
    ("C", 2.00,  20.00, 5.50, None, None,             None,              "2 · Nom et adresse"),
    ("D", 20.00, 44.00, 5.00, None, None,             None,              "3 · Email et téléphone"),
    ("E", 44.00, 54.00, 4.50, None, None,             None,              "4 · Produits, livraison, fiabilité"),
    ("F", 55.60, 55.85, 0.90, 55.75, BTN_SUBMIT,       SZ_SUBMIT,         None),
    ("G", 56.00, 60.30, 3.80, None, None,             None,              "Fournisseur ajouté"),
]
...
# create_supplier(establishment_id, name, adresse, ville, pays, code_postal,
# email, tel, website, type, livraison, reliability, status) is the
# matching tool -- fields checked against the tool schema before writing
# the prompt below, they line up with what the rush actually shows on
# screen (nom, adresse, email, tel, type de produits, livraison).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée le fournisseur [nom] à [adresse], email [email], "
                  "tél [téléphone], type [catégorie de produits], livraison "
                  "[oui/non], pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée ce fournisseur pour votre établissement…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # C -> D (cut, name/address -> email/phone)
             "slideleft",  # D -> E (cut, contact -> produits/livraison/fiabilité)
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Ajouter un fournisseur" on the click
    "N2": S["C"] + 0.20,         # nom + adresse
    "N3": S["D"] + 0.20,         # email + téléphone
    "N4": S["E"] + 0.20,         # catégories, livraison, fiabilité
    "N5": S["F"] + 0.10,         # cliquez sur Enregistrer -> succès
