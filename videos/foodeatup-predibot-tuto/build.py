#!/usr/bin/env python3
# FoodEatUp "Prédire ses commandes (ventes & production)" tutorial -- PrediBot.
# No avatar clip, no Claude sequence (no matching mcp__FoodEatUp__* tool -- see
# SCRIPT.md). Full ElevenLabs VO throughout except N7 (CTA), reused verbatim
# from foodeatup-fournisseurs-tuto/vo/N8.mp3 (byte-identical across the series).
...
ROOT = "/home/user/Video/videos/foodeatup-predibot-tuto"
...
# Coordinates measured from extracted frames (work/scan/, 5fps scan of the raw
# rush) -- chart icon on the Pizza Margherita card, and the "Exporter PDF"
# button inside the predictions modal.
BTN_CHART  = (752, 151);  SZ_CHART  = (44, 44)   # icône graphique (carte plat)
BTN_EXPORT = (1021, 743); SZ_EXPORT = (218, 68)  # "Exporter PDF"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.00, 4.60,  5.10, None, None,       None,       "1 - Ouvrez Ma carte et repérez votre plat"),
    ("B", 4.60, 4.85,  0.90, 4.70, BTN_CHART,  SZ_CHART,   None),
    ("C", 4.85, 6.60,  10.00, None, None,      None,       "2 - Consultez les prédictions IA sur 7 jours"),
    ("D", 6.60, 6.85,  0.90, 6.70, BTN_EXPORT, SZ_EXPORT,  None),
    ("E", 6.85, 12.88, 7.00, None, None,       None,       "3 - Exportez le rapport PDF"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C -> D (continuous: click Exporter PDF)
             "slideleft"]  # D -> E (cut: toast + PDF ouvert)
...
anchor = {
    "N0": 0.30,                 # intro hook
    "N1": S["A"] + 0.30,        # repérez le plat à analyser
    "N2": S["B"] + 0.10,        # clic sur l'icône graphique
    "N3": S["C"] + 0.20,        # tableau des prédictions 7 jours
    "N4": S["C"] + 4.50,        # indices de performance / confiance
    "N5": S["D"] + 0.10,        # clic Exporter PDF
    "N6": S["E"] + 0.30,        # bénéfice (PDF ouvert)
...
FINAL = f"{ROOT}/out/foodeatup-predibot-tuto-v1.mp4"
