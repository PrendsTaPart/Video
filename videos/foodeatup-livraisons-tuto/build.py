#!/usr/bin/env python3
# FoodEatUp "Suivre ses livraisons (statuts & dates)" tutorial.
# Module StockVisionAI. No avatar clip: full ElevenLabs VO throughout.
# Speed = setpts (never zoompan on real footage). xfade on every cut,
...
# exposes list_deliveries (read-only) for supplier deliveries -- no
# update_delivery_status/date/mode tool exists. update_order_status exists
# but targets client orders (different object/statuses). Per the standing
# rule (FOODEATUP-TUTORIELS-WORKFLOW.md), skip the sequence rather than
# invent a prompt. See SCRIPT.md.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-livraisons-tuto"
...
BTN_SHIP = (372, 608); SZ_SHIP = (486, 58)  # "Marquer comme expédiée" (card 1)

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A", 0.20,  5.80,  1.80, None,     None,    "1 · Apercu des livraisons"),
    ("B", 6.00,  14.30, 3.60, None,     None,    "2 · Filtrer par statut"),
    ("C", 15.80, 22.60, 3.00, None,     None,    "3 · Modifier la date prévue"),
    ("D", 23.00, 32.60, 3.00, None,     None,    "4 · Modifier le mode de livraison"),
    ("E1",34.70, 35.60, 0.90, BTN_SHIP, SZ_SHIP, None),
    ("E2",35.60, 39.50, 4.50, None,     None,    None),
    ("F", 39.60, 44.40, 4.60, None,     None,    "Statut mis à jour"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",        # intro -> A
             "fade",        # A -> B
             "slideleft",   # B -> C (cut into the date modal)
             "fade",        # C -> D (list -> mode modal)
             "fade",        # D -> E1 (list -> click punch)
             "fade",        # E1 -> E2 (click -> confirm modal)
             "fade",        # E2 -> F (confirmed -> final list)
             "fade"]        # F -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.15,         # filtrer par statut
    "N2": S["C"] + 0.15,         # modifier la date prévue
    "N3": S["D"] + 0.15,         # modifier le mode de livraison
    "N4": S["E1"] + 0.10,        # marquer comme expédiée -> confirmer
    "N5": S["F"] + 0.15,         # bénéfice : vue claire du départ à la réception
...
FINAL = f"{ROOT}/out/foodeatup-livraisons-tuto-v1.mp4"
