#!/usr/bin/env python3
# FoodEatUp "Decouvrir son accueil selon son role" tutorial (module Equipe & Planning).
...
#
# Rush: "mon espace" (manager Alice) -- tour du tableau de tuiles accessible
# selon le role (Tableau de bord, Stocks, Produits, Recettes, Carte & Menu,
# Production, Fournisseurs, Courses, Clients, Factures, Devis, Depenses,
# Finance, Livraisons, RH, AnalytiX BI, PrediBot IA -- 17 modules) -> menu
# "Equipe" (Pointages, Planning equipe, QR Codes Employes, Employes, Roles,
# Gestion Conges) -> "Roles" : cartes Admin (245 permissions) / Manager (210
# permissions), bouton "Creer un role" -> clic "Modifier" sur Manager ->
# modal "Modifier le role" : liste des modules avec compte de permissions
# (Tableau de bord 4/4, Gestion des Stocks 11/11, ... HACCP 0/41, Historique
# HACCP 0/16 -- le role Manager n'a AUCUN acces HACCP par defaut, exemple
# concret de la granularite du systeme) -> retour a "mon espace" (Alice
# retrouve exactement le meme espace, ses permissions n'ont pas change).
# Pas de clic net sur "Sauvegarder"/"Annuler" dans le rush -- transition
# traitee comme un simple cut, pas de zoom-punch invente sur ce bouton.
...
ROOT = "/home/user/Video/videos/foodeatup-accueil-role-tuto"
...
BTN_ROLES_ITEM = (90, 359);   SZ_ROLES_ITEM = (300, 36)   # "Roles" (menu Equipe)
BTN_MODIFIER   = (1073, 546); SZ_MODIFIER   = (228, 50)   # "Modifier" (carte Manager)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.00,  3.00, None,  None,           None,          "1 · Mon espace selon le rôle"),
    ("B", 2.00,  9.00,  6.00, None,  None,           None,          "Modules selon vos permissions"),
    ("C", 17.40, 18.30, 4.00, None,  None,           None,          "2 · Équipe > Rôles"),
    ("D", 18.30, 18.60, 0.90, 18.45, BTN_ROLES_ITEM, SZ_ROLES_ITEM, None),
    ("E", 20.00, 23.50, 7.00, None,  None,           None,          "3 · Rôles : Admin et Manager"),
    ("F", 24.00, 24.30, 0.90, 24.15, BTN_MODIFIER,   SZ_MODIFIER,   None),
    ("G", 25.00, 33.00, 5.50, None,  None,           None,          "4 · Permissions par module"),
    ("H", 33.50, 34.50, 8.00, None,  None,           None,          "HACCP : 0 permission (manager)"),
    ("I", 36.60, 41.52, 7.50, None,  None,           None,          "Accueil recalculé selon le rôle"),
]
INTRO_D, OUTRO_D = 7.00, 6.20
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__create_employee
# (establishment_id, first_name, last_name, email, phone_number, role?,
# schedule?) is the matching tool -- role accepts a plain string; the app UI
# also allows creating fully custom roles with granular permissions (see
# "Creer un role" in the rush) which has no dedicated MCP tool -- creating a
# custom role stays a manual UI step, only assigning an (existing) role name
# to an employee is scriptable via Claude.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée l'employé [prénom] [nom], email [email], téléphone "
                  "[téléphone], avec le rôle [rôle], pour qu'il ait "
                  "exactement les permissions déjà définies pour ce rôle, "
                  "pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cet employé avec les permissions de ce rôle…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: scrolling the same page)
             "slideleft",  # B -> C (cut: menu Equipe opened)
             "fade",       # C -> D (continuous: click on Roles)
             "slideleft",  # D -> E (cut, Roles page loaded)
             "fade",       # E -> F (continuous: click on Modifier)
             "slideleft",  # F -> G (cut into the opened modal)
             "fade",       # G -> H (continuous: scrolled further, HACCP row)
             "slideleft",  # H -> I (cut, modal closed, back to mon espace)
             "slideleft",  # I -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # tour de mon espace (manager)
    "N2": S["C"] + 0.10,         # nav Equipe > Roles
    "N3": S["E"] + 0.20,         # Roles : Admin/Manager, compteurs de permissions
    "N4": S["G"] + 0.20,         # Modifier le role : permissions par module
    "N5": S["H"] + 0.20,         # HACCP 0/41 + benefice (accueil recalcule)
...
FINAL = f"{ROOT}/out/foodeatup-accueil-role-tuto-v1.mp4"
