#!/usr/bin/env python3
# FoodEatUp "Prendre une photo, faire analyser par l'IA, obtenir un rapport
# de nettoyage" (Module Hygiene, Controle de conformite) tutorial.
...
#
# Rush shows: Accueil -> Hygiene > Controle de conformite (zones "A - Cuisine
# Quotidien" / "B - Cuisine Hebdo") -> Note optionnelle -> upload photo
# (Choisir un fichier / Prendre une photo) -> "Envoyer pour analyse" ->
# bandeau succes "L'analyse de conformite a ete declenchee automatiquement."
#
# NO Claude sequence on this one: the on-screen action (upload a photo, IA
# analyse automatique) has no matching mcp__FoodEatUp__* tool -- the closest
# candidate (create_hygiene_checklist_validation) is a manual key/value
# checklist submission, not a photo-upload/AI-analysis call, so per the
# pipeline rule ("pas de prompt invente") no claudePrompt/sequence is added,
# neither here nor on the Lovable tutorial entry.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-nettoyage-ia-tuto"
...
BTN_PHOTO   = (1085, 551); SZ_PHOTO   = (215, 52)  # "Prendre une photo"
BTN_ENVOYER = (243, 680);  SZ_ENVOYER = (220, 64)  # "Envoyer pour analyse"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.00,  2.60, None,  None,        None,      "1 - Hygiène"),
    ("B", 3.00,  9.00,  5.00, None,  None,        None,      "2 - Contrôle de conformité"),
    ("C", 9.00,  9.35,  0.35, 9.15,  BTN_PHOTO,    SZ_PHOTO,  None),
    ("D", 9.35,  22.50, 4.40, None,  None,        None,      "3 - Uploader la photo"),
    ("E", 22.50, 22.85, 0.35, 22.65, BTN_ENVOYER,  SZ_ENVOYER, None),
    ("F", 22.85, 26.00, 6.60, None,  None,        None,      "4 - Analyse IA en cours"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut: navigation to Conformité)
             "fade",       # B -> C (continuous: click on the visible page)
             "fade",       # C -> D (continuous: file picker -> upload)
             "fade",       # D -> E (continuous: click Envoyer pour analyse)
             "fade",       # E -> F (continuous: sending -> success banner)
             "fade"]       # F -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # Hygiène -> Contrôle de conformité, zone
    "N2": S["C"] + 0.10,         # note + import/prendre une photo
    "N3": S["E"] + 0.10,         # "cliquez sur Envoyer pour analyse"
    "N4": S["F"] + 0.10,         # IA analyse + notification (bénéfice)
    "N5": OUTRO_START + 0.35,    # CTA
}
keys = ["N0", "N1", "N2", "N3", "N4", "N5"]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-nettoyage-ia-tuto-v1.mp4"
