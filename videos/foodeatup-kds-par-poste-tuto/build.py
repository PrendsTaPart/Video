#!/usr/bin/env python3
# FoodEatUp "Afficher le KDS par poste" tutorial.
# Postes de travail (chaud/pass/froid) -> clic "Ouvrir l'écran" -> écran KDS dédié
# au poste Chaud -> bascule sur l'onglet Pass -> bascule sur l'onglet Froid (vide).
# Segments dimensionnés directement sur les VO mesurées (leçon retenue de
# foodeatup-fidelite-multicanal-tuto : ne pas deviner, mesurer vo/*.mp3 avant de
# fixer les cibles). banner() réutilise le pattern corrigé (2 drawtext, pas
# drawbox+t) dès la première passe.
...
ROOT = "/home/user/Video/videos/foodeatup-kds-par-poste-tuto"
...
def banner(text, seg_dur):
    # Pattern corrigé (voir FOODEATUP-TUTORIELS-WORKFLOW.md + SCRIPT.md de
    # foodeatup-fidelite-multicanal-tuto) : drawbox n'évalue x qu'une fois sur
    # cet ffmpeg (6.1.1). Deux drawtext avec box=1 à la place -- drawtext
    # réévalue t à chaque frame.
...
BTN_OPEN = (1221, 282); SZ_OPEN = (158, 38)   # "Ouvrir l'écran" (poste chaud)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
# Noir de transition d'onglet ~3,7s -> ~6,7s (vérifié frame par frame, comme sur
# fidelite-multicanal-tuto) : segment C démarre après, à 6,80s. Idem pour les
...
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Affiche la charge de mes postes cuisine (KDS) pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici la charge de chacun de vos postes en direct…"
...
def build_silent(outro_d):
    ...
    # parts = [intro, A, B, C, D, E, claude1, claude2, claude3, outro] (10)
    # -> 9 transitions. "slideleft" = coupure de scène (tab switch B->C,
    # bascule de poste C->D et D->E, puis la séquence Claude comme sur le
    # tuto précédent) ; "fade" partout ailleurs (action continue à l'écran).
    trans = ["fade"] * (len(parts) - 1)
    trans[2] = "slideleft"  # B (clic Ouvrir l'écran) -> C (écran KDS Chaud)
    trans[3] = "slideleft"  # C -> D (bascule poste Pass)
    trans[4] = "slideleft"  # D -> E (bascule poste Froid)
    trans[5] = "slideleft"  # E -> claude1
...
FINAL = f"{ROOT}/out/foodeatup-kds-par-poste-tuto-v1.mp4"
