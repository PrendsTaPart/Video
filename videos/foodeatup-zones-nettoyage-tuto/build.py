#!/usr/bin/env python3
# FoodEatUp "Retrouver l'historique de ses zones de nettoyage" tutorial (HACCP).
# Full ElevenLabs VO (N0-N5, N7 generated; N6/N8 reused verbatim from
# foodeatup-fournisseurs-tuto/vo/ -- byte-identical generic lines, see SCRIPT.md).
# Claude sequence: mcp__FoodEatUp__list_cleaning_actions matches exactly what the
# rush shows (historique filtrable par période). Speed = setpts (never zoompan on
# real footage). xfade on every cut, forced back to yuv420p at the end of the
...
ROOT = "/home/user/Video/videos/foodeatup-zones-nettoyage-tuto"
...
def banner(text, seg_dur):
    ...
    return (f"drawbox=x='{x}':y={y}:w=10:h=62:color={ORANGE}@0.98:t=fill,"
            ...
            f"drawtext=fontfile={FONT}:text='{text}':fontsize=27:fontcolor=white"
            f":x='({x})+34':y={y+16}")

# Coordinates measured from extracted frames (work/scan/, 2fps scan of the rush).
BTN_HISTORIQUE = (1390, 138); SZ_HISTORIQUE = (140, 50)   # nav "Historique"
CARD_NETTOYAGE = (1151, 665); SZ_NETTOYAGE  = (380, 260)  # carte "Plan de nettoyage"
EVENT_DAY28    = (462, 440);  SZ_DAY28      = (230, 30)   # action calendrier du 28
BTN_EXPORT     = (1697, 344); SZ_EXPORT     = (215, 50)   # "Exporter PDF"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.00,  6.80,  3.00,  None,  None,           None,          "1 - Zones de nettoyage puis Historique"),
    ("B", 6.80,  7.10,  0.90,  6.90,  BTN_HISTORIQUE,  SZ_HISTORIQUE, None),
    ("C", 7.30,  13.50, 9.50,  None,  None,           None,          "2 - Carte Plan de nettoyage"),
    ("D", 13.50, 16.10, 0.90,  13.60, CARD_NETTOYAGE,  SZ_NETTOYAGE,  None),
    ("E", 16.30, 22.50, 10.50, None,  None,           None,          "3 - Consultez et mettez a jour une action"),
    ("F", 34.30, 34.60, 0.90,  34.40, BTN_EXPORT,      SZ_EXPORT,     None),
    ("G", 37.50, 41.32, 9.00,  None,  None,           None,          "4 - Rapport PDF exporté"),
]
...
# mcp__FoodEatUp__list_cleaning_actions(establishment_id, date_from, date_to,
# limit) matches exactly what the rush shows: historique des actions de
# nettoyage, filtrable par période.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Liste mes actions de nettoyage de la zone [nom de la zone] "
                  "entre le [date de début] et le [date de fin], pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère l'historique de nettoyage de votre établissement…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on Historique)
             "slideleft",  # B -> C (cut into historique haccp page)
             "fade",       # C -> D (continuous: click Plan de nettoyage)
             "slideleft",  # D -> E (cut into Actions de nettoyage / detail)
             "slideleft",  # E -> F (cut: back to list, click Exporter PDF)
             "slideleft",  # F -> G (cut: PDF ouvert)
             "slideleft",  # G -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # ouvrez la carte Plan de nettoyage
    "N2": S["C"] + 0.20,         # compteurs + calendrier
    "N3": S["E"] + 0.20,         # cliquez sur une action -> détail
    "N4": S["E"] + 4.50,         # mise à jour du statut
    "N5": S["G"] + 0.20,         # exporter PDF -> rapport
...
FINAL = f"{ROOT}/out/foodeatup-zones-nettoyage-tuto-v1.mp4"
