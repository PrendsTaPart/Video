#!/usr/bin/env python3
# FoodEatUp "Lire ses Prévisions PrediBot" tutorial.
# No avatar clip, no Claude sequence (no matching mcp__FoodEatUp__* tool -- see
# SCRIPT.md: list_low_stocks/list_production_alerts/list_top_productions/
# get_daily_brief none reproduce PrediBot's AI predictions). Full ElevenLabs VO
# except N6 (CTA), reused verbatim from foodeatup-accueil-role-tuto/vo/N8.mp3
# (byte-identical across the series). Speed = setpts (never zoompan on real
# footage). xfade on every cut, forced back to yuv420p at the end of the chain.
# 48kHz stereo AAC, +faststart.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-predibot-previsions-tuto"
...
# Coordonnées mesurées sur les frames extraites (curseur/main visible au survol
# de chaque carte, work/scan/ -- grille de 3 cartes, 1920x828). Zoom-punch centré
# sur la carte entière (elle EST le bouton cliquable), pas une icône isolée.
BTN_STOCK  = (411, 352); SZ_STOCK  = (500, 250)  # carte "Prédictions de stock"
BTN_ORDERS = (948, 352); SZ_ORDERS = (500, 250)  # carte "Prévisions de commandes"
BTN_PROD   = (1503, 352); SZ_PROD  = (500, 250)  # carte "Production recommandée"

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A", 0.00,  3.65,  7.40, None,       None,      "1 - Tableau de bord PrediBot"),
    ("B", 3.65,  3.90,  0.85, BTN_STOCK,  SZ_STOCK,  None),
    ("C", 3.90, 12.00,  7.20, None,       None,      "2 - Predictions de stock"),
    ("D", 12.00, 13.25, 1.10, BTN_ORDERS, SZ_ORDERS, None),
    ("E", 13.25, 21.50, 7.40, None,       None,      "3 - Previsions de commandes"),
    ("F", 21.50, 22.85, 1.10, BTN_PROD,   SZ_PROD,   None),
    ("G", 22.85, 30.00,  8.70, None,      None,      "4 - Production recommandee"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on the visible dashboard)
             "slideleft",  # B -> C (cut into the opened detail page)
             "fade",       # C -> D (continuous: back to dashboard + click)
             "slideleft",  # D -> E (cut into the opened detail page)
             "fade",       # E -> F (continuous: back to dashboard + click)
             "slideleft"]  # F -> G (cut into the opened detail page)
    trans.append("fade")   # G -> outro
...
anchor = {
    "N0": 0.30,                 # intro hook
    "N1": S["A"] + 0.30,        # indicateurs cles du dashboard
    "N2": S["B"] + 0.10,        # clic Predictions de stock
    "N3": S["D"] + 0.10,        # clic Previsions de commandes
    "N4": S["F"] + 0.10,        # clic Production recommandee
    "N5": S["G"] + 2.00,        # benefice
...
FINAL = f"{ROOT}/out/foodeatup-predibot-previsions-tuto-v1.mp4"
