#!/usr/bin/env python3
# FoodEatUp "Booster la fidelite -- le programme" tutorial (module Marketing,
# Fidelite & Iris, section Fidelite & recompenses).
...
#
# Rush shows: page Fidelite & jeux (KPI : membres, points en circulation,
# points distribues, bons a valider) -> onglet Programme -> 1. Mode de gain
# (essai Hybride/Par euro, reglage final sur "Par passage", 10 pts/passage)
# -> 2. Multiplicateurs jours creux (clic "Ajouter un creneau", jours Mar+Mer,
# 11h00-14h30, multiplicateur x2, libelle "Points doubles" edite en "Points
# doubles midi") -> 3. Regles (validite Illimitee -> 12 mois glissants,
# plafond Aucun -> 200, case "Crediter meme avec un code promo") -> clic
# "Enregistrer le programme" -> toast "Programme enregistre" -> retour
# tableau de bord.
...
ROOT = "/home/user/Video/videos/foodeatup-fidelite-programme-tuto"
...
# 5fps windows around the click to pin the exact moment), native 1920x828.
# No apostrophe in any caption below (bug hit on foodeatup-ingredients-tuto).
BTN_ENREGISTRER = (789, 671); SZ_ENREGISTRER = (305, 58)  # "Enregistrer le programme"
...
# other way round.
segs = [
    ("A", 0.00,  3.50,  9.00, None,            None,           "1 - Fidelite et jeux"),
    ("B", 3.50,  7.00,  7.20, None,            None,           "2 - Mode de gain : par passage"),
    ("C", 7.00,  19.00, 7.90, None,            None,           "3 - Multiplicateur jours creux"),
    ("D", 19.00, 22.20, 5.40, None,            None,           "4 - Validite et plafond"),
    ("E", 22.20, 22.50, 1.00, BTN_ENREGISTRER, SZ_ENREGISTRER, None),
    ("F", 22.50, 28.76, 4.80, None,            None,           "5 - Programme enregistre"),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__Foodeatup__update_loyalty_program
# (establishment_id, earn_mode, visit_points, points_validity_months, active)
# is the matching tool -- covers the mode de gain (par passage), les points
# par passage et la validite montres a l'ecran. Les multiplicateurs jours
# creux et le plafond de points/commande sont des reglages UI sans equivalent
# MCP (schema verifie) -- non mentionnes dans le prompt, conformement a la
# regle du pipeline.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Configure mon programme de fidélité FoodEatUp (ID [ID établissement]) "
                 "en mode par passage : [X] points par passage, points valables "
                 "[Y] mois, programme actif.")
CLAUDE_RESPONSE = "Bien sûr ! Je mets à jour votre programme de fidélité…"
...
# N6 (6.16s) has to span stages 1+2 and N7 (4.62s) has to fit inside stage 3
# alone -- hence both a bit longer than the shared default [2.20,1.30,2.50].
CLAUDE_STAGE_D = [4.30, 2.40, 5.15]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: meme page, scroll vers mode de gain)
             "fade",       # B -> C (continuous: scroll vers multiplicateurs)
             "fade",       # C -> D (continuous: scroll vers regles)
             "fade",       # D -> E (continuous: clic Enregistrer)
             "fade",       # E -> F (continuous: enregistrement -> toast -> dashboard)
             "slideleft",  # F -> claude1 (cut vers la sequence Claude)
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # tableau de bord (membres, points, bons a valider)
    "N2": S["B"] + 0.20,         # mode de gain : par passage, 10 pts
    "N3": S["C"] + 0.20,         # multiplicateur jours creux (mar/mer 11h-14h30)
    "N4": S["D"] + 0.20,         # validite des points + plafond par commande
    "N5": S["E"] + 0.10,         # clic Enregistrer -> toast -> dashboard
...
FINAL = f"{ROOT}/out/foodeatup-fidelite-programme-tuto-v1.mp4"
