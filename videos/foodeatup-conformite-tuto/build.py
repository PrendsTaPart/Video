#!/usr/bin/env python3
# FoodEatUp "Faire son contrôle de conformité" tutorial (checklist hygiène).
# No avatar clip: full ElevenLabs VO throughout (voix Adam FR, TGAegA0zNRi8I6nUdq3i).
# Same engine as the rest of the series: setpts for speed (never zoompan on real
# footage), fixed crop+scale zoom-punch on clicks, xfade on every cut forced
# back to yuv420p, 48kHz stereo AAC, +faststart.
#
# Rush (24.88s) shows: liste "Checklist hygiène" -> clic sur un point de
# contrôle -> modal de validation (zone "Fait à", réponse Oui/Non/Non Évalué)
# -> commentaire + photo -> clic Valider -> toast "Checklist validée avec
# succès !" + item mis à jour. Correspond exactement à l'outil MCP
# create_hygiene_checklist_validation (establishment_id, template_id,
# reponses, zone_controle?, commentaires?, statut?).
...
ROOT = "/home/user/Video/videos/foodeatup-conformite-tuto"
...
# Coordinates eyeballed on extracted frames (1920x828 native). No apostrophe
# in any caption (bug hit on foodeatup-ingredients-tuto -- see
# FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_ITEM    = (1717, 501); SZ_ITEM    = (40, 40)    # chevron, "Les employés portent..." row
BTN_VALIDER = (1024, 759); SZ_VALIDER = (124, 54)   # "Valider" button in modal

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.30,  2.70,  5.50, None, None,        None,        "1 - Ouvrez un point de controle"),
    ("B", 2.70,  3.10,  0.90, 2.80, BTN_ITEM,     SZ_ITEM,     None),
    ("C", 3.10, 10.50,  6.50, None, None,        None,        "2 - Zone et resultat"),
    ("D", 10.50, 19.00, 5.00, None, None,        None,        "3 - Commentaire et photo"),
    ("E", 19.00, 19.50, 0.90, 19.20, BTN_VALIDER, SZ_VALIDER,  None),
    ("F", 20.50, 24.88, 11.50, None, None,        None,        "Controle valide"),
]
INTRO_D, OUTRO_D = 2.80, 6.20
...
# tool: create_hygiene_checklist_validation(establishment_id, template_id,
# reponses, zone_controle?, commentaires?, statut?) -- schema checked, matches
# exactly what's shown (zone + réponse oui/non/non évalué + commentaire).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Valide le point de contrôle [nom du point de contrôle] (modèle ID "
                  "[ID du modèle]) pour la zone [zone contrôlée], réponse "
                  "[oui/non/non évalué], commentaire [commentaire], pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! J'enregistre cette validation de checklist…"
...
CLAUDE_STAGE_D = [6.50, 3.00, 6.50]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on the visible row)
             "slideleft",  # B -> C (cut: modal opens)
             "fade",       # C -> D (continuous within the same modal)
             "fade",       # D -> E (continuous: click Valider)
             "slideleft",  # E -> F (cut: submit -> success toast)
             "slideleft",  # F -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur le point à contrôler"
    "N2": S["C"] + 0.20,         # zone + réponse Oui/Non/Non Évalué
    "N3": S["D"] + 0.20,         # commentaire + photo
    "N4": S["E"] + 0.10,         # clic Valider -> enregistré
    "N5": S["F"] + 3.5,          # bénéfice (traçabilité) -- après N4 sur le même écran succès
...
FINAL = f"{ROOT}/out/foodeatup-conformite-tuto-v1.mp4"
