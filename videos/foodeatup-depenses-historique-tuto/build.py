#!/usr/bin/env python3
# FoodEatUp "Classer ses factures dans les dépenses" tutorial (module
# Comptabilite & Achats). Same engine as every FoodEatUp tuto (see
...
#
# Rush is a real screen recording (1920x828 @25fps) of the module
# Comptabilite > Depenses : liste des depenses (stats Total payees/En
# attente/Total devis) -> clic "Creer une depense" -> formulaire (date,
# reference facture fournisseur, fournisseur "carrefour") -> zone "Joindre
# la facture", upload du PDF (Facture_FAC-2026-00006.pdf) -> recherche et
# ajout d'un article ("Gateau patate", qte 10, 6,50 EUR) + categorie
# "Desserts" -> statut "Payee" -> Enregistrer -> retour a la liste
# (nouvelle ligne #D20260003) -> menu Action -> Visualiser -> fiche detail
# montrant la facture jointe, telechargeable (l'historique de la depense).
# Pas de sequence "Utiliser avec Claude" : create_expense (MCP) ne gere pas
# l'upload de fichier, qui est l'action centrale de cette video -- regle du
# workflow, pas de prompt invente.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-depenses-historique-tuto"
...
segs = [
    ("A",  0.30,  2.60, 6.3, "1 - Vos depenses en un coup doeil"),
    ("B",  2.90,  3.30, 1.0, None),
    ("C",  5.00,  8.80, 6.5, "2 - Date, reference, fournisseur"),
    ("D",  9.60, 14.20, 6.2, "3 - Joindre la facture"),
    ("E", 18.00, 24.00, 6.4, "4 - Articles et categorie"),
    ("F", 26.00, 30.30, 7.15,"5 - Statut et enregistrement"),
    ("G1",32.00, 36.50, 2.0, None),
    ("G2",39.50, 46.50, 6.3, "6 - Historique de la depense"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: about to click Creer une depense)
             "slideleft",  # B -> C (cut: liste -> formulaire)
             "fade",       # C -> D (continuous: meme formulaire)
             "fade",       # D -> E (continuous: meme formulaire, scroll)
             "fade",       # E -> F (continuous: meme formulaire)
             "slideleft",  # F -> G1 (cut: formulaire enregistre -> liste)
             "slideleft",  # G1 -> G2 (cut: liste -> fiche detail)
             "fade"]       # G2 -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # liste des depenses
    "N2": S["C"] + 0.20,         # date, reference, fournisseur
    "N3": S["D"] + 0.20,         # joindre la facture
    "N4": S["E"] + 0.20,         # articles et categorie
    "N5": S["F"] + 0.20,         # statut et enregistrement
    "N6": S["G2"] + 0.20,        # historique de la depense
    "N8": OUTRO_START + 0.35,    # CTA
}
keys = ["N0","N1","N2","N3","N4","N5","N6","N8"]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-depenses-historique-tuto-v1.mp4"
