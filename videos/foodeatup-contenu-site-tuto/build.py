#!/usr/bin/env python3
# FoodEatUp "Ajouter du contenu sur son site" tutorial (module site-web-vitrine,
# tutoriel 07/8). Rush: 5 sous-onglets de "Contenu du site" -- Recrutement,
# Producteurs, Boissons, Privatisations (vue), Agenda.
#
# Same engine as the rest of the series: setpts for speed (never zoompan on real
# footage), fixed crop+scale zoom-punch on clicks, xfade on every cut forced back
# to yuv420p, 48kHz stereo AAC, +faststart. No avatar clip for this one -- straight
# from intro card into the screen recording.
...
ROOT = "/home/user/Video/videos/foodeatup-contenu-site-tuto"
...
# Coordinates measured by direct frame read (work/detail/jo_*.jpg, ag2_*.jpg) --
# plain blue button on white background, no colour-thresholding needed this time.
BTN_SAVE1 = (496, 644); SZ_SAVE1 = (145, 49)  # "Enregistrer" -- offre d'emploi
BTN_SAVE2 = (496, 712); SZ_SAVE2 = (145, 50)  # "Enregistrer" -- evenement agenda

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,   29.30, 4.60, None,   None,      None,       "1 · Recrutement : nouvelle offre"),
    ("B", 30.00,  30.35, 0.80, 30.00,  BTN_SAVE1, SZ_SAVE1,   None),
    ("C", 30.35,  35.00, 2.00, None,   None,      None,       "Offre publiée !"),
    ("D", 35.00,  52.50, 4.60, None,   None,      None,       "2 · Ajoutez vos producteurs"),
    ("E", 52.50,  57.00, 1.20, None,   None,      None,       None),
    ("F", 57.00,  89.50, 5.60, None,   None,      None,       "3 · Composez votre carte des boissons"),
    ("G", 89.50,  95.00, 5.80, None,   None,      None,       "4 · Privatisations & cartes cadeaux"),
    ("H", 95.00,  127.30, 4.60, None,  None,      None,       "5 · Publiez vos événements"),
    ("I", 127.30, 127.70, 0.80, 127.30, BTN_SAVE2, SZ_SAVE2,  None),
    ("J", 127.70, 132.84, 2.20, None,  None,      None,       "Événement publié !"),
]
INTRO_D, OUTRO_D = 4.80, 6.20
...
# tool: create_job_offer(establishment_id, title, contract, description,
# salary_min, salary_max, status) -- the Recrutement sub-tab is the first one
# shown and the theme named in the source filename Michael provided.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une offre d'emploi pour mon établissement FoodEatUp "
                  "(ID [ID établissement]) : poste [intitulé du poste], "
                  "contrat [type de contrat], salaire de [salaire min]€ à "
                  "[salaire max]€, avec la description : [description du poste].")
CLAUDE_RESPONSE = "Bien sûr ! Je publie votre offre d'emploi…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on the visible button)
             "fade",       # B -> C (submit -> confirmation)
             "slideleft",  # C -> D (cut into Producteurs tab)
             "fade",       # D -> E (continuous: save + confirm)
             "slideleft",  # E -> F (cut into Boissons tab)
             "slideleft",  # F -> G (cut into Privatisations tab)
             "slideleft",  # G -> H (cut into Agenda tab)
             "fade",       # H -> I (continuous: click on the visible button)
             "fade",       # I -> J (submit -> confirmation)
             "slideleft",  # J -> claude1
...
GAP = 0.22
anchor = {
    "N0": S["intro"] + 0.15,     # accroche, over the intro card
    "N1": S["A"] + 0.20,         # Recrutement: fill + save + confirm
    "N2": S["D"] + 0.20,         # Producteurs
    "N3": S["F"] + 0.20,         # Boissons
    "N4": S["G"] + 0.20,         # Privatisations
    "N5": S["H"] + 0.20,         # Agenda: fill + save + confirm
...
FINAL = f"{ROOT}/out/foodeatup-contenu-site-tuto-v1.mp4"
