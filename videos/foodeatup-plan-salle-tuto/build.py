#!/usr/bin/env python3
# FoodEatUp "Dessiner son plan de salle / QR code à table" tutorial.
...
#
# Rush shows an existing floor plan (8 tables, 2 zones) -- no zone/table
# creation UI on screen. It focuses on: selecting a table, generating its QR
# code, copying the link, and the resulting customer-facing order page
# ("Gosushi démo"). A McAfee download-scan popup flashes ~16.3-19.5s and a
# stray reload blank flashes ~27.3-27.7s -- both skipped entirely rather than
# cropped (this is an edited/sped-up tutorial, not a continuous capture; the
# source already jumps QR-modal -> toast -> plain-QR-again -> new tab, so
# skipping the redundant middle to cut straight to the clean new-tab shot is
# consistent with how every other video in the series compresses dead time).
...
ROOT = "/home/user/Video/videos/foodeatup-plan-salle-tuto"
...
# Coordinates measured by colour-thresholding the actual frames (PIL bbox on
# full-res PNG exports), not eyeballed:
#  - T3 table card: pink bbox (650,376)-(765,480) -> center (707,428)
#  - "QR code de la table" button: blue bbox (1398,478)-(1739,533) -> (1568,506)
#  - "Copier le lien" button: read off a gridded crop -> (1085,675)
# No apostrophe in any caption below (bug hit on foodeatup-ingredients-tuto --
# see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_T3   = (707, 428);   SZ_T3   = (115, 104)  # table T3 card
BTN_QR   = (1568, 506);  SZ_QR   = (341, 55)   # "QR code de la table"
BTN_COPY = (1085, 675);  SZ_COPY = (270, 50)   # "Copier le lien"

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A", 0.30,  4.00,  3.90, None,     None,    "1 . Votre plan de salle"),
    ("B", 4.30,  4.65,  0.90, BTN_T3,   SZ_T3,   None),
    ("C", 5.00,  8.00,  3.70, None,     None,    "2 . Table selectionnee"),
    ("D", 8.30,  8.65,  0.90, BTN_QR,   SZ_QR,   None),
    ("E", 11.50, 15.00, 4.30, None,     None,    "3 . QR code genere"),
    ("F", 19.55, 19.90, 0.90, BTN_COPY, SZ_COPY, None),
    ("G", 20.00, 21.00, 1.30, None,     None,    None),
    ("H", 23.60, 26.50, 3.00, None,     None,    "Vos clients scannent"),
    ("I", 28.00, 31.50, 5.70, None,     None,    "Ils commandent et payent"),
]
...
# (videos/_shared/claude_prompt_sequence.py). The tutorial's topic (building
# the floor plan) maps to two MCP tools used together, in the order they'd be
# called: mcp__FoodEatUp__create_zone(establishment_id, name, color?) then
# mcp__FoodEatUp__create_table(establishment_id, name, capacity?, zone_id?,
# shape?) -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Cree une zone [nom de la zone, ex. Terrasse] dans mon plan "
                  "de salle, puis ajoute une table [nom de la table, ex. T3] "
                  "de [nombre] couverts dans cette zone, pour mon "
                  "etablissement FoodEatUp (ID [ID etablissement]).")
CLAUDE_RESPONSE = "Bien sur ! Je cree votre zone et votre table…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # B -> C (cut into the table detail panel)
             "fade",       # C -> D (continuous: click QR code de la table)
             "slideleft",  # D -> E (cut into the QR modal reveal)
             "fade",       # E -> F (continuous: click Copier le lien)
             "fade",       # F -> G (continuous: toast appears)
             "slideleft",  # G -> H (cut to the new browser tab)
             "slideleft",  # H -> I (cut to the Gosushi customer page)
             "slideleft",  # I -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.10,         # plan de salle, zones
    "N2": S["C"] + 0.20,         # table selectionnee, statut + commande
    "N3": S["E"] + 0.20,         # QR code genere
    "N4": S["F"] + 0.10,         # copier le lien -> clients scannent / arrivent
    "N5": S["I"] + 0.20,         # ils commandent et payent (benefice)
...
FINAL = f"{ROOT}/out/foodeatup-plan-salle-tuto-v1.mp4"
