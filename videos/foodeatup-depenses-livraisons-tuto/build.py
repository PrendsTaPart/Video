#!/usr/bin/env python3
# FoodEatUp "Archiver ses dépenses en les reliant à ses livraisons" tutorial.
# Same engine as foodeatup-historique-reception-tuto/build.py: no avatar clip,
# full ElevenLabs VO throughout, setpts speed (never zoompan on real footage),
...
ROOT = "/home/user/Video/videos/foodeatup-depenses-livraisons-tuto"
...
BTN_VOIR      = (949, 608);  SZ_VOIR      = (300, 54)   # "Voir le détail" (2e carte livraison)
BTN_AJOUTER   = (1586, 527); SZ_AJOUTER   = (264, 56)   # "+ Ajouter une facture"
BTN_CHOISIR   = (960, 615);  SZ_CHOISIR   = (226, 52)   # "Choisir un fichier" (modal import)
BTN_VALIDER   = (1538, 396); SZ_VALIDER   = (360, 56)   # "Valider et enregistrer"
BTN_VOIRDEP   = (830, 578);  SZ_VOIRDEP   = (240, 52)   # "Voir la dépense" (modal confirmation)
BTN_MENU      = (146, 137);  SZ_MENU      = (60, 60)    # icône hamburger
BTN_COMPTA    = (140, 308);  SZ_COMPTA    = (280, 50)   # "Comptabilité" (menu latéral)
BTN_DEPENSES  = (160, 548);  SZ_DEPENSES  = (280, 50)   # "Dépenses" (sous-menu Comptabilité)
...
segs = [
    ("A", 0.00,  2.60,  2.60, None, None,         None,         "Gestion des livraisons"),
    ("B", 2.60,  2.90,  0.90, 2.70, BTN_VOIR,      SZ_VOIR,      None),
    ("C", 2.90,  4.85,  2.20, None, None,         None,         "Livraison reçue"),
    ("D", 4.85,  5.05,  0.90, 4.90, BTN_AJOUTER,   SZ_AJOUTER,   None),
    ("E", 5.05,  8.90,  2.00, None, None,         None,         "Importer une facture"),
    ("F", 8.90,  9.50,  0.90, 9.00, BTN_CHOISIR,   SZ_CHOISIR,   None),
    ("G", 9.50, 14.60,  3.00, None, None,         None,         None),
    ("H", 14.60, 16.60, 3.20, None, None,         None,         "Produits détectés"),
    ("I", 16.60, 17.10, 0.90, 16.70, BTN_VALIDER,  SZ_VALIDER,   None),
    ("J", 17.10, 20.60, 2.20, None, None,         None,         None),
    ("K", 20.60, 21.10, 0.90, 20.70, BTN_VOIRDEP,  SZ_VOIRDEP,   None),
    ("L", 21.10, 25.60, 1.80, None, None,         None,         "Dépense enregistrée"),
    ("M", 25.60, 26.10, 0.90, 25.70, BTN_MENU,     SZ_MENU,      None),
    ("N", 26.10, 27.10, 0.90, None, None,         None,         None),
    ("O", 27.10, 27.60, 0.90, 27.20, BTN_COMPTA,   SZ_COMPTA,    None),
    ("P", 27.60, 29.60, 0.90, None, None,         None,         None),
    ("Q", 29.60, 30.10, 0.90, 29.70, BTN_DEPENSES, SZ_DEPENSES,  None),
    ("R", 30.10, 35.72, 2.00, None, None,         None,         "Historique des dépenses"),
]
INTRO_D, OUTRO_D = 4.90, 5.30
...
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Enregistre une dépense pour mon établissement FoodEatUp (ID [ID établissement]) : "
                  "fournisseur [nom du fournisseur], produit [désignation] x[quantité] à "
                  "[prix unitaire] € HT, référence facture [référence facture fournisseur].")
CLAUDE_RESPONSE = "Bien sûr ! J'enregistre cette dépense pour votre établissement…"
...
# N7 over stage3), measured N6=5.38s / N7=4.21s, stages sized to give each
# room (measure-VO-before-segments rule, see FOODEATUP-TUTORIELS-WORKFLOW.md).
CLAUDE_STAGE_D = [3.20, 2.60, 4.40]  # reveal, copied, chatbot mockup
...
FINAL = f"{ROOT}/out/foodeatup-depenses-livraisons-tuto-v1.mp4"
