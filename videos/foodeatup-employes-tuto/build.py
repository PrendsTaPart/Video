#!/usr/bin/env python3
# FoodEatUp "Ajouter ses employés" tutorial (module Équipe & Planning).
# Same engine as every FoodEatUp tuto (see videos/FOODEATUP-TUTORIELS-WORKFLOW.md):
# setpts speed change (never zoompan on real footage), zoom-punch crop on the two
# form-submit clicks + the "Voir" click, xfade on every cut, forced back to
# yuv420p at the end of the chain. 48kHz stereo AAC, +faststart.
#
# Rush: liste employés vide -> "Ajouter un employé" -> formulaire 2 étapes
# (Informations générales : prénom/nom/email/téléphone/rôle, puis Horaires de
# travail : jour/heure début/heure fin/repos) -> "Ajouter" -> carte employé
# créée (Alice Charbit, Chef, Permission HACCP) -> "Voir" -> fiche complète
# (coordonnées, rôle, horaires). matches mcp__FoodEatUp__create_employee
# (establishment_id, first_name, last_name, email, phone_number, role?, schedule?).
...
ROOT = "/home/user/Video/videos/foodeatup-employes-tuto"
...
BTN_ADD_EMP = (1695, 344); SZ_ADD_EMP = (250, 62)  # "Ajouter un employé" (list)
BTN_SUIVANT = (1202, 759); SZ_SUIVANT  = (142, 62)  # "Suivant" (step 1 -> 2)
BTN_ADD2    = (1205, 674); SZ_ADD2     = (142, 62)  # "Ajouter" (step 2 submit)
BTN_VOIR    = (389, 682);  SZ_VOIR     = (170, 54)  # "Voir" (carte employe)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.60,  2.60, None,  None,        None,     "1 · Employés"),
    ("B", 2.60,  2.90,  0.90, 2.75,  BTN_ADD_EMP, SZ_ADD_EMP, None),
    ("C", 3.00,  26.90, 6.00, None,  None,        None,     "2 · Informations Générales"),
    ("D", 27.00, 27.30, 0.90, 27.10, BTN_SUIVANT, SZ_SUIVANT, None),
    ("E", 27.40, 41.00, 6.00, None,  None,        None,     "3 · Horaires de travail"),
    ("F", 41.10, 41.40, 0.90, 41.20, BTN_ADD2,    SZ_ADD2,   None),
    ("G", 41.50, 45.40, 3.00, None,  None,        None,     "Employé ajouté"),
    ("H", 45.40, 45.70, 0.90, 45.55, BTN_VOIR,    SZ_VOIR,   None),
    ("I", 45.80, 49.20, 4.00, None,  None,        None,     "4 · Fiche complète"),
]
...
# schedule?) is the matching tool -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée l'employé [prénom] [nom], email [email], téléphone "
                  "[téléphone], rôle [rôle], pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! J'ajoute cet employé à votre équipe…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C -> D (continuous: click on Suivant)
             "slideleft",  # D -> E (cut to the horaires screen)
             "fade",       # E -> F (continuous: click on Ajouter)
             "fade",       # F -> G (submit -> success/list)
             "fade",       # G -> H (continuous: click on Voir)
             "slideleft",  # H -> I (cut to the detail panel)
             "slideleft",  # I -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Ajouter un employe"
    "N2": S["C"] + 0.20,         # informations generales
    "N3": S["E"] + 0.20,         # horaires de travail
    "N4": S["F"] + 0.10,         # "cliquez sur Ajouter" -> succes
    "N5": S["I"] + 0.20,         # benefice (fiche complete, "Voir")
...
FINAL = f"{ROOT}/out/foodeatup-employes-tuto-v1.mp4"
