#!/usr/bin/env python3
# FoodEatUp "Consulter ses productions en cours" (HACCP) tutorial.
...
#
# Rush shows the "Production haccp" list (1 plat, "Uuuu"), then Editer (modale
# ingrédients/date/heure/durée de vie + nom/catégorie/prix/TVA), then closes
# and opens the separate "Ingrédients" modale (stock/DLC/n° de lot, alerte
# manquant, "Tout ajouter aux courses", "Créer étiquette HACCP") -- this last
# modale is the one that matches the video's own title card and get_production
# _ingredients. The stock modale is static after ~16.5s (no further clicks
# visible before it closes near la fin du rush) -- slowed down rather than cut,
# a static screen played slower reads exactly the same as at speed.
...
ROOT = "/home/user/Video/videos/foodeatup-productions-tuto"
...
# Coordinates read off the rush frames (t=5.5-6.5s and t=16.0-16.5s), source
# space 1920x828. No apostrophe in any caption below (bug hit on
# foodeatup-ingredients-tuto -- see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_EDIT = (241, 587); SZ_EDIT = (240, 55)  # "Éditer"
BTN_ING  = (506, 587); SZ_ING  = (240, 55)  # "Ingrédients"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  6.00,  6.00, None, None,     None,    "1 · Vos productions en cours"),
    ("B", 6.00,  6.35,  0.90, 6.20, BTN_EDIT, SZ_EDIT, None),
    ("C", 6.35,  16.00, 6.00, None, None,     None,    "2 · Modifier ingrédients, date, prix"),
    ("E", 16.00, 16.35, 0.90, 16.20, BTN_ING, SZ_ING,  None),
    ("F", 16.50, 24.80, 16.00, None, None,    None,    "3 · Stock, DLC et traçabilité HACCP"),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__list_production_plans
# (establishment_id, start_date?, end_date?, status?) matches the video's own
# title ("consulter ses productions en cours").
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Affiche mes productions en cours pour mon établissement "
                  "FoodEatUp (ID [ID établissement]), du [date début] au [date fin].")
CLAUDE_RESPONSE = "Bien sûr ! Voici vos productions en cours…"
...
# Sized generously: N5's benefit line and N6's Claude explanation both drift a
# bit past their own click/segment -- same accepted pattern as
# foodeatup-produits-tuto (see its SCRIPT.md / build.py comment). Stage1
# enlarged further here (vs. produits' 6.00) because F itself already carries
# 3 VO lines (N3+N4+N5, ~20s) against a 16s target.
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # C -> E (close edit, click Ingrédients)
             "fade",       # E -> F (continuous: modal opens)
             "slideleft",  # F -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # liste des plats en préparation
    "N2": S["B"] + 0.10,         # "cliquez sur Éditer" -> ingrédients/date/prix
    "N3": S["E"] + 0.10,         # "cliquez sur Ingrédients" -> stock/DLC/lot
    "N4": S["F"] + 0.20,         # alerte manquant + courses/étiquette HACCP
    "N5": S["F"] + 9.00,         # bénéfice (traçabilité temps réel)
...
keys = [f"N{i}" for i in [0,1,2,3,4,5,6,7,8]]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-productions-tuto-v1.mp4"
