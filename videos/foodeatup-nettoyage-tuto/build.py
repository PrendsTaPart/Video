#!/usr/bin/env python3
# FoodEatUp "Ajouter et paramétrer son plan de nettoyage (Zones)" tutorial.
...
BTN_ADD_ZONE = (1591, 43);  SZ_ADD_ZONE = (170, 46)   # "Ajouter une zone de nettoyage" (header)
BTN_SAVE     = (1024, 658); SZ_SAVE     = (136, 56)   # "Enregistrer" (modal submit)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
# Timestamps measured by frame extraction (ffmpeg select+r=4 around each
# click window) on the 57.9s rush. Click coordinates read directly off the
# native 1920x828 frames (button centers).
segs = [
    ("A", 0.30, 9.00, 4.60, None, None,          None,        "1 · Plan de nettoyage"),
    ("B", 9.00, 9.30, 0.90, 9.10, BTN_ADD_ZONE,   SZ_ADD_ZONE, None),
    ("C", 9.60, 24.90, 5.20, None, None,         None,        "2 · Nom et description"),
    ("D", 24.90, 25.20, 0.90, 25.00, BTN_SAVE,    SZ_SAVE,     None),
    ("E", 25.30, 29.00, 3.80, None, None,         None,        "Zone créée"),
    ("F", 29.00, 57.92, 5.90, None, None,         None,        "3 · Marquer une action"),
]
INTRO_D, OUTRO_D = 4.75, 6.20
...
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée la zone de nettoyage [nom de la zone] avec les postes "
                  "[poste 1, poste 2, ...] pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cette zone de nettoyage pour votre établissement…"
...
# Same 3-stage timing as the rest of the series (reveal / copied / chatbot
# mockup) -- N6=4.41s / N7=4.41s measured first (see SCRIPT.md), stages sized
# to give each line room (measure-VO-before-segments rule).
