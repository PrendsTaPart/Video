#!/usr/bin/env python3
# FoodEatUp "Configurer ses unites de mesure" tutorial.
...
BTN_ADD    = (1669, 368)   # "+ Ajouter une unite"
BTN_CREATE = (1024, 750)   # "Creer l'unite"

# (name, src_start, src_end, factor, click_src_time_or_None, button)
# Factors sized so each segment's ON-SCREEN duration roughly matches the VO
# line(s) that narrate it -- avoids both dead air and an oversized outro hold.
segs = [
    ("A", 0.0,  1.6,  0.44, 1.1,   BTN_ADD),     # list + click Ajouter (held)
    ("B", 1.6,  4.0,  2.00, None,  None),        # modal opens (quick bridge)
    ("C", 4.0,  20.0, 1.95, None,  None),        # form fill: nom/abrev/facteur/base
    ("D", 20.0, 24.0, 0.68, 20.4,  BTN_CREATE),  # click Creer + toast (held zoom)
    ("E", 24.0, 33.48, 1.62, None, None),        # list confirms new unit
]
...
run(["ffmpeg","-y","-i",f"{ROOT}/work/video_silent.mp4"] + inputs +
    ["-filter_complex", ";".join(filters), "-map","0:v","-map","[voa]",
     ...
     f"{ROOT}/out/foodeatup-unites-tuto-v1.mp4"])
