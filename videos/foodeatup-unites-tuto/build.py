#!/usr/bin/env python3
# FoodEatUp "Régler ses unités de mesure" tutorial.
...
# exposes list_units (read-only, global referential) -- no create_unit/
# add_unit tool exists, so there's nothing to build a real prompt around.
# Per the standing rule (FOODEATUP-TUTORIELS-WORKFLOW.md), skip the
# sequence rather than invent a prompt.
#
# This project previously had a v1 committed to this branch (2026-08-02,
# earlier session) that predates the compatibility fixes: plain concat
# instead of xfade, no per-line loudnorm before mixing, alimiter without
# level=disabled, -shortest instead of -t, no banners/punch highlight, and
# it used an outdated intro.jpg (different title wording, different photo
# treatment) that Michael has since replaced. Rebuilt from scratch on the
# current pipeline with the new assets.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-unites-tuto"
...
BTN_ADD    = (1619, 367); SZ_ADD    = (346, 58)  # "+ Ajouter une unité"
BTN_CREATE = (1024, 748); SZ_CREATE = (173, 60)  # "Créer l'unité"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.60,  2.90, None, None,       None,       "1 · Mes unités"),
    ("B", 2.60,  2.90,  0.90, 2.90, BTN_ADD,     SZ_ADD,     None),
    ("C", 4.00,  20.00, 9.00, None, None,       None,       "2 · Nom, abréviation, facteur"),
    ("D", 20.00, 20.30, 0.90, 20.30, BTN_CREATE, SZ_CREATE,  None),
    ("E", 20.30, 33.28, 4.00, None, None,       None,       "Unité créée"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C -> D (continuous: click Créer l'unité)
             "fade"]       # D -> E (submit -> toast/list) -> outro handled below
    # trans has one entry per boundary among segs (4) + E->outro (fade)
    trans.append("fade")
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Ajouter une unité"
    "N2": S["C"] + 0.20,         # nom/abréviation/facteur/base
    "N3": S["D"] + 0.10,         # "cliquez sur Créer l'unité" -> succès
    "N4": S["E"] + 0.20,         # bénéfice : sert aux ingrédients/recettes
...
FINAL = f"{ROOT}/out/foodeatup-unites-tuto-v1.mp4"
