#!/usr/bin/env python3
# FoodEatUp "Lire ses statistiques par module" tutorial.
# Pure navigation/reading tour of the Analytix BI dashboard -> 6 module pages
# (Finances, Stocks, RH & Pointage, HACCP, Production, Assistant IA). No
# create/submit action in the rush, so no zoom-punch on a button -- straight
# fade cuts between screens (each is a real page navigation). Speed =
# setpts (never zoompan on real footage). 48kHz stereo AAC, +faststart.
...
ROOT = "/home/user/Video/videos/foodeatup-statistiques-module-tuto"
...
# No apostrophe in any caption (bug hit on foodeatup-ingredients-tuto).
segs = [
    ("A", 0.50,  9.80,  6.80, "1 . Tableau de bord Analytix BI"),
    ("B", 11.50, 19.00, 5.40, "2 . Finances"),
    ("C", 19.50, 27.00, 4.00, "3 . Stocks"),
    ("D", 27.30, 32.00, 5.10, "4 . RH et Pointage"),
    ("E", 32.30, 37.80, 3.20, "5 . HACCP"),
    ("F", 38.00, 42.50, 2.60, "6 . Production"),
    ("G", 43.00, 50.60, 4.60, "7 . Assistant IA"),
]
INTRO_D, OUTRO_D = 3.80, 6.20
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__finance_summary
# (establishment_id, date_from?, date_to?) is the matching read-only tool --
# it mirrors the Finances screen shown in the rush.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Donne-moi la synthèse financière de mon établissement FoodEatUp "
                  "(ID [ID établissement]) du [date début] au [date fin].")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère votre synthèse financière…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (dashboard -> Finances, page nav)
             "fade",       # B -> C (Finances -> Stocks)
             "fade",       # C -> D (Stocks -> RH & Pointage)
             "fade",       # D -> E (RH -> HACCP)
             "fade",       # E -> F (HACCP -> Production)
             "fade",       # F -> G (Production -> Assistant IA)
             "slideleft",  # G -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # tableau de bord
    "N2": S["B"] + 0.20,         # finances
    "N3": S["C"] + 0.20,         # stocks
    "N4": S["D"] + 0.20,         # RH & pointage
    "N5": S["E"] + 0.20,         # HACCP + production
    "N6": S["G"] + 0.20,         # assistant IA (bénéfice)
...
FINAL = f"{ROOT}/out/foodeatup-statistiques-module-tuto-v1.mp4"
