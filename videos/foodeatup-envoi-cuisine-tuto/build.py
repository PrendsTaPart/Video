#!/usr/bin/env python3
# FoodEatUp "Envoyer une commande en cuisine" tutorial (module service-commande,
# sujet "Envoyer en Cuisine en direct"). Same engine as the rest of the series:
# no avatar clip, full ElevenLabs VO, speed = setpts (never zoompan on real
# footage), xfade on every cut, forced back to yuv420p at the end of the chain.
# 48kHz stereo AAC, +faststart. Banner = two drawtext layers (box=1), NOT
# drawbox -- this ffmpeg (6.1.1) never evaluates drawbox's x/y/w/h when they
# depend on t (see FOODEATUP-TUTORIELS-WORKFLOW.md, "Pièges déjà rencontrés").
...
ROOT = "/home/user/Video/videos/foodeatup-envoi-cuisine-tuto"
...
# Step banner -- two drawtext layers sharing one slide-in x expression (see
# workflow doc): drawbox never evaluates a t-dependent x on this ffmpeg build,
# so the plate has to be drawtext's own `box`, not a separate drawbox.
BANNER_Y = H - 108
...
# not eyeballed. No apostrophe in any caption below (bug hit on
# foodeatup-ingredients-tuto -- see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_CONFIRMER = (253, 637); SZ_CONFIRMER = (252, 63)   # "Confirmer" (CMD-2026-00100)
BTN_PRETE     = (262, 670); SZ_PRETE     = (273, 62)   # "Prete" (CMD-2026-00101)

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
# Targets sized on the measured VO durations (N0-N5, see vo/*.mp3), not the
# other way round -- rule from FOODEATUP-TUTORIELS-WORKFLOW.md.
segs = [
    ("A", 0.20,  4.00,  6.46, None,          None,       "1 · Vos commandes du jour"),
    ("B", 4.10,  4.35,  0.95, BTN_CONFIRMER, SZ_CONFIRMER, None),
    ("C", 4.60,  7.30,  7.70, None,          None,       None),
    ("D", 7.40,  14.20, 5.35, None,          None,       "2 · Réception en cuisine (KDS)"),
    ("E", 14.40, 17.60, 6.50, None,          None,       "3 · Passage au statut Prête"),
    ("F", 18.50, 18.75, 0.95, BTN_PRETE,     SZ_PRETE,   None),
    ("G", 19.00, 28.40, 4.75, None,          None,       None),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__update_order_status
# (establishment_id, order_id, status="confirmee") is the matching tool -- it's
# exactly the "Confirmer -> envoyee en cuisine" action shown in the rush.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Confirme la commande [numéro de commande] pour mon "
                  "établissement FoodEatUp (ID [ID établissement]) afin de "
                  "l'envoyer en cuisine.")
CLAUDE_RESPONSE = "Bien sûr ! Je confirme cette commande et je l'envoie en cuisine…"
...
# N6 (4.96s) has to span stages 1+2, N7 (4.49s) has to fit inside stage 3
# alone -- both longer than the shared default [2.20,1.30,2.50].
CLAUDE_STAGE_D = [3.20, 2.30, 5.10]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # B -> C (continuous: toast appears)
             "slideleft",  # C -> D (cut into the KDS screen)
             "slideleft",  # D -> E (cut back to Commandes)
             "fade",       # E -> F (continuous: click on Prete)
             "fade",       # F -> G (continuous: toast + filters update)
             "slideleft",  # G -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # vue Commandes du jour
    "N2": S["B"] + 0.20,         # clic Confirmer -> punch
    "N3": S["C"] + 0.20,         # toast "envoyee en cuisine"
    "N4": S["D"] + 0.20,         # ecran KDS
    "N5": S["E"] + 0.20,         # suivi jusqu'a Prete (+ benefice)
...
FINAL = f"{ROOT}/out/foodeatup-envoi-cuisine-tuto-v1.mp4"
