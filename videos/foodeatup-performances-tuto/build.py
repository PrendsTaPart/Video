#!/usr/bin/env python3
# FoodEatUp "Suivre ses performances (cote employe)" tutorial (module Equipe
# & Planning, catalogue #18). Same engine as every FoodEatUp tuto (see
...
# "Mon planning" page -> browser chrome (tabs/url bar/bookmarks) occupies the
# top 192px and is cropped out, giving a clean 1920x828 canvas. Flow: Mon
# planning (brief) -> avatar menu (Mes modules/Dashboard/Planning/Conges/
# Performances/Profil/Deconnexion) -> clic Performances -> tableau de bord :
# score global en anneau (131, "A ameliorer") avec legende Excellent/Moyen/A
# ameliorer -> Classement de l'equipe (3 employes, score + presence) + Rang
# 1/15 + Moyenne d'equipe -> Historique des performances (barres Semaine
# 1-5). Aucun outil mcp__FoodEatUp__* ne calcule/n'expose ce score de
# performance employe (metrique interne au produit, pas un endpoint API) ->
# pas de sequence "Utiliser avec Claude" (meme raisonnement que les autres
# tutos cote employe de cette serie).
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-performances-tuto"
...
BTN_PERF = (1569, 420); SZ_PERF = (180, 28)  # "Performances" (menu avatar)

# (name, src_start, src_end, target_out_duration, click_time, button, btn_size, caption)
segs = [
    ("A", 0.20,  1.50,  4.20, None, None,     None,    "1 · Mon planning"),
    ("B", 3.60,  3.90,  0.90, 3.75, BTN_PERF, SZ_PERF, None),
    ("C", 6.00,  7.50,  7.70, None, None,     None,    "2 · Score de performance"),
    ("D", 8.00,  10.00, 6.10, None, None,     None,    "Classement de l equipe"),
    ("E", 11.00, 14.00, 4.50, None, None,     None,    "Historique des performances"),
    ("F", 16.00, 18.40, 3.00, None, None,     None,    None),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click Performances)
             "slideleft",  # B -> C (cut, performances page loaded)
             "fade",       # C -> D (continuous: scroll to classement)
             "fade",       # D -> E (continuous: scroll to historique)
             "fade",       # E -> F (continuous: scroll back to top)
             "fade"]       # F -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # depuis le menu, acces aux performances
    "N2": S["C"] + 0.20,         # score global + indicateur
    "N3": S["D"] + 0.20,         # classement de l'equipe
    "N4": S["E"] + 0.20,         # historique semaine par semaine
...
FINAL = f"{ROOT}/out/foodeatup-performances-tuto-v1.mp4"
