#!/usr/bin/env python3
# FoodEatUp "Imprimer ses etiquettes (vente / stockage)" tutorial (module
# Hygiene & HACCP). Same engine as every FoodEatUp tuto (see
# videos/FOODEATUP-TUTORIELS-WORKFLOW.md): setpts speed change (never zoompan
# on real footage), zoom-punch crop on button clicks, xfade on every cut,
# forced back to yuv420p at the end of the chain. 48kHz stereo AAC, faststart.
#
# Rush (46.6s): module Etiqueteuse -> selection d'un produit (Pizza
# margaritta) -> "Imprimer 1 etiquette(s)" -> modale Etiqueteuse (date, type,
# produit pre-rempli) -> DLC / numero de lot / code-barres de vente calcules
# automatiquement -> NB etiquettes (1->3) + equipement de stockage (frigo) ->
# "Creer les etiquettes" -> "3 etiquette(s) creee(s) avec succes !" -> apercu
# des 3 etiquettes (lot, DLC, stockage) prete pour impression -> "Valider ->
# Historique" -> "Etiquettes validees ! ... ajoutees a l'historique" -> retour
# a la liste, selection remise a zero.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-etiquettes-tuto"
...
CLAUDE_PROMPT = (
    "Cree une etiquette HACCP pour [quantite] [nom du produit], DLC le "
    "[date], stockee au [equipement de stockage], pour mon etablissement "
...
BTN_CHECKBOX   = (1324, 187);  SZ_CHECKBOX   = (60, 60)    # case a cocher (carte Pizza margaritta)
BTN_IMPRIMER1  = (1681, 748);  SZ_IMPRIMER1  = (300, 55)   # "Imprimer 1 etiquette(s)"
BTN_CREER      = (1028, 719);  SZ_CREER      = (280, 55)   # "Creer les etiquettes"
BTN_VALIDER    = (1253, 358);  SZ_VALIDER    = (360, 55)   # "Valider -> Historique"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  3.80,  4.20, None,  None,          None,          "1 - Selectionnez vos produits a etiqueter"),
    ("B", 3.80,  4.10,  0.80, 3.95,  BTN_CHECKBOX,  SZ_CHECKBOX,   None),
    ("C", 4.10,  6.90,  2.40, None,  None,          None,          None),
    ("D", 6.90,  7.20,  0.80, 7.05,  BTN_IMPRIMER1, SZ_IMPRIMER1,  None),
    ("E", 7.30,  12.00, 5.00, None,  None,          None,          "2 - Date, type et produit pre-remplis"),
    ("F", 13.50, 22.50, 5.00, None,  None,          None,          "3 - DLC, lot et code-barres calcules automatiquement"),
    ("G", 24.00, 28.80, 4.00, None,  None,          None,          "4 - Quantite et equipement de stockage"),
    ("H", 28.80, 29.10, 0.80, 28.95, BTN_CREER,     SZ_CREER,      None),
    ("I", 29.20, 32.20, 3.00, None,  None,          None,          None),
    ("J", 33.00, 40.20, 6.00, None,  None,          None,          "5 - Etiquettes pretes, tracabilite HACCP incluse"),
    ("K", 40.20, 40.50, 0.80, 40.35, BTN_VALIDER,   SZ_VALIDER,    None),
    ("L", 40.60, 44.50, 5.00, None,  None,          None,          None),
    ("M", 44.50, 46.56, 3.20, None,  None,          None,          None),
]
INTRO_D, OUTRO_D = 6.70, 6.00
...
CLAUDE_D = [4.50, 1.40, 4.20]  # reveal, copie, chatbot mockup (N7 est longue)
...
def build_silent(outro_d, claude_d):
    ...
    render_claude_stage3_png(f"{png_dir}/claude3.png", W, H, CLAUDE_PROMPT,
        response="Etiquette HACCP creee avec lot et DLC, prete a imprimer.")
    ...
    for i in range(len(labels_order) - 1):
        ...
        if {a, b} <= {"claude1", "claude2", "claude3"} or (a == "M" and b == "claude1") \
...
anchor = {
    "N0": 0.30,                    # intro hook
    "N1": S["A"] + 0.30,           # selectionnez vos produits
    "N2": S["E"] + 0.20,           # date/type/produit
    "N3": S["F"] + 0.20,           # DLC/lot/code-barres
    "N4": S["G"] + 0.20,           # quantite + equipement
    "N5": S["J"] + 0.20,           # etiquettes pretes / apercu
    "N6": S["L"] + 0.20,           # validees -> historique
    "N7": S["claude1"] + 0.25,     # prompt Claude (reveal)
    "N8": S["claude3"] + 0.25,     # chatbot mockup (resultat)
...
FINAL = f"{ROOT}/out/foodeatup-etiquettes-tuto-v1.mp4"
