#!/usr/bin/env python3
# FoodEatUp "Ajouter une reservation (module Table)" tutorial.
...
#
# Rush: page Reservations (plan de salle + file d'attente -> liste
# Reservations) -> "+ Nouvelle reservation" -> modale : nom "jean dupont",
# telephone, email "jean@contact.fr", date 12/08/2026, heure 06:07 PM,
# couverts 4 -> selection table T4 (filtre Salle principale) -> "Creer la
# reservation" -> retour liste : Jean Dupont "En attente" -> menu d'actions
# ouvert (Confirmer/Check-in/Modifier/No-show/Annuler/Supprimer).
...
ROOT = "/home/user/Video/videos/foodeatup-reservation-tuto"
...
# x1.20 zoom-punch). No apostrophe in any caption.
BTN_NOUVELLE_RESERVATION = (1638, 310); SZ_NOUVELLE_RESERVATION = (250, 52)  # "+ Nouvelle reservation"
BTN_TABLE_T4              = (965, 465); SZ_TABLE_T4              = (203, 66)  # table "T4" card
BTN_CREER_RESERVATION     = (1026, 745); SZ_CREER_RESERVATION    = (227, 47)  # "Creer la reservation"
BTN_ACTIONS_MENU          = (1633, 165); SZ_ACTIONS_MENU         = (36, 36)   # "..." row menu

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A1", 0.30,  4.00,  2.00, None,                     None,                    None),
    ("A2", 4.60,  8.00,  1.80, None,                     None,                    None),
    ("B",  8.00,  8.35,  0.90, BTN_NOUVELLE_RESERVATION, SZ_NOUVELLE_RESERVATION, None),
    ("C",  9.00,  24.00, 3.60, None,                     None,                    "1 - Coordonnees du client"),
    ("D",  32.00, 40.50, 3.20, None,                     None,                    "2 - Date heure et couverts"),
    ("E",  44.00, 48.50, 5.00, None,                     None,                    "3 - Choisissez une table"),
    ("F",  51.50, 52.00, 0.90, BTN_TABLE_T4,              SZ_TABLE_T4,             None),
    ("G",  52.00, 52.35, 0.90, BTN_CREER_RESERVATION,     SZ_CREER_RESERVATION,    None),
    ("H",  56.00, 60.50, 5.50, None,                     None,                    "Reservation creee"),
    ("I",  62.00, 65.856,6.40, BTN_ACTIONS_MENU,          SZ_ACTIONS_MENU,         None),
]
INTRO_D, OUTRO_D = 4.20, 5.60
...
# Matching tool: mcp__FoodEatUp__create_reservation(establishment_id,
# customer_name, party_size, date, time, table_id?, zone?, ...) -- same
# fields shown in the modal (name, covers, date, time), table optional.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une réservation pour [nom du client], [nombre de couverts] "
                  "couverts, le [date] à [heure], pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée la réservation tout de suite…"
...
CLAUDE_STAGE_D = [2.60, 2.60, 6.00]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A1
             "slideleft",  # A1 -> A2 (cut: plan de salle -> liste reservations)
             "fade",       # A2 -> B (continuous: click Nouvelle reservation)
             "slideleft",  # B -> C (cut into the opened modal)
             "fade",       # C -> D (continuous: coordonnees -> creneau)
             "fade",       # D -> E (continuous: creneau -> table)
             "fade",       # E -> F (continuous: table list -> click T4)
             "fade",       # F -> G (continuous: T4 selected -> click Creer)
             "slideleft",  # G -> H (submit -> retour liste)
             "fade",       # H -> I (continuous: resultat -> menu d'actions)
             "slideleft",  # I -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A1"] + 0.20,        # overview + click Nouvelle reservation
    "N2": S["C"] + 0.20,         # nom, telephone, email
    "N3": S["D"] + 0.20,         # date, heure, couverts
    "N4": S["E"] + 0.10,         # tables disponibles -> selection
    "N5": S["G"] + 0.10,         # click Creer -> resultat
    "N6": S["I"] + 0.15,         # menu d'actions
    "N7": S["claude1"] + 0.20,   # explique le prompt (reveal + copie)
    "N8": S["claude3"] + 0.20,   # colle dans Claude -> resultat instantane
...
FINAL = f"{ROOT}/out/foodeatup-reservation-tuto-v1.mp4"
