#!/usr/bin/env python3
# FoodEatUp "Controler a reception de vos livraisons" (Module HACCP) tutorial.
...
#
# Rush shows: browsing "Reception du jour" -> opening a delivered order
# (Commande #CMD-20260615104456-109, fournisseur Louay, Farine de ble T55
# 200kg commande / 199kg recue) -> the product row's Action menu (Photo DLC,
# DLC manuelle, Temperature, Scanner produit) -> clicking "Controle a
# reception" -> the "Nouvelle reception" form (date/heure, bon de livraison
# photo, reference, fournisseur, tags produits, etat de livraison, commentaires).
...
ROOT = "/home/user/Video/videos/foodeatup-reception-tuto"
...
# native pixels, one frame per action.
BTN_DOTS     = (1682, 423);  SZ_DOTS   = (40, 24)   # "..." Action menu (ligne produit)
BTN_CONTROLE = (1642, 250);  SZ_CTRL   = (300, 46)  # "Controle a reception"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",  0.20,  2.00,  3.60, None,  None,        None,     "1 - Réception du jour"),
    ("B",  2.00,  6.50,  5.60, None,  None,        None,     "2 - Détail de la commande"),
    ("C",  6.50,  6.85,  0.35, 6.65,  BTN_DOTS,     SZ_DOTS,  None),
    ("D",  6.85, 14.00,  3.60, None,  None,        None,     "3 - Photo DLC"),
    ("E", 14.00, 18.00,  2.00, None,  None,        None,     "DLC manuelle"),
    ("F", 18.00, 21.50,  1.80, None,  None,        None,     "Température"),
    ("G", 21.50, 26.50,  2.50, None,  None,        None,     "Scanner produit"),
    ("H", 26.50, 28.60,  3.60, None,  None,        None,     None),
    ("I", 28.60, 28.95,  0.35, 28.75, BTN_CONTROLE, SZ_CTRL,  None),
    ("J1", 28.95, 31.50, 5.20, None,  None,        None,     "4 - Nouvelle réception"),
    ("J2", 31.50, 34.80, 6.80, None,  None,        None,     "5 - État et commentaires"),
]
...
# heure_controle, etat_livraison, fournisseur_nom, reference_bl,
# temperature_produits_frais, commentaires, validate) is the matching tool --
# it covers the "Controle a reception" form shown in the rush. The per-product
# actions (Photo DLC / DLC manuelle / Temperature / Scanner produit) have no
# dedicated MCP tool, so no prompt is shown for those.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Enregistre un controle a reception pour la commande "
                  "[reference BL] du fournisseur [nom du fournisseur], "
                  "livraison [conforme / non conforme], le [date] a [heure], "
                  "pour mon etablissement FoodEatUp (ID [ID etablissement]).")
CLAUDE_RESPONSE = "Bien sur ! J'enregistre ce controle a reception pour votre etablissement..."
...
CLAUDE_STAGE_D = [2.60, 2.60, 5.40]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut: opens a specific order)
             "fade",       # B -> C (continuous: click on the visible page)
             "slideleft",  # C -> D (cut into the Photo DLC modal)
             "slideleft",  # D -> E (cut, Photo DLC -> DLC manuelle)
             "slideleft",  # E -> F (cut, DLC manuelle -> Temperature)
             "slideleft",  # F -> G (cut, Temperature -> Scanner produit)
             "slideleft",  # G -> H (cut, back to the order detail)
             "fade",       # H -> I (continuous: click Controle a reception)
             "slideleft",  # I -> J1 (cut into the Nouvelle reception form)
             "fade",       # J1 -> J2 (continuous: scroll within the same form)
             "slideleft",  # J2 -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # order detail reveal
    "N2": S["C"] + 0.10,         # Action menu -> Photo DLC/DLC manuelle/Temperature/Scanner
    "N3": S["H"] + 0.10,         # "pret a valider ? Controle a reception"
    "N4": S["J1"] + 0.10,        # date/heure/bon de livraison/reference/fournisseur
    "N5": S["J2"] + 0.10,        # produits/etat de livraison/commentaires/etape suivante
...
FINAL = f"{ROOT}/out/foodeatup-reception-tuto-v1.mp4"
