#!/usr/bin/env python3
# FoodEatUp "Gerer les recompenses fidelite" tutorial (module Marketing,
# Fidelite & Iris). No avatar clip: full ElevenLabs VO throughout. Speed =
...
#
# Rush (88.48s) shows 3 actions on the loyalty rewards catalogue (Fidelite &
# jeux > Recompenses): (1) modify an existing Bon-EUR reward into a Produit
# reward (type, plat, libelle, points, stock, quota) then save; (2) add a
# new Bon-EUR reward; (3) add a new Avantage reward, then save again.
...
ROOT = "/home/user/Video/videos/foodeatup-gerer-recompenses-tuto"
...
# build (6.1.1) -- animating a drawbox banner with a t-based expression
# silently draws it off-screen forever (documented in
# FOODEATUP-TUTORIELS-WORKFLOW.md). Fix retained across the series: two
# drawtext calls sharing the same slide expression, using drawtext's own
# box=1 as the plate (drawtext does evaluate t per frame).
BANNER_Y = H - 108
...
# Coordinates measured by colour-thresholding the blue "Enregistrer le
# catalogue" button on real frames (work/frames/click1.png @ t=31.2s,
# click2b.png @ t=85.5s -- taken just before each click, button not yet in
# its disabled/loading state). Same x-centre both times (fixed layout), y
# differs because the catalogue has one more reward row by the second save.
BTN_SAVE1 = (948, 673); SZ_SAVE1 = (296, 46)
BTN_SAVE2 = (948, 750); SZ_SAVE2 = (296, 44)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",       0.00,  4.00, 13.25, None,  None,      None,      "Catalogue de recompenses"),
    ("B",       4.00, 27.00,  9.50, None,  None,      None,      "Modifier une recompense"),
    ("C",      27.00, 31.00,  5.00, None,  None,      None,      None),
    ("clic1",  31.00, 31.40,  0.90, 31.20, BTN_SAVE1, SZ_SAVE1,  None),
    ("confirm1",31.40, 34.90, 2.30, None,  None,      None,      None),
    ("D",      37.40, 58.00,  7.50, None,  None,      None,      "Ajouter une recompense"),
    ("E",      61.00, 85.50,  8.50, None,  None,      None,      "Recompense Avantage"),
    ("clic2",  85.50, 85.90,  0.90, 85.70, BTN_SAVE2, SZ_SAVE2,  None),
    ("confirm2",85.90, 88.48, 2.00, None,  None,      None,      None),
]
# Durations sized against measured VO length (N0..N9: 3.71/4.31/7.37/4.44/
# 4.26/4.21/5.62/4.41/4.08/5.02s) -- rule from FOODEATUP-TUTORIELS-WORKFLOW.md:
# measure the VO before fixing segment durations, not after.
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__upsert_loyalty_reward
# (establishment_id, kind, label, points_cost, plat_id?, amount?, stock?,
# active?, reward_id?) is the matching tool -- covers all 3 reward kinds
# shown in the rush (product/amount/perk) and both create and edit
# (reward_id optional). Cleanest MCP match found on the series so far.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une récompense de fidélité de type [Produit / Bon € / Avantage], "
                  "intitulée [libellé], à [coût en points] points, pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici votre nouvelle récompense…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (legend -> edit form)
             "slideleft",  # B -> C (type/plat/points -> stock/quota)
             "fade",       # C -> clic1 (continuous: click on the visible page)
             "slideleft",  # clic1 -> confirm1 (cut into the confirmation)
             "slideleft",  # confirm1 -> D (saved -> new reward form)
             "slideleft",  # D -> E (Bon EUR -> Avantage)
             "fade",       # E -> clic2 (continuous: click on the visible page)
             "slideleft",  # clic2 -> confirm2 (cut into the confirmation)
             "slideleft",  # confirm2 -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # retrouver le catalogue
    "N2": S["A"] + 5.00,         # 3 types de recompenses (same segment A, later)
    "N3": S["B"] + 0.20,         # modifier type/plat/points
    "N4": S["C"] + 0.20,         # stock/quota + enregistrer
    "N5": S["D"] + 0.20,         # nouvelle recompense Bon EUR
    "N6": S["E"] + 0.20,         # recompense Avantage
...
FINAL = f"{ROOT}/out/foodeatup-gerer-recompenses-tuto-v1.mp4"
