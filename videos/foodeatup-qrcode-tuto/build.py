#!/usr/bin/env python3
# FoodEatUp "Diffuser son QR code" tutorial.
#
# No avatar clip in this one (Michael asked to drop the HeyGen avatar / its
# native voice for this video) -- straight from the intro card into the
# screen recording. Shared "use it with Claude" 3-stage sequence matches the
# one concrete server-side action in the rush: saving the social links
# (update_section). QR code / flyer / business card generation are pure
# client-side downloads, no MCP equivalent -- no invented prompt for those.
#
# Same engine as the rest of the series: setpts for speed (never zoompan on
# real footage), fixed crop+scale zoom-punch on the one save click, xfade on
...
ROOT = "/home/user/Video/videos/foodeatup-qrcode-tuto"
...
# Coordinates measured by extracting full-res frames around the save click.
BTN_SAVE = (516, 680); SZ_SAVE = (765, 50)  # "Enregistrer les liens"
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared 3-stage chatbot animation. Only the
# social-links save has a real MCP equivalent: update_section(establishment_id,
# section_id, props) merges partial props on a site section -- here the
# social-links section. QR/flyers/cards are client-side generation, no tool.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Mets à jour la section réseaux sociaux de mon site avec Facebook "
                  "[URL Facebook], Instagram [URL Instagram], TikTok [URL TikTok] "
                  "et Google Avis [URL Google Avis] pour mon établissement FoodEatUp "
                  "(ID [ID établissement], section [ID section]).")
CLAUDE_RESPONSE = "Bien sûr ! Je mets à jour vos réseaux sociaux…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (tab switch)
             "slideleft",  # B -> C (tab switch)
             "slideleft",  # C -> D1 (tab switch)
             "fade",       # D1 -> D2 (continuous: click on the visible button)
             "fade",       # D2 -> E (submit -> confirmation)
             "slideleft",  # E -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]  + 0.20,   # QR code variants + download
    "N2": S["B"]  + 0.20,   # Flyers
    "N3": S["C"]  + 0.20,   # Cartes de visite
    "N4": S["D1"] + 0.20,   # Reseaux sociaux -> Enregistrer les liens
    "N5": S["E"]  + 0.20,   # benefice: QR -> vitrine en ligne / horaires / reservations
...
FINAL = f"{ROOT}/out/foodeatup-qrcode-tuto-v1.mp4"
