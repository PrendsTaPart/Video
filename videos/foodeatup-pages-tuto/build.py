#!/usr/bin/env python3
# FoodEatUp "Gerer les pages de son site" tutorial.
...
ROOT = "/home/user/Video/videos/foodeatup-pages-tuto"
...
# banner() uses drawtext's own box=1 (not drawbox) so the sliding plate
# shares drawtext's per-frame `t` -- ffmpeg 6.1.1's drawbox only evaluates
# x/y once at init, so a t-based slide on drawbox never moves (see the
# "bandeaux invisibles" pitfalls in FOODEATUP-TUTORIELS-WORKFLOW.md).
BANNER_Y = H - 108
...
# Coordinates measured on the 1920x828 rush by reading the frames directly
# (fps=10 extracts around each click, see scratchpad analysis).
BTN_PAGES_NAV = (110, 469);  SZ_PAGES_NAV = (280, 44)   # "Pages" item, left sidebar
BTN_TOGGLE    = (1791, 338); SZ_TOGGLE    = (144, 46)   # "Depublier" / "Publier" (Accueil row)
BTN_VOIR      = (1685, 338); SZ_VOIR      = (66, 46)    # "Voir" (Accueil row)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.00, 2.90,  2.40, None, None,          None,         "1 - Menu Pages, à gauche"),
    ("B", 2.90, 3.10,  0.60, 2.95, BTN_PAGES_NAV,  SZ_PAGES_NAV, None),
    ("C", 3.10, 7.00,  8.50, None, None,          None,         "Accueil, Carte, À propos, Contact"),
    ("D", 7.00, 7.15,  0.60, 7.05, BTN_TOGGLE,     SZ_TOGGLE,    None),
    ("E", 7.15, 8.50,  4.00, None, None,          None,         "2 - Dépublier retire la page"),
    ("F", 8.50, 8.75,  0.60, 8.55, BTN_TOGGLE,     SZ_TOGGLE,    None),
    ("G", 8.75, 9.50,  4.20, None, None,          None,         "3 - Publier la republie aussitôt"),
    ("H", 9.50, 9.65,  0.60, 9.55, BTN_VOIR,       SZ_VOIR,      None),
    ("I", 10.90, 17.96, 4.20, None, None,         None,         "En direct sur votre site"),
]
...
# across the whole series (videos/_shared/claude_prompt_sequence.py). Matching
# tool: toggle_site_page(establishment_id, page_slug, publish) -- exactly what
# the rush shows (Accueil depubliee puis republiee).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Dépublie temporairement la page [nom de la page, ex : Accueil] de mon "
                  "site FoodEatUp (établissement ID [ID établissement]) le temps d'une "
                  "mise à jour, puis republie-la.")
CLAUDE_RESPONSE = "Bien sûr ! Je mets à jour le statut de cette page…"
...
FINAL = f"{ROOT}/out/foodeatup-pages-tuto-v1.mp4"
