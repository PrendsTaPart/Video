#!/usr/bin/env python3
# FoodEatUp "Retrouver votre agenda marketing" tutorial (jours creux + campagne planifiee).
# Same engine as foodeatup-haccp-export-tuto/build.py: setpts never zoompan on real
# footage, xfade on every cut forced back to yuv420p, 48kHz stereo AAC +faststart,
...
ROOT = "/home/user/Video/videos/foodeatup-agenda-marketing-tuto"
...
def banner(text, seg_dur):
    # drawbox does not evaluate x/y per frame on this ffmpeg build (6.1.1) --
    # a t-based x expression leaves the plate parked off-screen, invisible.
    # Fix: use drawtext's own box=1 (drawtext DOES re-evaluate x per frame),
    # two passes -- orange plate offset 10px left, blue plate on top.
    # See FOODEATUP-TUTORIELS-WORKFLOW.md and foodeatup-mouvement-stock-tuto/build.py.
...
BTN_CREER    = (1046, 372); SZ_CREER    = (75, 26)    # "Creer ->" (jour creux, jeudi 6)
BTN_CONFIRM  = (1323, 654); SZ_CONFIRM  = (330, 50)   # "Confirmer la planification"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.30,  6.80, None,  None,        None,        "Agent IA : propositions chiffrees"),
    ("B", 4.80,  7.30,  5.50, None,  None,        None,        "Agenda : vos jours creux detectes"),
    ("C", 7.70,  7.95,  0.90, 7.82,  BTN_CREER,   SZ_CREER,    None),
    ("D", 9.50,  11.50, 5.90, None,  None,        None,        "1 - Cible : choisissez le segment"),
    ("E", 12.00, 19.00, 5.50, None,  None,        None,        "2 - Message : offre et code promo"),
    ("F", 19.60, 26.60, 3.00, None,  None,        None,        "3 - Planification : choisissez la date"),
    ("G", 26.80, 28.10, 4.50, None,  None,        None,        "4 - Conformite : cout et exclusions"),
    ("H", 28.15, 28.40, 0.90, 28.28, BTN_CONFIRM, SZ_CONFIRM,  None),
    ("I", 30.30, 31.00, 2.70, None,  None,        None,        "Campagne planifiee"),
]
...
# mcp__FoodEatUp__propose_campaigns(establishment_id) matches exactly what's
# shown on screen (Agent IA marketing : campagnes chiffrees sur jours creux,
# marge, historique reel) -- verifie via un appel reel (etablissement 26).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Propose-moi des campagnes marketing pour mon établissement "
                  "FoodEatUp (ID [ID établissement]), basées sur mes jours "
                  "creux et mes segments de clients.")
CLAUDE_RESPONSE = ("Bien sûr ! Votre jour le plus creux est mercredi. Je vous "
                    "propose : « Mercredi Délicieux » pour vos clients "
                    "prometteurs (-10 %), et « Retour des Risqués » pour "
                    "reconquérir vos 2 clients à risque.")
...
CLAUDE_STAGE_D = [3.00, 2.30, 5.60]  # reveal, copied, chatbot mockup
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut, Agent IA -> Agenda)
             "fade",       # B -> C (continuous: click Creer)
             "slideleft",  # C -> D (click -> modal Cible)
             "fade",       # D -> E (Cible -> Message)
             "fade",       # E -> F (Message -> Planification)
             "fade",       # F -> G (Planification -> Conformite)
             "fade",       # G -> H (continuous: click Confirmer)
             "fade",       # H -> I (click -> succes)
             "slideleft",  # I -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.15,         # Agent IA, propositions chiffrees
    "N2": S["B"] + 0.15,         # Agenda, jours creux -> clic Creer
    "N3": S["D"] + 0.15,         # etape 1 Cible
    "N4": S["E"] + 0.15,         # etape 2 Message + 3 Planification
    "N5": S["G"] + 0.10,         # etape 4 Conformite -> clic Confirmer -> succes
...
FINAL = f"{ROOT}/out/foodeatup-agenda-marketing-tuto-v1.mp4"
