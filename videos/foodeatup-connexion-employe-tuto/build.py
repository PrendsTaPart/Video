#!/usr/bin/env python3
# FoodEatUp "Se connecter cote employe - URL & code PIN" tutorial (module Equipe & Planning).
# Same engine as every FoodEatUp tuto (see videos/FOODEATUP-TUTORIELS-WORKFLOW.md):
# setpts speed change (never zoompan on real footage), zoom-punch crop on button
# clicks, xfade on every cut, forced back to yuv420p at the end of the chain.
# 48kHz stereo AAC, +faststart.
#
# Rush (25.96s): page "qr code actif" -> URL de connexion, clic "Copier" -> nouvel
# onglet navigateur (cote employe) -> URL collee dans la barre d'adresse -> page
# "Qui etes-vous ?" -> clic sur le profil "alice Charbit" -> saisie du code PIN
# (4 chiffres) -> clic "Mon espace" -> "mon espace" : grille de modules limitee au
# role de l'employe (manager), meme logique dans le menu du haut.
#
# Pas de sequence Claude ici : aucun outil MCP FoodEatUp ne couvre la connexion
# cote employe (URL/QR + PIN), meme raison que creer-son-code-pin / creer-son-compte
# dans le reste de la serie.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-connexion-employe-tuto"
...
BTN_COPIER         = (1740, 500); SZ_COPIER         = (100, 90)   # "Copier" (URL de connexion)
BTN_PROFILE_ALICE  = (1175, 715); SZ_PROFILE        = (220, 190)  # carte profil "alice Charbit"
BTN_MON_ESPACE     = (1615, 740); SZ_MON_ESPACE     = (365, 95)   # bouton "Mon espace"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",  0.20,  3.30,  4.20, None,  None,              None,          "1 · URL de connexion"),
    ("B",  3.45,  3.75,  0.80, 3.60,  BTN_COPIER,        SZ_COPIER,     None),
    ("C",  5.30,  6.60,  2.80, None,  None,              None,          "2 · Côté employé"),
    ("D",  8.40,  9.40,  1.80, None,  None,              None,          "3 · Qui êtes-vous ?"),
    ("E",  9.40,  9.80,  0.80, 9.60,  BTN_PROFILE_ALICE, SZ_PROFILE,    None),
    ("F", 10.00, 13.60,  4.20, None,  None,              None,          "4 · Code PIN personnel"),
    ("G", 14.55, 14.95,  0.80, 14.75, BTN_MON_ESPACE,    SZ_MON_ESPACE, None),
    ("H1",17.00, 19.40,  3.20, None,  None,              None,          "Mon espace"),
    ("H2",20.00, 23.00,  4.00, None,  None,              None,          "Modules selon le rôle"),
    ("H3",24.00, 25.80,  2.60, None,  None,              None,          None),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click Copier)
             "slideleft",  # B -> C (cut: nouvel onglet navigateur)
             "slideleft",  # C -> D (cut: navigation vers la page de connexion)
             "fade",       # D -> E (continuous: clic sur le profil)
             "fade",       # E -> F (continuous: clavier PIN apparait)
             "fade",       # F -> G (continuous: clic Mon espace)
             "slideleft",  # G -> H1 (cut: nouvelle page /employee/hub)
             "fade",       # H1 -> H2 (continuous: scroll dans la grille)
             "fade",       # H2 -> H3 (continuous: retour en haut de page)
             "fade"]       # H3 -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # url de connexion / copier
    "N2": S["C"] + 0.20,         # nouvel onglet cote employe
    "N3": S["D"] + 0.20,         # qui etes-vous / profil
    "N4": S["F"] + 0.30,         # code pin
    "N5": S["G"] + 0.10,         # clic mon espace / pointer
    "N6": S["H2"] + 0.20,        # modules selon le role
...
FINAL = f"{ROOT}/out/foodeatup-connexion-employe-tuto-v1.mp4"
