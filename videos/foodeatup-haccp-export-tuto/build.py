#!/usr/bin/env python3
# FoodEatUp "Exporter tout son classeur HACCP" tutorial (historique + export).
# Same engine as foodeatup-fournisseurs-tuto/build.py (see that file for the
# detailed rationale of each choice: setpts never zoompan on real footage,
# xfade on every cut forced back to yuv420p, 48kHz stereo AAC +faststart,
# segment durations calibrated on the VO line that comments them).
...
ROOT = "/home/user/Video/videos/foodeatup-haccp-export-tuto"
...
# Coordinates measured on extracted frames (work/frames/, 1920x828 native) --
# t8.jpg for Exporter CSV, t42.jpg for Exporter l'historique.
BTN_EXPORT_CSV  = (1720, 352); SZ_EXPORT_CSV  = (260, 46)   # "Exporter CSV" (Températures)
BTN_EXPORT_HIST = (1115, 335); SZ_EXPORT_HIST = (330, 55)   # "Exporter l'historique" (Checklist Hygiène)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  6.00,  5.70, None, None,             None,             "Historique : tous vos modules HACCP"),
    ("C", 8.00,  13.60, 4.90, None, None,             None,             "1 · Températures : filtrez, exportez en CSV"),
    ("D", 13.60, 13.85, 0.90, 13.72, BTN_EXPORT_CSV,   SZ_EXPORT_CSV,    None),
    ("E", 14.20, 20.00, 4.90, None, None,             None,             "2 · Traçabilité & Contrôle à réception"),
    ("F", 40.20, 42.00, 4.00, None, None,             None,             "3 · Checklist Hygiène"),
    ("G", 42.00, 42.25, 0.90, 42.10, BTN_EXPORT_HIST,  SZ_EXPORT_HIST,   None),
    ("H", 42.40, 44.20, 6.50, None, None,             None,             "Export généré avec succès"),
]
...
# mcp__FoodEatUp__list_haccp_temperatures(establishment_id, start_date?,
# end_date?, equipment_id?, type?) matches exactly what's shown on screen
# (stats + Frigo 1 detail, real data checked against establishment 26/GoSushi).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Sors-moi l'historique des relevés de température de mon "
                  "établissement FoodEatUp (ID [ID établissement]) du "
                  "[date début] au [date fin].")
CLAUDE_RESPONSE = ("Bien sûr ! Sur cette période : 19 911 relevés, 14 504 "
                    "conformes, 95 non conformes. Le Frigo 1 est à 13,0 °C…")
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> C (cut, hub -> temperatures)
             "fade",       # C -> D (continuous: click Exporter CSV)
             "fade",       # D -> E (click -> traçabilité/réception)
             "slideleft",  # E -> F (cut, -> checklist hygiène)
             "fade",       # F -> G (continuous: click Exporter l'historique)
             "fade",       # G -> H (click -> succès)
             "slideleft",  # H -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.15,         # hub, 7 cartes
    "N2": S["C"] + 0.15,         # températures + Exporter CSV
    "N3": S["E"] + 0.15,         # traçabilité + contrôle réception
    "N4": S["F"] + 0.10,         # checklist hygiène -> clic Exporter l'historique
    "N5": S["H"] + 0.10,         # export généré -> astuce du chef
...
FINAL = f"{ROOT}/out/foodeatup-haccp-export-tuto-v1.mp4"
