#!/usr/bin/env python3
# FoodEatUp "Agent Gestion Fournisseur (Predibot)" tutorial.
...
ROOT = "/home/user/Video/videos/foodeatup-agent-gestion-fournisseur-tuto"
SRC  = f"{ROOT}/assets/screen.mp4"
W, H, FPS = 1526, 1032, 25
SEG = f"{ROOT}/work/seg"
...
def render_banner_png(text, path):
    ...
    same pattern as foodeatup-production-ingredients-tuto/build.py. Also
    the only option here: this ffmpeg build (imageio-ffmpeg static binary,
    no fontconfig-linked drawtext) has no `drawtext` filter at all."""
...
# Coordonnées mesurées sur les frames réelles du rush (espace source 1526x1032) :
# bouton d'envoi (fleche verte) du champ de message Predibot, cadre au moment
# de l'envoi du message de validation (t~19.6s).
BTN_SEND = (1483, 985); SZ_SEND = (46, 46)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20, 10.00, 7.60, None, None, None, "1 · Vos commandes fournisseurs"),
    ("B", 12.00, 19.60, 4.60, 19.55, BTN_SEND, SZ_SEND, "2 · Valider une réception"),
    ("C", 19.90, 22.50, 3.30, None, None, None, "Réponse instantanée"),
    ("D", 22.50, 26.32, 5.40, None, None, None, "3 · Déjà à jour dans FoodEatUp"),
]
...
# across the whole series (videos/_shared/claude_prompt_sequence.py).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Valide la réception de la commande [référence BL], fournisseur "
                  "[nom du fournisseur], état [conforme/non conforme], avec le commentaire "
                  "[RAS ou détail], pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je valide cette réception fournisseur pour votre établissement…"
...
FINAL = f"{ROOT}/out/foodeatup-agent-gestion-fournisseur-tuto-v1.mp4"
