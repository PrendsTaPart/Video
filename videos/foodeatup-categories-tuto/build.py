#!/usr/bin/env python3
# FoodEatUp "Créer ses catégories" tutorial.
...
#
# Segment targets were computed against the FULL measured VO chain (N0-N8,
# see vo/*.mp3) before building -- walking the xfade start times forward and
# making sure every line begins at its own anchor with zero push-chain drift
# (lesson from the tva rebuild: checking only "does the total fit" lets a
# 4-6s voice/picture offset build up by the end of the video).
...
ROOT = "/home/user/Video/videos/foodeatup-categories-tuto"
...
BTN_ADD_CAT = (1570, 367); SZ_ADD_CAT = (300, 56)   # "Ajouter une catégorie" (header)
BTN_SUBMIT  = (1211, 737); SZ_SUBMIT  = (128, 56)   # "Ajouter" (modal submit)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.60,  2.90, None, None,       None,       "1 · Vos catégories"),
    ("B", 4.30,  4.55,  0.90, 4.35, BTN_ADD_CAT, SZ_ADD_CAT, None),
    ("C", 5.00,  11.00, 6.30, None, None,       None,       "2 · Type et nom"),
    ("D", 12.00, 34.50, 5.20, None, None,       None,       "3 · Tags à la volée"),
    ("E", 44.50, 53.50, 4.00, None, None,       None,       "4 · Icône et couleur"),
    ("F", 54.30, 54.60, 0.90, 54.40, BTN_SUBMIT, SZ_SUBMIT,  None),
    ("G", 55.30, 60.30, 4.20, None, None,       None,       "Catégorie créée"),
]
...
# (videos/_shared/claude_prompt_sequence.py). Only the prompt text and the
# assistant reply are specific to this tutorial. mcp__FoodEatUp__
# create_category(establishment_id, name, type in {ingredient, produit,
# recette}, icon?, color?) is the matching tool.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une catégorie [nom de la catégorie] de type "
                  "[produit / ingrédient / recette] pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cette catégorie pour votre établissement…"
...
# stage3 -- stage durations sized against those measured lines.
...
def build_silent(outro_d):
    ...
    # slideleft on every skipped-footage cut (B->C jumps the dropdown open
    # animation, C->D jumps to tags, D->E jumps to the icon block) and
    # between the claude stages; fade elsewhere.
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: click on the visible page)
             "slideleft",  # B -> C (cut into the opened modal)
             "slideleft",  # C -> D (cut to tags typing)
             "slideleft",  # D -> E (cut to icon/colour block)
             "fade",       # E -> F (continuous: click Ajouter)
             "fade",       # F -> G (submit -> toast/list)
             "slideleft",  # G  -> claude1
...
GAP = 0.22
anchor = {
    "N0": 0.30,                  # intro hook (spills over A -- page overview)
    "N1": S["B"] + 0.10,         # "cliquez sur Ajouter une catégorie" on the click
    "N2": S["C"] + 0.20,         # type + nom
    "N3": S["D"] + 0.20,         # tags
    "N4": S["E"] + 0.20,         # icône + couleur
    "N5": S["F"] + 0.10,         # validez -> toast/liste (spills over G)
...
FINAL = f"{ROOT}/out/foodeatup-categories-tuto-v1.mp4"
