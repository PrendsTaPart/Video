#!/usr/bin/env python3
# FoodEatUp "Configurer son profil entreprise" tutorial (rebuild -- the
# previous version predates this pipeline and shipped with the 96kHz mono /
# no-faststart playback bug, and no animations).
#
# No avatar clip in this project: full ElevenLabs VO throughout. Speed =
# setpts (never zoompan on real footage -- freezes the image). xfade on every
# cut, forced back to yuv420p at the end of the chain. 48kHz stereo AAC,
# +faststart. Segment targets are set close to each VO line's *measured*
# duration up front this time (see vo/*.mp3) -- picking them blind and
# reconciling after burned 22s of dead outro on the abonnement rebuild.
...
BTN_EDIT_INFO = (1616, 170); SZ_EDIT_INFO = (300, 46)   # "Modifier les informations"
BTN_SAVE      = (1024, 702); SZ_SAVE      = (140, 56)   # "Modifier" (save)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.00,  2.30, None, None,           None,          "1 · Vos informations"),
    ("B", 2.00,  2.50,  0.90, 2.00, BTN_EDIT_INFO,   SZ_EDIT_INFO,  None),
    ("C", 3.00,  4.50,  3.00, None, None,           None,          "2 · Modifiez vos informations"),
    ("D", 4.50,  9.50,  5.00, None, None,           None,          None),
    ("E", 9.50,  9.80,  0.90, 9.50, BTN_SAVE,        SZ_SAVE,       None),
    ("F", 10.00, 14.20, 3.80, None, None,           None,          "Informations mises à jour"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade","fade","fade","fade","fade","fade","fade"]
