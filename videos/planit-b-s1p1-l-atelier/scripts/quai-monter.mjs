 ...
 *   3. vérifie assets/voice/vo.mp3 (durée ≤ 6.0s, sinon ARRÊT explicite)
 ...
 *     references/planit-brand.md, puis écrit assets/voice/vo.mp3 et
 ...
 *   assets/voice/vo.mp3       (la voix off synthétisée)
...
function meanVolumeDb(file, start, dur) {
  // NB : volumedetect écrit ses statistiques (mean_volume, max_volume...)
  // au niveau de log "info", pas "error" — passer "-v error" ici les
  // supprime silencieusement et fait toujours renvoyer null, même quand
  // la piste est parfaitement audible. On garde donc le niveau par défaut
  // (info) et on se contente de couper la bannière/les stats de progression.
  const args = ["-hide_banner", "-nostats"];
  if (start != null) args.push("-ss", String(start));
  if (dur != null) args.push("-t", String(dur));
  args.push("-i", file, "-af", "volumedetect", "-f", "null", "-");
  // spawnSync (pas execFileSync) : on a besoin de stderr même quand ffmpeg
  // termine avec un code 0 — execFileSync ne renvoie que stdout dans ce
  // cas, et c'est justement sur stderr que volumedetect écrit ses stats.
...
function checkVoice(episodeDir) {
  const voicePath = path.join(episodeDir, "assets", "voice", "vo.mp3");
  if (!existsSync(voicePath)) die(`voix off introuvable : assets/voice/vo.mp3`);
...
function assemble(episodeDir, config) {
  ...
  const commonTokens = {
    ...
    __QUAI_VOICE_SRC__: "assets/voice/vo.mp3",
