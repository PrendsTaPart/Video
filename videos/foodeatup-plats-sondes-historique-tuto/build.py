#!/usr/bin/env python3
# FoodEatUp "Retrouver mes plats sondes (historique)" tutorial (module HACCP).
...
#
# Rush (25.80s) has two screens: Production > Temperatures > onglet Plats
# (sonder les plats a coeur, ajuster puis Enregistrer -> confirm -> succes),
# puis Historique > Temperatures > onglet Plats (6 KPIs, recherche/filtre/
# export CSV, liste des plats sondes avec statut conforme/non conforme).
...
ROOT = "/home/user/Video/videos/foodeatup-plats-sondes-historique-tuto"
...
# FOODEATUP-TUTORIELS-WORKFLOW.md, hit on foodeatup-mouvement-stock-tuto and
# others). Fix retained across the series: two drawtext calls sharing the
# same slide expression, using drawtext's own box=1 as the plate (drawtext
# does evaluate t per frame) -- orange plate offset 10px left as a fringe,
# blue plate on top. See videos/foodeatup-mouvement-stock-tuto/build.py.
BANNER_Y = H - 108
...
# Coordinate measured by colour-thresholding the green "Enregistrer les
# releves de temperature" button on the real frame at t=10.5s (before the
# click) -- see work/frames/click.png. Button centre (1549, 743), size
# (616, 95), fixed floating-action-button position (no scroll between the
# earlier frames and the click).
BTN_ENREGISTRER = (1549, 743); SZ_ENREGISTRER = (616, 95)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",       2.88, 10.80, 7.80, None,  None,            None,           "Production > Plats : sonder a coeur"),
    ("clic",   10.80, 11.30, 0.90, 11.05, BTN_ENREGISTRER, SZ_ENREGISTRER, None),
    ("confirm",11.30, 14.60, 2.30, None,  None,            None,           None),
    ("B",      16.00, 18.20, 3.35, None,  None,            None,           "Historique > Temperatures"),
    ("C",      18.20, 20.20, 8.50, None,  None,            None,           "Vos indicateurs cles"),
    ("D",      20.20, 22.50, 6.50, None,  None,            None,           "Basculez sur Plats"),
    ("E",      22.50, 25.80, 5.70, None,  None,            None,           "Recherche, date, export CSV"),
]
# Durations sized against measured VO length (N0..N8: 3.89/5.62/2.87/6.92/
# 5.93/5.20/4.41/4.18/5.02s) -- rule from FOODEATUP-TUTORIELS-WORKFLOW.md:
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__list_haccp_temperatures
# (establishment_id, equipment_id?, type?, start_date?, end_date?) is the
# closest matching tool -- read-only, used here on a period without
# equipment_id to retrieve dish (plat) readings, same tool as the sibling
# equipment-history video (foodeatup-historique-temperature-tuto).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Liste l'historique de mes plats sondés (température à cœur) entre "
                  "le [date de début] et le [date de fin], pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici l'historique de vos plats sondés…"
...
# Same override as productions-tuto/vitrine-tuto (vs. shared default
# [2.20,1.30,2.50]): N6 (reused from tva-tuto, 4.41s) and N7 (4.18s) both
# need real room to land on their own stage instead of drifting.
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> clic (continuous: click on the visible page)
             "slideleft",  # clic -> confirm (cut into the modal)
             "slideleft",  # confirm -> B (cut to Historique nav)
             "slideleft",  # B -> C (landing -> KPIs)
             "slideleft",  # C -> D (KPIs -> Plats list)
             "slideleft",  # D -> E (list -> tools)
             "slideleft",  # E -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.20,         # onglet Plats, saisie + validez
    "N2": S["B"] + 0.10,         # direction Historique
    "N3": S["C"] + 0.20,         # 6 KPIs
    "N4": S["D"] + 0.10,         # bascule Plats + liste
    "N5": S["E"] + 0.10,         # recherche / filtre / export CSV
...
FINAL = f"{ROOT}/out/foodeatup-plats-sondes-historique-tuto-v1.mp4"
