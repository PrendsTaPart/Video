#!/usr/bin/env python3
# FoodEatUp "Suivre ses credits SMS & WhatsApp" tutorial (module Marketing,
# Fidelite & Iris). Rush = continuous scroll through a read-only billing
# screen (Abonnement -> Mes credits & minutes), no create/click forms, so
# no zoom-punch here -- plain full-frame segments + step banners. Speed =
...
#
# No Claude sequence: no mcp__Foodeatup__* tool reads credit balance,
# dotations or voice minutes (billing data, not a business entity exposed
# by the MCP) -- same reasoning as creer-son-code-pin / generer-qr-code-
# pointage. See SCRIPT.md.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-credits-com-tuto"
...
segs = [
    ("A", 0.20,  8.00,  4.58, "1 - Abonnement puis Mes credits et minutes"),
    ("B", 8.00,  9.50,  6.43, "2 - Quota du mois"),
    ("C", 9.50,  11.50, 6.56, "3 - Achats et dotations"),
    ("D", 11.50, 14.00, 8.94, "4 - Minutes voix"),
    ("E", 14.50, 22.24, 6.67, "5 - Recharge et pack annuel"),
]
INTRO_D, OUTRO_D = 4.90, 5.55
...
def build_silent(outro_d):
    ...
    trans = ["fade",        # intro -> A
             "slideleft",   # A -> B (fin du defilement rapide -> section credits)
             "fade",        # B -> C (meme ecran, 3 colonnes)
             "fade",        # C -> D (meme ecran, 3 colonnes)
             "slideleft",   # D -> E (defilement vers recharge / pack annuel)
             "fade"]        # E -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.15,         # navigation
    "N2": S["B"] + 0.15,         # quota du mois
    "N3": S["C"] + 0.15,         # achats et dotations
    "N4": S["D"] + 0.15,         # minutes voix
    "N5": S["E"] + 0.15,         # recharge + pack annuel
...
FINAL = f"{ROOT}/out/foodeatup-credits-com-tuto-v1.mp4"
