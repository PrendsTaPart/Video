#!/usr/bin/env python3
# FoodEatUp "Relever une température d'équipement" tutorial.
# Same engine as foodeatup-tva-tuto (see FOODEATUP-TUTORIELS-WORKFLOW.md):
# setpts speed (never zoompan on real footage), xfade on every cut, forced
# back to yuv420p at the end of the chain, 48kHz stereo AAC, +faststart.
# Segment targets sized against each VO line's measured duration (see
# vo/*.mp3) before building, not after.
...
BTN_SAVE_TEMP = (1575, 755); SZ_SAVE_TEMP = (590, 64)   # "Enregistrer les relevés de température"
BTN_CONFIRM   = (875, 607);  SZ_CONFIRM   = (250, 55)   # "Oui, enregistrer !" (modal)
BTN_OK        = (975, 582);  SZ_OK        = (120, 65)   # "OK" (modal succès)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
# Raw source (screen.mp4) is 20.68s: idle list (A) -> adjustment via +/-
# steppers (C, continuous multi-click, no single-button punch) -> click
# "Enregistrer" (D, punch) -> confirmation modal (E) -> click "Oui,
# enregistrer !" (F, punch) -> success modal (G) -> click "OK" (H, punch) ->
# back to list with counters updated / non-conformité auto-détectée (I).
segs = [
    ("A", 0.00, 3.50,  5.00, None,  None,          None,        "1 · Vos équipements de froid"),
    ("C", 3.50, 10.00, 4.60, None,  None,          None,        "2 · Ajustez la température mesurée"),
    ("D", 10.00, 10.35, 0.90, 10.15, BTN_SAVE_TEMP, SZ_SAVE_TEMP, None),
    ("E", 10.35, 12.50, 2.30, None, None,          None,        "3 - Confirmez la sauvegarde"),
    ("F", 12.50, 12.85, 0.90, 12.65, BTN_CONFIRM,   SZ_CONFIRM,   None),
    ("G", 12.85, 14.50, 1.60, None, None,          None,        None),
    ("H", 14.50, 14.85, 0.90, 14.65, BTN_OK,        SZ_OK,        None),
    ("I", 14.85, 20.68, 3.60, None, None,          None,        "Non-conformité détectée automatiquement"),
]
INTRO_D, OUTRO_D = 5.20, 5.60
...
# mcp__FoodEatUp__add_temperature(establishment_id, equipment_id, temperature, measured_at?)
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Enregistre un relevé de température de [température]°C pour "
                  "l'équipement [nom de l'équipement] (ID [ID équipement]) de mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Relevé enregistré ! Je vérifie aussitôt s'il respecte les seuils…"
...
# N5 explains the prompt over stage1+2 (measured 4.26s), N6 presents sending
# it to Claude over stage3 (measured 4.13s) -- stages sized to give each
# room (measure-VO-before-segments rule, see FOODEATUP-TUTORIELS-WORKFLOW.md).
CLAUDE_STAGE_D = [2.60, 2.00, 4.60]  # reveal, copied, chatbot mockup
...
OUTRO_START = S["outro"]
print("S:", {k: round(v, 2) for k, v in S.items()})
...
anchor = {
    ...
    "N3": S["D"] + 0.20,
    "N4": S["G"] + 0.20,
    "N5": S["claude1"] + 0.20,   # explains the prompt (reveal + copied)
    "N6": S["claude3"] + 0.20,   # presents sending it to Claude
