#!/usr/bin/env python3
# FoodEatUp "Retrouver mes étiquettes — historique" tutorial (module HACCP).
...
#
# Rush (51.65s) flow reconstructed from frame sampling every 3s:
#   0-9s   liste "Historique étiquettes" -> clic sur une étiquette -> panneau
#          détail (infos, QR code, notes, stockage)
#   9-18s  historique de l'étiquette qui se déroule (créations/impressions
#          horodatées)
#   18-30s Journal d'audit : stats globales + filtre dates + détail produit
#   30-39s Aperçu de l'export -> prévisualisation du rapport PDF généré
#   39-51.6s détail étiquette (activité), retour liste, page Export des
#          données (total étiquettes / produits uniques)
#
# Segment targets sized against the measured VO chain (N0-N8, vo/*.mp3)
# before building, same discipline as categories/vitrine builds.
...
ROOT = "/home/user/Video/videos/foodeatup-etiquettes-historique-tuto"
...
# (videos/_shared/claude_prompt_sequence.py). list_haccp_labels is read-only
# but directly matches "retrouve l'historique de mes étiquettes".
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Montre-moi l'historique des étiquettes HACCP de mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici l'historique de vos étiquettes…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut: detail -> historique expand)
             "slideleft",  # B -> C (cut: historique -> journal d'audit)
             "slideleft",  # C -> D (cut: audit -> export preview)
             "slideleft",  # D -> E (cut: export -> retour liste/export page)
             "slideleft",  # E -> claude1
...
GAP = 0.22
anchor = {
    "N0": 0.30,                  # intro hook (spills over A)
    "N1": S["A"] + 0.20,         # clic sur une étiquette -> détail
    "N2": S["B"] + 0.20,         # historique de l'étiquette
    "N3": S["C"] + 0.20,         # journal d'audit
    "N4": S["D"] + 0.20,         # export PDF
    "N5": S["E"] + 0.20,         # bénéfice (retour liste/export)
...
FINAL = f"{ROOT}/out/foodeatup-etiquettes-historique-tuto-v1.mp4"
