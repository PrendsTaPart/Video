#!/usr/bin/env python3
# FoodEatUp "Recharger ses credits, Pack Com'" tutorial (module Marketing,
# Fidelite & Iris, catalogue #11). Same engine as every FoodEatUp tuto (see
# videos/FOODEATUP-TUTORIELS-WORKFLOW.md): setpts speed change (never
# zoompan on real footage), xfade on every cut, forced back to yuv420p.
# 48kHz stereo AAC, +faststart. No crop needed: rush is already 1920x828.
#
# Short rush (13.52s), no clicks to punch on: the page shows the Marketing &
# Commercial pack card (already active, 1500 credits SMS/Email/WhatsApp/mois
# included) then scrolls to the Options & modules section. No
# mcp__Foodeatup__* tool covers buying/tracking credits (Stripe billing) --
# no "Utiliser avec Claude" sequence, see SCRIPT.md.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-recharger-credits-tuto"
...
segs = [
    ("A", 5.50,  8.50, 11.00, "1 - Pack Marketing et Commercial"),
    ("B", 8.50, 13.40,  6.00, "2 - Options et modules"),
]
INTRO_D, OUTRO_D = 6.50, 6.00
...
def build_silent(outro_d):
    ...
    trans = ["fade",   # intro -> A
             "fade",   # A -> B (continuous scroll, not a context cut)
             "fade"]   # B -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # Pack Marketing & Commercial, credits included
    "N2": S["A"] + 0.30,         # same anchor -- sequential placement pushes it right after N1
    "N3": S["B"] + 0.30,         # Options & modules
    "N4": OUTRO_START + 0.35,    # CTA
}
...
FINAL = f"{ROOT}/out/foodeatup-recharger-credits-tuto-v1.mp4"
