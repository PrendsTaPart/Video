#!/usr/bin/env python3
# FoodEatUp "Installer la borne d'accueil" tutorial (module Equipe & Planning).
...
#
# Rush is a real Chrome recording (1920x1020 @60fps) of the public kiosk page
# (foodeatup.com/employee/qr/<token>) -- browser chrome (tabs/url bar/
# bookmarks) occupies the top 197px and is cropped out, giving a clean
# 1920x823 canvas. Flow: "Qui etes-vous ?" employee grid -> select "Soulayma
# Abdenbi" -> "Saisissez votre pin" (4-digit keypad) -> "Pointer" (clock in/
# out) or "Mon espace" (personal dashboard) -> "Mon espace" reveals only the
# modules her role (manager) can access, header shows her logged in (avatar
# "SA"). No mcp__FoodEatUp__* tool covers this (employee self-service on a
# shared kiosk via PIN, not an admin/API action) -- same reasoning as
# foodeatup-jarvis-tuto's QR pairing, so no "use it with Claude" sequence.
import subprocess, os, sys

ROOT = "/home/user/Video/videos/foodeatup-borne-tuto"
...
# Crop out the Chrome browser chrome (tabs/url bar/bookmarks, top 197px of the
# native 1920x1020 recording) once, up front -- every segment below reads from
# this already-cropped, already-25fps source.
if not os.path.exists(SRC):
    run(["ffmpeg","-y","-v","error","-i",SRC_RAW,"-vf",f"crop={W}:{H}:0:197,fps={FPS}",
...
# Coordinates measured on the extracted, already-cropped frames (1920x823).
# No apostrophe in any caption below (bug hit on foodeatup-ingredients-tuto --
# see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_EMPLOYEE  = (1502, 335);  SZ_EMPLOYEE  = (165, 170)  # carte "Soulayma Abdenbi"
BTN_MONESPACE = (1614, 641);  SZ_MONESPACE = (364, 60)   # "Mon espace"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.00,  1.00,  6.00, None,  None,          None,          "1 · Sélectionnez votre profil"),
    ("B", 4.85,  5.15,  0.90, 5.00,  BTN_EMPLOYEE,  SZ_EMPLOYEE,   None),
    ("C", 5.30,  6.00,  3.50, None,  None,          None,          "2 · Saisissez votre code PIN"),
    ("D", 6.00,  9.55,  5.00, None,  None,          None,          "3 · Pointer ou Mon espace"),
    ("E", 9.55,  9.85,  0.90, 9.70,  BTN_MONESPACE, SZ_MONESPACE,  None),
    ("F", 11.00, 13.20, 7.00, None,  None,          None,          "4 · Modules selon le rôle"),
    ("G", 13.20, 15.557,4.50, None,  None,          None,          "Connecté en tant que Soulayma"),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (cut: scrolled to Soulayma's tile)
             "slideleft",  # B -> C (cut into the PIN screen)
             "fade",       # C -> D (continuous: typing the PIN)
             "fade",       # D -> E (continuous: click on Mon espace)
             "slideleft",  # E -> F (cut, Mon espace grid revealed)
             "fade",       # F -> G (continuous: scroll to top)
             "fade"]       # G -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.30,         # "chaque employe retrouve son profil sur la borne"
    "N2": S["B"] + 0.10,         # "il selectionne son profil, saisit son PIN"
    "N3": S["D"] + 0.20,         # "pointer, ou acceder a son espace"
    "N4": S["F"] + 0.20,         # "son espace affiche les modules selon le role"
    "N5": S["G"] + 0.20,         # "chaque connexion identifiee, pointage automatique"
...
FINAL = f"{ROOT}/out/foodeatup-borne-tuto-v1.mp4"
