#!/usr/bin/env python3
# FoodEatUp "Retrouver l'historique de mes relevés de température" tutorial.
#
# No avatar clip: full ElevenLabs VO throughout (same voice as the rest of
# the FoodEatUp tutorial series -- Adam - Instructor, TGAegA0zNRi8I6nUdq3i).
# Rush: Production > Temperatures (releves du jour) -> Historique (4 modules
# HACCP) -> Historique > Temperatures (dashboard, filtres, Equipements/Plats,
# export CSV). Cut a few frames before the raw footage shows a browser
# extension popup (not part of the product) triggered by the CSV download.
#
# Same engine as the rest of the series: setpts for speed (never zoompan on
# real footage), xfade on every cut forced back to yuv420p, 48kHz stereo AAC,
# +faststart.
...
ROOT = "/home/user/Video/videos/foodeatup-historique-temperature-tuto"
...
# No apostrophe in any caption below -- ffmpeg drawtext filter string breaks
# on it (bug hit on foodeatup-ingredients-tuto).
segs = [
    ("A", 0.00,  4.00,  3.00, "1 - Vos releves de temperature du jour"),
    ("B", 6.50,  8.50,  2.30, "2 - Ouvrez Historique HACCP"),
    ("C", 10.50, 13.00, 3.00, "3 - Historique par equipement et par plat"),
    ("D", 14.00, 16.00, 2.00, None),
    ("E", 16.00, 19.30, 3.00, "4 - Filtrez par statut et par date"),
    ("F", 19.30, 19.80, 1.00, None),
]
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- matching tool: list_haccp_temperatures
# (establishment_id, equipment_id?, type?, start_date?, end_date?). Prompt
# covers exactly what the rush shows: filtering the temperature history by
# equipment and by period.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Liste mes relevés de température HACCP pour l'équipement "
                  "[nom de l'équipement] entre le [date de début] et le "
                  "[date de fin], pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère votre historique de températures…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (cut into Historique landing)
             "slideleft",  # B -> C (cut into Temperatures dashboard)
             "fade",       # C -> D (continuous: Equipements -> Plats)
             "fade",       # D -> E (continuous: filters)
             "fade",       # E -> F (continuous: export click)
             "slideleft",  # F -> claude1
...
anchor = {
    ...
    "N5": S["E"] + 0.20,
    "N6": S["F"] + 0.10,
    "claude_reveal": S["claude1"] + 0.20,
    "claude_result": S["claude3"] + 0.20,
    "outro_cta": OUTRO_START + 0.35,
}
keys = ["N1", "N2", "N3", "N4", "N5", "N6", "claude_reveal", "claude_result", "outro_cta"]
off, prev_end = {}, -GAP
for k in keys:
...
FINAL = f"{ROOT}/out/foodeatup-historique-temperature-tuto-v1.mp4"
