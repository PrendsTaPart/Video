#!/usr/bin/env python3
# FoodEatUp "Commander et envoyer sa liste de courses au fournisseur" tutorial.
...
ROOT = "/home/user/Video/videos/foodeatup-commande-fournisseur-tuto"
...
# anchored on the click beat and continuing into the following informative
# segment (same pattern as foodeatup-tva-tuto / foodeatup-fournisseurs-tuto).
# The "Commander tous les produits ?" modal (segment C) has no zoom-punch:
# the rush doesn't show a clear confirm click on either button before the
...
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une commande auprès de mon fournisseur [nom du fournisseur] avec "
                  "les produits [liste des produits et quantités], livraison prévue le "
                  "[date de livraison], pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cette commande fournisseur pour votre établissement…"
...
# N6=4.41s / N7=4.49s measured -- same shape as foodeatup-tva-tuto's N6/N7
# (4.41 / 4.68s), reuse its stage timings unchanged (measure-VO-before-
# segments rule already satisfied by that video, durations are close enough
# that the shared default gives both stages comfortable room).
...
FINAL = f"{ROOT}/out/foodeatup-commande-fournisseur-tuto-v1.mp4"
