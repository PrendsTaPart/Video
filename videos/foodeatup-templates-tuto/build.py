#!/usr/bin/env python3
# FoodEatUp "Utiliser nos modeles FoodEatUp" tutorial (HACCP > Documents:
# ready-made document templates). No avatar clip: full ElevenLabs VO
...
# +faststart.
#
# NO "use it with Claude" sequence on this video: mcp__FoodEatUp__* has no
# tool for browsing/fetching static document templates (list_haccp_* tools
# are about actual HACCP records, not the Documents template library shown
# here). Per the standing rule (FOODEATUP-TUTORIELS-WORKFLOW.md), skip the
# sequence rather than invent a prompt.
#
# Segment targets sized against the FULL measured VO chain (N0-N5) BEFORE
# building, learning from the reception-stock-tuto build where segments
# were first sized too short and drifted up to 6s -- here each VO group's
# segment span is set >= its line's duration with margin from the start.
import subprocess, os

ROOT = "/home/user/Video/videos/foodeatup-templates-tuto"
...
BTN_DOCTILE = (745, 531);  SZ_DOCTILE = (330, 150)  # "Documents" tile (HACCP grid)
BTN_DOWNLOAD = (1650, 307); SZ_DOWNLOAD = (50, 50)   # download icon on the file row

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20, 1.20, 2.80, None, None,        None,       "1 . Module HACCP"),
    ("B", 1.30, 2.70, 4.00, None, None,        None,       None),
    ("C", 2.70, 2.95, 1.30, 2.90, BTN_DOCTILE, SZ_DOCTILE, None),
    ("D", 3.00, 4.90, 7.60, None, None,        None,       "2 . Dossiers de modeles predefinis"),
    ("E", 4.90, 5.20, 1.00, None, None,        None,       None),
    ("F", 5.50, 8.30, 3.80, None, None,        None,       "3 . Vos fiches et modeles"),
    ("G", 8.35, 8.55, 1.00, 8.45, BTN_DOWNLOAD, SZ_DOWNLOAD, None),
    ("H", 8.60, 10.70, 3.80, None, None,       None,       None),
    ("I", 11.00, 14.68, 3.00, None, None,      None,       None),
]
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: scroll to full HACCP grid)
             "fade",       # B -> C (continuous: click Documents tile)
             "slideleft",  # C -> D (cut into the Documents page)
             "fade",       # D -> E (continuous: click into Temperatures folder)
             "fade",       # E -> F (continuous: folder opens, file list)
             "fade",       # F -> G (continuous: click download icon)
             "fade",       # G -> H (continuous: download notification)
             "slideleft",  # H -> I (cut into the opened PDF preview)
             "fade"]       # I -> outro
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "ouvrez Documents"
    "N2": S["D"] + 0.20,         # dossiers predefinis
    "N3": S["E"] + 0.10,         # ouvrir un dossier -> liste des fichiers
    "N4": S["G"] + 0.10,         # telecharger un document
...
FINAL = f"{ROOT}/out/foodeatup-templates-tuto-v1.mp4"
