#!/usr/bin/env python3
# FoodEatUp "Ciblage et consentement clients" tutorial (module Marketing,
# Fidelite & Iris). Rush = continuous scroll through a read-only dashboard
# (no create/click forms), so no zoom-punch here -- plain full-frame
# segments + step banners. Speed = setpts (never zoompan on real footage).
# xfade on every cut, forced back to yuv420p at the end of the chain.
# 48kHz stereo AAC, +faststart.
#
# Claude sequence: mcp__Foodeatup__list_rfm_segments matches the "Segments
# dynamiques" panel (read-only consultation prompt). The consent/STOP panel
# has no MCP write-tool equivalent (update_client only exposes a generic
# Actif/Inactif/Suspendu status) -- documented in howItWorks/chefTip only,
# not fabricated into a prompt (see SCRIPT.md).
...
ROOT = "/home/user/Video/videos/foodeatup-ciblage-consentement-tuto"
...
# (name, src_start, src_end, target_out_duration, caption) -- read-only
# scroll tour, timestamps read off work/frames/f_*.png (1 fps extraction).
segs = [
    ("A", 0.20,  4.00,  5.73, "1 - Ouvrir Segments et consentements"),
    ("B", 4.00,  6.50,  9.25, "2 - Segments calcules automatiquement"),
    ("C", 6.50,  9.20,  6.48, "3 - Plafond mensuel par client"),
    ("D", 9.50,  19.00, 7.97, "4 - Canaux et STOP par contact"),
    ("E", 20.50, 28.24, 8.70, "Conformite RGPD garantie"),
]
INTRO_D, OUTRO_D = 5.20, 5.55
...
# text changes. Featured prompt = list_rfm_segments (read-only, matches the
# "Segments dynamiques" panel -- consultation, not creation).
CLAUDE_PROMPT = ("Quels segments RFM puis-je cibler pour ma prochaine campagne sur mon "
                  "établissement FoodEatUp (ID [ID établissement]), avec le nombre de "
                  "clients et le pourquoi de chaque segment ?")
CLAUDE_RESPONSE = "Bien sûr ! Voici vos segments RFM avec leurs critères et compteurs..."
# N6 (4.41s, identique au reste de la série) doit tenir sur stage1+2 ; N7
# (7.37s, plus long que la moyenne -- réponse plus détaillée) doit tenir sur
# stage3 seul. Élargi vs. le défaut du module ([2.20, 1.30, 2.50]).
CLAUDE_STAGE_D = [3.35, 2.15, 8.00]
...
def build_silent(outro_d):
    ...
    trans = ["fade",        # intro -> A
             "slideleft",   # A -> B (onglet Segments et consentements -> contenu)
             "fade",        # B -> C (defilement continu, meme ecran)
             "slideleft",   # C -> D (defilement vers le tableau contacts)
             "fade",        # D -> E (defilement vers la mention RGPD/journal)
             "slideleft",   # E -> claude1 (scene cut)
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.15,         # navigation + clic onglet
    "N2": S["B"] + 0.15,         # segments dynamiques
    "N3": S["C"] + 0.15,         # plafond mensuel
    "N4": S["D"] + 0.15,         # contacts, canaux, STOP
    "N5": S["E"] + 0.15,         # RGPD + journal des envois
...
FINAL = f"{ROOT}/out/foodeatup-ciblage-consentement-tuto-v1.mp4"
