#!/usr/bin/env python3
# FoodEatUp "Commander par QR code" tutorial.
#
# Rush covers the full customer self-order loop: staff reveals a table's QR
# code from the floor plan, the client scans it, browses the menu, adds
# items, sends the order to the kitchen, then pays the bill by card. No
# avatar clip (same choice as foodeatup-qrcode-tuto -- single consistent
# ElevenLabs voice across the series). Claude sequence matches the one
# concrete server-side action: create_order (table order -> invoice/quote).
#
# Same engine as the rest of the series: setpts for speed (never zoompan on
# real footage), fixed crop+scale zoom-punch on the one final click (Payer),
# banner() uses the single-clamp slide-in fix (drawbox's expression
# evaluator on ffmpeg 6.1.1 silently fails -- box never draws -- when x
# combines a leading constant offset with TWO subtracted min/max clamps).
...
ROOT = "/home/user/Video/videos/foodeatup-commander-qrcode-tuto"
...
def banner(text, seg_dur):
    ...
    # Single clamp only (slide-in) -- see module docstring. The crossfade
    # into the next segment hides the exit instead of animating a slide-out.
    tin, sl = 0.15, 0.32
...
# Coordinates measured on full-res frames (1920x828) around each moment.
BTN_PAY = (949, 603); SZ_PAY = (350, 46)  # "Payer 10,20 €" (final card payment)

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  14.00, 24.60, 5.80, None,    None,   "Plan de salle : selectionnez la table"),
    ("B",  24.60, 31.50, 3.00, None,    None,   "QR code genere pour la table"),
    ("C",  31.50, 38.50, 5.80, None,    None,   "Le client scanne le QR code"),
    ("D",  38.50, 43.70, 5.20, None,    None,   "Ajout au panier en un clic"),
    ("E",  43.70, 46.50, 4.80, None,    None,   "Commande envoyee en cuisine"),
    ("F1", 46.50, 70.80, 6.40, None,    None,   "Paiement securise par carte"),
    ("F2", 70.80, 71.60, 0.80, BTN_PAY, SZ_PAY, None),
]
...
# ---------------------------------------------------------------------------
# "Use it with Claude" sequence -- shared 3-stage chatbot animation. The one
# server-side action with a real MCP equivalent: create_order(establishment_id,
# items[], table_id, service_mode). QR reveal / scan / floor-plan status are
# client-side or staff-UI actions without a direct MCP call -- no invented
# prompt for those (consistent with qrcode-tuto's treatment of QR/flyers/cards).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée une commande sur place pour la table [numéro de table] avec "
                  "[plat 1] x[quantité], [plat 2] x[quantité], pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée votre commande…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (floor plan -> QR modal)
             "fade",       # B -> C (QR reveal -> scan)
             "slideleft",  # C -> D (scan -> menu)
             "fade",       # D -> E (add to cart -> sent)
             "slideleft",  # E -> F1 (sent -> payment)
             "fade",       # F1 -> F2 (continuous: click on the visible button)
             "slideleft",  # F2 -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]  + 0.20,   # each table has its own QR code
    "N2": S["C"]  + 0.20,   # scan with phone -> menu opens
    "N3": S["D"]  + 0.20,   # browse + add to cart
    "N4": S["E"]  + 0.20,   # sent straight to kitchen
    "N5": S["F1"] + 0.20,   # pay the bill by card
...
FINAL = f"{ROOT}/out/foodeatup-commander-qrcode-tuto-v1.mp4"
