#!/usr/bin/env python3
# FoodEatUp "Retrouver l'historique de ses productions" tutorial (module HACCP,
# onglet Historique > Production). No avatar clip: full ElevenLabs VO
# throughout. Speed = setpts (never zoompan on real footage). xfade on every
# cut, forced back to yuv420p at the end of the chain. 48kHz stereo AAC,
# +faststart.
#
# Rush (16.48s) is a pure consultation screen -- no form to fill, a single
# click (the "Production" card from the Historique overview). Everything
# after that click is read-only: 6 KPIs, a 6-month trend chart, then a
# searchable/filterable/exportable list of past productions.
...
ROOT = "/home/user/Video/videos/foodeatup-historique-productions-tuto"
...
# Coordinate measured by colour-thresholding the "Production" card border
# (RGB 27,109,243, the same blue as every card outline) on the real frame --
# see work/frames/ (extracted from the rush at t=0.75s, before any hover
# state). Card centre (1620, 657), size (506, 264) -- much bigger than the
# small buttons on other tutorials because this is a whole dashboard card,
# not a form button.
BTN_PRODUCTION = (1620, 657); SZ_PRODUCTION = (506, 264)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A",    0.00, 4.10, 4.20, None, None,           None,           "Historique > Production"),
    ("clic", 4.10, 4.40, 0.90, 4.25, BTN_PRODUCTION, SZ_PRODUCTION,  None),
    ("C",    5.20, 7.20, 9.50, None, None,           None,           "Vos indicateurs clés"),
    ("D",    7.20, 9.50, 4.50, None, None,           None,           "Tendance des 6 derniers mois"),
    ("E",    9.50, 16.40, 9.50, None, None,          None,           "Recherche, période, export CSV"),
]
# Durations sized against measured VO length (N0..N8: 3.94/3.24/5.75/3.84/
# 4.73/3.60/4.41/4.62/5.02s) -- a first pass with tighter targets caused N2
# (5.75s, the longest line) to spill ~2.7s into segment D. Rule from
# foodeatup-tva-tuto/SCRIPT.md: measure the VO before fixing segment
# durations, not after.
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__list_top_productions
# (establishment_id, days?) is the matching tool -- read-only, exactly what
# this screen shows (no create/edit action appears in the rush).
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Montre-moi l'historique des productions des [nombre] derniers "
                  "jours pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Voici l'historique de vos productions…"
...
# Same override as produits-tuto/vitrine-tuto (vs. shared default
# [2.20,1.30,2.50]): N6 (reused, 4.41s) and N7 (4.62s) both need real room to
# land on their own stage instead of drifting onto the next one.
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # clic -> C (cut into the opened dashboard)
             "slideleft",  # C -> D (KPIs -> chart)
             "slideleft",  # D -> E (chart -> list/tools)
             "slideleft",  # E -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["clic"] + 0.10,      # "cliquez sur Production"
    "N2": S["C"] + 0.20,         # 6 KPIs
    "N3": S["D"] + 0.20,         # tendance des 6 derniers mois
    "N4": S["E"] + 0.10,         # recherche / periode / export CSV
    "N5": S["E"] + 4.40,         # benefice (repere best-sellers) -- same segment, later
...
FINAL = f"{ROOT}/out/foodeatup-historique-productions-tuto-v1.mp4"
