#!/usr/bin/env python3
# FoodEatUp "Deduire ses besoins de production" tutorial (StockVision AI, 1er
# tuto du module). No avatar clip: full ElevenLabs VO throughout. Speed =
...
#
# Rush: "Ma carte" (badge Ingredients manquants) -> clic oeil sur "Pizza
# Margherita napolitaine" -> fiche plat (Prix/Portions/Temps/Chef) -> tableau
# "Ingredients requis" (Tomate/Farine Suffisant, Fromage tranches Manquant) ->
# clic "Ajouter a la liste des courses" -> ecran "Liste des courses" (Fromage
# tranches ajoute sous Fournisseur non defini, aux cotes de Lait/Tomates/
# Sauce soja/Algues nori/Avocat deja presents).
...
ROOT = "/home/user/Video/videos/foodeatup-besoins-production-tuto"
...
# work/frames/eye_icons_zoom.png + numpy bbox on add_001.jpg). No apostrophe
# in any caption below (bug hit on foodeatup-ingredients-tuto -- see
# FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_EYE = (868, 54);   SZ_EYE = (56, 56)     # icone oeil, carte Pizza Margherita
BTN_ADD = (1033, 784); SZ_ADD = (379, 64)    # "Ajouter a la liste des courses"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20, 4.00, 3.00, None, None,    None,   "1 - Ma carte : ingredients manquants"),
    ("B", 4.00, 4.30, 0.90, 4.15, BTN_EYE, SZ_EYE, None),
    ("C", 4.30, 5.20, 2.00, None, None,    None,   "2 - Detail du plat"),
    ("D", 5.20, 7.60, 4.50, None, None,    None,   "3 - Suffisant ou manquant ?"),
    ("E", 7.60, 7.90, 0.90, 7.75, BTN_ADD, SZ_ADD, None),
    ("F", 7.90, 28.24, 6.00, None, None,   None,   "Ajoute a la liste des courses"),
]
...
# (videos/_shared/claude_prompt_sequence.py). Two matching MCP tools:
# create_production_plan(establishment_id, item_id, planned_quantity,
# planned_date) covers the "Qte a produire" field seen on each dish card;
# get_production_ingredients(establishment_id, production_id) covers the
# Suffisant/Manquant table. No MCP tool exposes "Ajouter a la liste des
# courses" itself (checked against the FoodEatUp tool list), so that precise
# click has no Claude equivalent in the prompt below.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Planifie une production de [quantite] [nom du plat] pour le "
                  "[date], puis indique-moi les ingredients manquants pour mon "
                  "etablissement FoodEatUp (ID [ID etablissement]).")
CLAUDE_RESPONSE = "Bien sur ! Je verifie vos ingredients manquants…"
...
# Same durations as foodeatup-tva-tuto (validated pattern): reveal / copied /
# chatbot mockup, sized for N6 (reused, 4.41s) and N7 (4.49s, close to tva's
# 4.68s).
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C -> D (continuous: scroll to the table)
             "fade",       # D -> E (continuous: click Ajouter)
             "slideleft",  # E -> F (submit -> liste des courses)
             "slideleft",  # F -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["A"] + 0.10,         # "reperez le badge Ingredients manquants"
    "N2": S["B"] + 0.10,         # "ouvrez la fiche d'un plat" (click + header)
    "N3": S["D"] + 0.20,         # tableau Suffisant/Manquant
    "N4": S["E"] + 0.10,         # "cliquez sur Ajouter a la liste des courses"
    "N5": S["F"] + 0.20,         # resultat (liste de courses)
...
FINAL = f"{ROOT}/out/foodeatup-besoins-production-tuto-v1.mp4"
