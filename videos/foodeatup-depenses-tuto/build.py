#!/usr/bin/env python3
# FoodEatUp "Tenir ses dépenses (StockVision AI)" tutorial.
# Rush shows the StockVision AI OCR path: import a supplier invoice from a
# delivery, let the AI extract the line items, review, then "Valider et
# enregistrer" auto-creates the expense in accounting -- no manual item entry
# UI appears on screen. create_expense's own MCP schema takes structured
# items (name/quantity/unit_price) directly, not an image/OCR pipeline, so
# the Claude prompt below models the manual/direct equivalent (same pattern
# as the ingredients/produits tutorials' "photo facture" vs direct prompts).
# No update_expense / delete_expense tool exists -- only create_expense,
# list_expenses, get_expense -- so no edit/delete prompt is fabricated here.
...
ROOT = "/home/user/Video/videos/foodeatup-depenses-tuto"
...
# work/frames*/), not eyeballed. No apostrophe in any caption below (bug hit
# on foodeatup-ingredients-tuto -- see FOODEATUP-TUTORIELS-WORKFLOW.md).
BTN_ADD_FACTURE = (1587, 496); SZ_ADD_FACTURE = (267, 62)  # "+ Ajouter une facture"
BTN_VALIDER     = (1538, 554); SZ_VALIDER     = (361, 64)  # "Valider et enregistrer"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  4.00,  2.60, None, None,            None,           "1 · Vos livraisons"),
    ("B", 4.00,  6.75,  2.00, None, None,            None,           "2 · Facture fournisseur"),
    ("P1", 6.75, 7.05,  0.90, 6.90, BTN_ADD_FACTURE, SZ_ADD_FACTURE, None),
    ("C", 7.05,  8.75,  1.80, None, None,            None,           "3 · Importer la facture"),
    ("D", 8.75,  16.50, 4.60, None, None,            None,           "4 · Analyse StockVision AI"),
    ("E", 16.50, 20.85, 3.80, None, None,            None,           "5 · Vérification des produits"),
    ("P2", 20.85, 21.15, 0.90, 21.00, BTN_VALIDER,   SZ_VALIDER,     None),
    ("F", 21.15, 22.75, 1.60, None, None,            None,           None),
    ("G", 22.75, 32.16, 5.80, None, None,            None,           "6 · Dépense enregistrée"),
]
INTRO_D, OUTRO_D = 3.00, 6.00
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__create_expense
# (establishment_id, items[] {name, quantity, unit_price, tax_rate?}, category?,
# supplier_id?, ...) is the matching tool -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Enregistre une dépense pour mon établissement FoodEatUp "
                  "(ID [ID établissement]) : [produit] x[quantité] à [prix unitaire] "
                  "euros HT, TVA [taux]%, auprès du fournisseur [nom du fournisseur].")
CLAUDE_RESPONSE = "Bien sûr ! J'enregistre cette dépense dans votre comptabilité…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> B (continuous: navigate to detail then scroll)
             "fade",       # B -> P1 (continuous: click on the visible button)
             "slideleft",  # P1 -> C (cut into the opened modal)
             "slideleft",  # C -> D (cut: file chosen -> analysis running)
             "slideleft",  # D -> E (cut: analysis done -> validation page)
             "fade",       # E -> P2 (continuous: click Valider)
             "fade",       # P2 -> F (submit -> success)
             "slideleft",  # F -> G (voir la depense -> detail page)
             "slideleft",  # G -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "depuis une livraison, importez la facture"
    "N2": S["D"] + 0.20,         # AI analysis running
    "N3": S["E"] + 0.20,         # review detected products
    "N4": S["P2"] + 0.10,        # "cliquez sur Valider et enregistrer"
    "N5": S["G"] + 0.20,         # benefit (full detail in accounting)
...
FINAL = f"{ROOT}/out/foodeatup-depenses-tuto-v1.mp4"
