#!/usr/bin/env python3
# FoodEatUp "Roles et permissions -- controler le pointage (pauses & photo)"
# tutorial.
#
# Drive folder 15 ("Gestion des pauses, pointage entree et sortie et
# Empreinte photo du pointage") but the rush actually records the ADMIN-side
# Roles/Permissions screen (Equipe > Roles > Modifier le role), not the
# employee punch-clock UI. Per Michael's explicit direction, the video is
# framed from the boss/chef/director point of view: what pointage (clock
# in/out, breaks, photo) permissions a role is granted, contrasted with a
# sensitive module (HACCP) a Manager does NOT have.
#
# No avatar. Same engine as the rest of the series: setpts for speed (never
# zoompan on real footage), fixed crop+scale zoom-punch on the two permission
# rows, xfade on every cut forced back to yuv420p, 48kHz stereo AAC,
# +faststart.
...
ROOT = "/home/user/Video/videos/foodeatup-roles-permissions-tuto"
...
# Coordinates measured on full-res (1920x828) frames around t=32.0-33.0s,
# where the "Modifier le role" scroll is essentially frozen (mouse idle).
BTN_POINTAGES = (960, 290); SZ_POINTAGES = (1140, 48)  # "Pointages (7/7)" row
BTN_HACCP     = (960, 520); SZ_HACCP     = (1140, 48)  # "HACCP (0/41)" row

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  0.00,  5.00, 6.00, None,          None,         "Mon espace — vue Manager"),
    ("B",  15.00, 18.00, 7.50, None,          None,         "QR Code de pointage"),
    ("C1", 19.00, 21.30, 8.30, None,          None,         "Rôles : Admin & Manager"),
    ("C2", 22.00, 26.00, 8.90, None,          None,         "Modifier le rôle Manager"),
    ("D1", 32.50, 33.10, 7.60, BTN_POINTAGES, SZ_POINTAGES, "Pointages : 7 / 7"),
    ("D2", 32.50, 33.10, 7.60, BTN_HACCP,     SZ_HACCP,     "HACCP : 0 / 41"),
]
...
# tool covers editing a role's permissions itself (same class of action as
# creer-son-compte / regler-ses-unites -- security-sensitive admin config),
# but the director can ask Claude directly about attendance/break data via
# list_attendances(establishment_id, date_from, date_to) -- same tool used
# in retrouver-les-pointages-historique, reframed here for break oversight.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Donne-moi le détail des pauses de [prénom] [nom] du [date "
                  "début] au [date fin], pour mon établissement FoodEatUp "
                  "(ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je récupère ses pointages et je vous détaille ses pauses…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "slideleft",  # A -> B (mon espace -> QR code page)
             "slideleft",  # B -> C1 (QR page -> roles listing)
             "fade",       # C1 -> C2 (open the Manager role modal)
             "fade",       # C2 -> D1 (same modal, scrolled to Pointages)
             "fade",       # D1 -> D2 (same modal, scrolled to HACCP)
             "slideleft",  # D2 -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]  + 0.20,  # mon espace vue manager
    "N2": S["B"]  + 0.20,  # QR code pointage employe
    "N3": S["C1"] + 0.20,  # roles Admin/Manager
    "N4": S["C2"] + 0.20,  # modifier un role
    "N5": S["D1"] + 0.20,  # Pointages 7/7 puis HACCP 0/41 (une ligne, 2 plans)
    "N6": S["claude1"] + 0.20,  # explique le prompt (reveal + copied)
...
FINAL = f"{ROOT}/out/foodeatup-roles-permissions-tuto-v1.mp4"
