#!/usr/bin/env python3
# FoodEatUp "Monter ses recettes / fiches techniques" tutorial (module Configuration).
...
#
# Rush (93.1s): "Mes recettes (0)" -> "Créer une recette" -> upload d'une photo de
# plat -> "Générer avec l'IA" (nom / difficulté / description auto-remplis depuis
# la photo) -> section "Ingrédients et Quantités" -> ajout de 4 ingrédients
# (Sushi riz, Nori, Thon au naturel, Mayonnaise) avec cout total recalculé en
# direct à chaque ajout. Le rush s'arrête avant la saisie du prix de vente et le
# clic "Enregistrer la recette" (jamais filmé) -- le montage se termine donc sur
# le tableau d'ingrédients avec le cout total, pas sur un écran de succès.
#
# Matches mcp__FoodEatUp__create_recipe(establishment_id, name, ingredients[],
# steps[], category_names?, dificulty?, how_many_people?, recommended_price?,
# tax_rate?) -- les ingrédients inconnus sont créés automatiquement par l'outil.
...
ROOT = "/home/user/Video/videos/foodeatup-recettes-tuto"
...
BTN_CREATE_RECIPE = (1682, 344); SZ_CREATE_RECIPE = (225, 55)  # "Creer une recette"
BTN_GENERATE_IA   = (535, 146);  SZ_GENERATE_IA   = (230, 44)  # "Generer avec l'IA"

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.00,  2.00, None,  None,               None,             "1 · Mes recettes"),
    ("B", 2.00,  2.30,  0.80, 2.10,  BTN_CREATE_RECIPE,  SZ_CREATE_RECIPE, None),
    ("C", 4.00,  6.50,  2.50, None,  None,               None,             "2 · Photo de la recette"),
    ("D", 7.00,  7.30,  0.80, 7.15,  BTN_GENERATE_IA,    SZ_GENERATE_IA,   None),
    ("E", 8.00,  16.00, 5.00, None,  None,               None,             "3 · Champs generes par IA"),
    ("F", 20.00, 28.00, 3.00, None,  None,               None,             "4 · Ingrédients et quantités"),
    ("G", 28.00, 68.00, 7.00, None,  None,               None,             None),
    ("H", 88.00, 93.00, 4.00, None,  None,               None,             "Coût total en temps réel"),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__create_recipe
# is the matching tool -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée la recette [nom de la recette], difficulté [facile / moyen / "
                  "difficile], pour [nombre] personnes, avec les ingrédients "
                  "[ingrédient 1] [quantité] [unité], [ingrédient 2] [quantité] "
                  "[unité], et les étapes [étape 1], [étape 2], pour mon "
                  "établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée cette recette pour votre établissement…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "fade",       # C -> D (continuous: click Generer avec l'IA)
             "slideleft",  # D -> E (cut, fields revealed by the AI)
             "slideleft",  # E -> F (cut, scroll to ingredients section)
             "fade",       # F -> G (continuous: adding ingredients)
             "fade",       # G -> H (continuous: final cost update)
             "slideleft",  # H -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # click Creer une recette
    "N2": S["C"] + 0.20,         # photo de la recette
    "N3": S["D"] + 0.10,         # click Generer avec l'IA
    "N4": S["E"] + 0.30,         # champs generes par l'IA
    "N5": S["F"] + 0.20,         # section ingredients
    "N6": S["G"] + 0.30,         # cout calcule en temps reel
    "N7": S["H"] + 0.20,         # benefice (relie produits/carte)
...
FINAL = f"{ROOT}/out/foodeatup-recettes-tuto-v1.mp4"
