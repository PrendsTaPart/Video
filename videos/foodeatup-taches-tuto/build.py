#!/usr/bin/env python3
# FoodEatUp "Assigner les tâches sur le planning" tutorial (module Équipe & Planning).
...
#
# Rush (85s): planning équipe -> "Nouvelle tâche" -> formulaire (intitulé, employé,
# date, heure, priorité, catégorie, récurrence) -> "Assigner" -> tâche dans le
# planning -> crayon "Modifier" -> changement de récurrence -> "Enregistrer" ->
# cocher la tâche terminée. La partie "Publier le planning" (shifts, pas tâches)
# n'est pas reprise : hors sujet pour cette vidéo centrée sur les tâches. Matches
...
ROOT = "/home/user/Video/videos/foodeatup-taches-tuto"
...
BTN_NEW_TASK = (1644, 540); SZ_NEW_TASK = (230, 53)  # "Nouvelle tache" (liste)
BTN_ASSIGN   = (1039, 715); SZ_ASSIGN   = (142, 56)  # "Assigner" (creation)
BTN_EDIT     = (1664, 697); SZ_EDIT     = (30, 30)   # crayon "Modifier" (ligne)
BTN_SAVE     = (1118, 715); SZ_SAVE     = (140, 56)  # "Enregistrer" (edition)
BTN_DONE     = (142, 693);  SZ_DONE     = (34, 34)   # case a cocher "terminee"
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__assign_task
# (establishment_id, professional_id, name, category?, date?, time?, priority?)
# is the matching tool -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Assigne la tâche de nettoyage [nom de la tâche] à [prénom] [nom] "
                  "(ID employé [ID employé]), le [date] à [heure], priorité "
                  "[priorité], pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! J'assigne cette tâche à votre employé…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # C -> D (cut, identite -> priorite/categorie/recurrence)
             "fade",       # D -> E (continuous: click Assigner)
             "fade",       # E -> F (submit -> success/list)
             "fade",       # F -> G (continuous: click crayon Modifier)
             "slideleft",  # G -> H (cut into the edit modal)
             "fade",       # H -> I (continuous: click Enregistrer)
             "fade",       # I -> J (continuous: click case a cocher)
             "fade",       # J -> K (cochee -> terminee)
             "slideleft",  # K -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Nouvelle tache"
    "N2": S["C"] + 0.20,         # intitule / employe / date
    "N3": S["D"] + 0.20,         # heure / priorite / categorie / recurrence
    "N4": S["E"] + 0.10,         # "cliquez sur Assigner" -> succes
    "N5": S["G"] + 0.10,         # "vous pouvez la modifier" (clic crayon)
    "N6": S["I"] + 0.10,         # "enregistrer, puis cocher terminee"
...
FINAL = f"{ROOT}/out/foodeatup-taches-tuto-v1.mp4"
