#!/usr/bin/env python3
# FoodEatUp "Mes depenses fournisseur" (Comptabilite) tutorial.
#
# Rush (79.3s), Drive folder "7- Mes depenses fournisseur" -- confirmed
# matching file size on Google Drive. Covers: Depenses list -> "Creer une
# depense" -> date d'achat/reference facture/fournisseur/joindre la facture
# -> chercher un produit (auto prix) -> quantite/description -> categorie +
# statut -> Enregistrer -> back to list with the new expense. The rush also
# shows viewing/deleting the expense afterwards, out of scope here (video
# stays focused on creation, same pattern as foodeatup-creer-devis-tuto).
#
...
ROOT = "/home/user/Video/videos/foodeatup-depenses-fournisseur-tuto"
...
BTN_CREER_DEPENSE = (1672, 329);  SZ_CREER_DEPENSE = (215, 50)   # "Créer une dépense" (list page)
BTN_GOSUSHI       = (1384, 438);  SZ_GOSUSHI       = (720, 55)   # autocomplete option "Plateau GoSushi 28 pcs"
BTN_LISTE_NEW     = (1213, 480);  SZ_LISTE_NEW     = (1590, 55)  # nouvelle ligne #D20260002 dans la liste

# (name, src_start, src_end, target_out_duration, button, btn_size, caption)
segs = [
    ("A",  0.00,  4.00, 7.00, None,              None,             "Liste de vos dépenses"),
    ("A2", 6.00,  6.80, 4.00, BTN_CREER_DEPENSE, SZ_CREER_DEPENSE, "Créer une dépense"),
    ("B",  8.00, 11.00, 7.00, None,              None,             "Achat, facture, fournisseur"),
    ("C",  32.60, 33.40, 6.00, BTN_GOSUSHI,      SZ_GOSUSHI,       "Ajouter un produit"),
    ("D",  40.00, 42.00, 6.50, None,             None,             "Quantité & description auto"),
    ("E",  47.00, 49.00, 7.00, None,             None,             "Catégorie & statut"),
    ("F",  58.00, 58.40, 6.00, BTN_LISTE_NEW,    SZ_LISTE_NEW,     "Dépense enregistrée"),
]
...
# match: create_expense(establishment_id, items[], supplier_id,
# supplier_invoice_reference, purchase_date, category, status,
# payment_method) -- same fields filled on screen. No MCP field for the
# uploaded invoice file itself, so the prompt stays scoped to the data.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Enregistre une dépense chez le fournisseur [nom du "
                  "fournisseur], référence facture [référence], avec "
                  "[quantité] [nom du produit] à [prix]€ HT, catégorie "
                  "[catégorie], statut [payée / en attente / déclinée], "
                  "pour mon établissement FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! J'enregistre cette dépense tout de suite…"
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             "fade",       # A -> A2 (click Créer une dépense)
             "slideleft",  # A2 -> B (form opens)
             "fade",       # B -> C (continuous: search a product)
             "fade",       # C -> D (product added, qty/description)
             "fade",       # D -> E (catégorie/statut)
             "slideleft",  # E -> F (Enregistrer + retour liste)
             "slideleft",  # F -> claude1
...
GAP = 0.22
anchor = {
    "N1": S["A"]   + 0.20,  # liste des dépenses
    "N2": S["A2"]  + 0.10,  # clic Créer une dépense
    "N3": S["B"]   + 0.20,  # achat/facture/fournisseur
    "N4": S["C"]   + 0.20,  # chercher un produit
    "N5": S["D"]   + 0.20,  # quantité/description/total
    "N6": S["E"]   + 0.20,  # catégorie/statut
    "N7": S["F"]   + 0.10,  # enregistrer + retour liste
...
FINAL = f"{ROOT}/out/foodeatup-depenses-fournisseur-tuto-v1.mp4"
