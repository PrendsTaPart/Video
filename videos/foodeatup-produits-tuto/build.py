#!/usr/bin/env python3
# FoodEatUp "Créer ses produits" tutorial.
...
#
# Rush shows a single, straightforward "ready-made product" creation (pâte de
# piment fermentée) -- no recipe/ingredient affiliation UI appears on screen
# (create_product's own MCP schema doesn't expose that either -- categories,
# quantity, TVA, price only). Michael explained the recipe-affiliation /
# shopping-list logic (produit avec recette -> liste de courses ajoute les
# ingrédients ; produit sans recette -> liste de courses ajoute le produit
# lui-même) as context for the chef tip on Lovable, not as something to fake
# on screen here.
...
ROOT = "/home/user/Video/videos/foodeatup-produits-tuto"
...
BTN_NEW  = (1671, 306); SZ_NEW  = (235, 65)  # "Nouveau produit"
BTN_ADD  = (1683, 605); SZ_ADD  = (119, 59)  # "Ajouter" (modal submit)

# (name, src_start, src_end, target_out_duration, click_time_or_None, button, btn_size, caption)
segs = [
    ("A", 0.20,  2.00,  2.50, None, None,    None,    "1 · Vos produits"),
    ("B", 2.00,  2.35,  0.90, 2.30, BTN_NEW, SZ_NEW,  None),
    ("C", 3.00,  24.00, 6.00, None, None,    None,    "2 · Photo, nom et description"),
    ("D", 24.00, 44.00, 6.00, None, None,    None,    "3 · Catégorie, quantité, TVA, prix"),
    ("E", 44.00, 44.35, 0.90, 44.20, BTN_ADD, SZ_ADD, None),
    ("F", 44.50, 59.60, 6.00, None, None,    None,    "Produit ajouté"),
]
...
# (videos/_shared/claude_prompt_sequence.py). mcp__FoodEatUp__create_product
# (establishment_id, name, price_by_unit, unit, stock_quantity?, description?,
# is_promotion?) is the matching tool -- fields checked against the schema.
# ---------------------------------------------------------------------------
CLAUDE_PROMPT = ("Crée le produit [nom], prix HT [prix]€, unité [unité], "
                  "quantité en stock [quantité], pour mon établissement "
                  "FoodEatUp (ID [ID établissement]).")
CLAUDE_RESPONSE = "Bien sûr ! Je crée ce produit pour votre établissement…"
...
# Sized generously (vs. shared default [2.20,1.30,2.50]): N5's benefit line
# and N6's Claude explanation both drift a bit past their own click/segment
# (established acceptable pattern -- see foodeatup-ingredients-tuto/SCRIPT.md
# for why forcing zero drift everywhere can force unnatural slow-motion).
# Verified by simulation: N6 lands within claude1->claude2, N7 within
# claude3.
...
def build_silent(outro_d):
    ...
    trans = ["fade",       # intro -> A
             ...
             "slideleft",  # C -> D (cut, photo/nom/description -> catégorie/prix)
             "fade",       # D -> E (continuous: click Ajouter)
             "fade",       # E -> F (submit -> success/list)
             "slideleft",  # F -> claude1
...
anchor = {
    "N0": 0.30,                  # intro hook
    "N1": S["B"] + 0.10,         # "cliquez sur Nouveau produit"
    "N2": S["C"] + 0.20,         # photo / nom / description
    "N3": S["D"] + 0.20,         # catégorie / quantité / TVA / prix
    "N4": S["E"] + 0.10,         # "cliquez sur Ajouter" -> succès
    "N5": S["F"] + 0.20,         # bénéfice (liste de courses selon stock/recettes)
...
FINAL = f"{ROOT}/out/foodeatup-produits-tuto-v1.mp4"
