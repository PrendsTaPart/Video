...
/** Un post prêt à partir : il ne manque que la vidéo à importer. */
export type Publication = {
  statut: StatutReseau;
  /** Date de diffusion, réelle si planifiée, sinon celle du calendrier. */
  date: string | null;
  heure: string;
  compte: string;
  format: string;
  /** Le texte à coller, sans les mots-dièse. */
  legende: string;
  hashtags: string[];
  cta: string;
  ...
  lienCta: string | null;
  motsCles: string[];
  /** Permalien du post, une fois en ligne. */
  url?: string;
};
...
export type Episode = {
  ...
  /** Image extraite du master. null = pas encore monté, on affiche le teasing. */
  posterUrl: string | null;
  /** Date du calendrier éditorial. null si l'épisode est bloqué. */
            ...
            "reseaux": {
              "facebook": {
                ...
                "motsCles": [
                  ...
                  "configuration logiciel restaurant",
                  "vue d'ensemble",
          ...
          },
          {
            ...
            "reseaux": {
              "facebook": {
                ...
                "motsCles": [
                  "prévision de ventes restaurant",
                  "IA restaurant",
                  "pilotage",
                  "lire ses prévisions",
                  "predibot restaurant"
          ...
          },
          {
            ...
            "reseaux": {
              "facebook": {
                ...
                "motsCles": [
                  "logiciel de réservation restaurant",
                  "plan de salle",
                  "no-show",
...
export const publicationsDuJour = (iso: string) => {
  const out: { episode: Episode; reseau: Reseau; publication: Publication }[] = [];
  for (const e of tousLesEpisodes())
    for (const r of Object.keys(e.reseaux) as Reseau[])
      if (e.reseaux[r].date === iso) out.push({ episode: e, reseau: r, publication: e.reseaux[r] });
  return out.sort((a, b) => a.publication.heure.localeCompare(b.publication.heure));
};

/** Le texte complet prêt à coller : légende + ligne de mots-dièse. */
export const texteAColler = (p: Publication) =>
