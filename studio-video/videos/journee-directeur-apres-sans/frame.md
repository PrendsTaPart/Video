...
colors:
  ...
  bg-secondary: "#E4E5E8"
  text-primary: "#3A3F45"
  text-secondary: "rgba(58,63,69,0.70)"
  accent: "rgba(58,63,69,0.70)"
  line: "#8A9099"
  alert: "#D64545"
  white-overlay: "rgba(58,63,69,0.05)"
...
typography:
  ...
  # — display / hero ramp (Inter 400, sentence case) —
...
- **Fonts** — already set to **Inter** (display) / **Inter** (body); ignore any preset font name lingering in prose (e.g. "Playfair" mentions below are the preset's original serif — read as Inter, sentence case, weight 400).
...
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream", "taupe") mean the remapped brand values below. **This project HAND-CORRECTS the automatic remix** (legally mandated exact palette — see project `BRIEF.md`): the auto-remix initially put the brand red on the structural `line` role and a stray off-palette pink on `bg-secondary`; both were fixed by hand. Current semantics:
  - `line` = the brand **gray** `#8A9099` — every 1px hairline / structural separator / broken-line segment. **Never used for body text** (fails AA on the light ground) — decoration only.
  - `accent` / `text-secondary` = a **muted anthracite** (`rgba(58,63,69,0.70)`) — chrome labels, numerals, secondary reading text. Deliberately NOT the brand red (the red stays scarce) and NOT the raw gray (AA contrast).
  - `alert` (new key, not in the original preset) = the brand red `#D64545` — the ONE functional accent, used ONLY for the visual grammar's friction markers: red crosses / empty-state marks, the loss counter, alert states. Never used as a default chrome or label color, never decorative.
  - `white-overlay` repointed to a faint anthracite tint (`rgba(58,63,69,0.05)`) so cards read distinctly on the near-white `bg-primary` (the original warm-stone version relied on a darker canvas).

### Fonts (files — required `@font-face`, one shipped family only)

Only **Inter** ships as a file in this project, at `assets/fonts/`. Every frame worker MUST declare its own `@font-face` inside its `<template>` pointing at these paths (never name a font with no file). Do not use "Playfair Display" anywhere — it has no file in this project.

```css
@font-face { font-family: "Inter"; src: url("../../assets/fonts/Inter-400.woff2") format("woff2"); font-weight: 400; font-display: block; }
@font-face { font-family: "Inter"; src: url("../../assets/fonts/Inter-500.woff2") format("woff2"); font-weight: 500; font-display: block; }
@font-face { font-family: "Inter"; src: url("../../assets/fonts/Inter-600.woff2") format("woff2"); font-weight: 600; font-display: block; }
@font-face { font-family: "Inter"; src: url("../../assets/fonts/Inter-700.woff2") format("woff2"); font-weight: 700; font-display: block; }
@font-face { font-family: "Inter"; src: url("../../assets/fonts/Inter-800.woff2") format("woff2"); font-weight: 800; font-display: block; }
```

Frame files live at `compositions/frames/NN-*.html`, so the relative path from there to `assets/fonts/` is `../../assets/fonts/`.

### Broken-line / broken-column (series signature — added for this project)

Not in the original preset. Every frame that shows progression (a day, a workflow, a sequence of steps) renders it as a **broken line**, never a smooth continuous timeline: 3–5 short `line`-colored (`#8A9099`) segments, each offset vertically or with a visible gap from its neighbor, none of them ever touching or resolving into a single continuous stroke. This is the visual opposite of a fluid brand timeline and is the through-line of the whole "SANS" series — use it at least once per frame that depicts time or process (the day's arc, the TabChaos back-and-forth, the closing beat).

### Friction marks (series signature — added for this project)

Where a normal SaaS system would show a green check, this system shows one of: a red `✕` (`{colors.alert}`, small, precise — never a cartoon scribble), an empty outlined box (`line`-colored, unfilled), or a `line`-colored `?`. Never a green check or checkmark anywhere in this project.
